...
class Incident(models.Model, rm.RestModel, rm.MetaDataModel):
    ...
    def triggerGroup(self):
        ...
        body = self.getProperty("details", self.description)
        ...
        count = 0
        group = self.group
        sms_perms = f"sms_{perm}"
        email_perms = f"email_{perm}"
        while group is not None or count > 4:
            self.notifyWith(group, subject, body, sms_msg, sms_perms, email_perms)
            group = group.parent

    def notifyMembers(self, subject, body, sms_msg, sms_perms, email_perms):
        members = self.group.getMembers(perms=sms_perms, as_member=True)
        for member in members:
            member.sendSMS(sms_msg)
        members = self.group.getMembers(perms=email_perms, as_member=True)
        for member in members:
