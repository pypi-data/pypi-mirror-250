...
class DoubleField(BaseField):
    ...
    def get_byte_length(self):
        return super().get_byte_length() + 8 if self.is_set() else super().get_byte_length()
    ...
    def from_bytes(self, buffer):
        if ByteConverter.to_boolean(buffer.read(1)):
            self.set(ByteConverter.to_double(buffer.read(8)))
    ...
    def to_bytes(self):
        ...
        try:
            buffer.append(ByteConverter.from_double(self.get()))
