...
class RequestsHttpSource(Source):
    ...
    def iter_bytes(self, *, debug_dir: Path | None, resource_full_id: "ResourceFullId") -> Iterator[bytes]:
        if self._retrying is None:
            response_dump_file = None if debug_dir is None else debug_dir / f"{resource_full_id}.txt"
            yield from self._iter_bytes_attempt(
                resource_full_id=resource_full_id, response_dump_file=response_dump_file
            )
            return
        ...
        for attempt in self._retrying:
            response_dump_file = (
                ...
                else debug_dir / f"{resource_full_id}.attempt_{attempt.retry_state.attempt_number}.txt"
            )
            with attempt:
                yield from self._iter_bytes_attempt(
                    resource_full_id=resource_full_id, response_dump_file=response_dump_file
    ...
    def _iter_bytes_attempt(
        self, *, resource_full_id: "ResourceFullId", response_dump_file: Path | None
    ) -> Iterator[bytes]:
        logger.debug(
