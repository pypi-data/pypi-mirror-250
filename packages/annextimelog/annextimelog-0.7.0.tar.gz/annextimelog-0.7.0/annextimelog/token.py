    ...
    def from_strings(
        ...
        if simplify:
            ...
            match timetokens:  # this is where the magic happens 🪄
                ...
                case [  # last day # TODO
                    TimeKeywordIter() as w,
                    TimeKeywordUnit() as unit,
                ]:
                    othertokens.append(
                        TimeFrame(
                            string=s,
                            start=now.this(unit.name, w.n),
                            end=now.this(unit.name, w.n + 1),
                        )
                    )
                ...
                case [  # e.g. last week, next month, this day
                    TimeKeywordIter(name="this"),
                    TimeKeywordPeriod(
                        name="second" | "minute" | "hour" | "day" | "week" as period
                    ),
                ]:
...
class TimeKeywordUnit(TimeKeyword):
    KEYWORDS: ClassVar = set("second minute hour day".split())
