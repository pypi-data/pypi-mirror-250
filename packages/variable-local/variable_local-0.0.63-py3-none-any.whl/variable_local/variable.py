...
class VariablesLocal(GenericCRUD):
    ...
    def add(self, variable_id: int, variable_name: str) -> None:
        ...
        try:
            if variable_id is not None and variable_name is not None:
                ...
                self.name2id_dict_lower[variable_name.lower()] = variable_id
    ...
    def get_variable_id_by_variable_name(self, variable_name: str) -> int:
        ...
        if variable_name in self.name2id_dict:
            self.name2id_dict_lower[variable_name.lower()] = self.name2id_dict[variable_name]
        variable_name_lower = variable_name.lower()
        variable_id = (self.name2id_dict_lower.get(variable_name_lower) or
                       self.name2id_dict_lower.get(variable_name_lower.replace("_", " ")) or
                       self.name2id_dict_lower.get(variable_name_lower.replace(" ", "_")))
