        ...
        with zipfile.ZipFile(zip_filename, 'w') as zipf:
            for source_path in source_paths:
                if os.path.isfile(source_path):
                    # 如果是文件，将其复制到/result目录
                    shutil.copy2(source_path, destination_path)
                    # 将文件加入到zip包中
                    zipf.write(os.path.join(destination_path, os.path.basename(source_path)),
                               os.path.basename(source_path))
                elif os.path.isdir(source_path):
                    # 如果是目录，递归复制整个目录
                    for foldername, subfolders, filenames in os.walk(source_path):
                        # 添加目录到zip文件中（创建空目录）
                        zipf.write(foldername, os.path.relpath(foldername, source_path))
                        for filename in filenames:
                            ...
                            # 将文件加入到zip包中
                            zipf.write(destination_file_path, relative_path)
