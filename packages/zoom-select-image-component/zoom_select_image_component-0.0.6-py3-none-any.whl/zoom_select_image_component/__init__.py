...
# image_key is not used, but needs to be there so that cache is busted when it changes
@st.cache_resource
def image_to_bytes(_image: Image, image_key: str):
...
def image_to_url(_image: Image, image_key: str):
    ...
    image_bytes = image_to_bytes(_image, image_key)

    return media_file_manager.add(image_bytes, 'image/png', image_key)
