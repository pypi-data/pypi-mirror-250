...
class Scrol_frame(Canvas):
    def __init__(self, master, scr_x = None, scr_y = None, whereToPosition = (0, 0),anchor =  "nw", **kwargs):
        """
        ...
        self.workingFrame = frame(self)

        self.workingFrame.bind("<Configure>", lambda e: self.configure(scrollregion = self.bbox("all")))
        self.create_window(whereToPosition, window = self.workingFrame, anchor = anchor)
        if scr_y:
