...
def test_parse_1constraint():
    ...
    assert _parse_constraints(Student.attr_1cs) == [cs.not_null.value]
    assert _parse_constraints(Student.id) == [cs.auto_primary_key.value]


def test_parse_2c():
    constraints = {cs.not_null.value, cs.unique.value}
    assert set(_parse_constraints(Student.attr_2cs)) == constraints


def test_parse_3c():
    constraints = {cs.not_null.value, cs.unique.value, cs.primary_key.value}
...
def test_parse_1c_from_1v():
    assert _parse_constraints(Student.attr_str_and_cs) == [cs.not_null.value]
    assert _parse_constraints(Student.attr_int_and_cs) == [cs.not_null.value]
    assert _parse_constraints(Student.attr_bool_and_cs) == [cs.not_null.value]
    assert _parse_constraints(Student.attr_float_and_cs) == [cs.not_null.value]
...
def test_parse_ignore_constraint():
    assert _parse_constraints(Student.attr_tuple_c) == [cs.ignore.value]
    assert _parse_constraints(Student.attr_list_c) == [cs.ignore.value]
    assert _parse_constraints(Student.attr_dict_c) == [cs.ignore.value]
    assert _parse_constraints(Student.attr_set_c) == [cs.ignore.value]
