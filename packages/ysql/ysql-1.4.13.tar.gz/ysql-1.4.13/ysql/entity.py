...
_PRIMARY_KEY = _Constraint(value="PRIMARY KEY")
_AUTO_PRIMARY_KEY = _Constraint(value="PRIMARY KEY AUTOINCREMENT")
_NOT_NULL = _Constraint(value="NOT NULL")
_UNIQUE = _Constraint(value="UNIQUE")
_IGNORE = _Constraint(value="IGNORE")
...
_FOREIGN_KEY_FLAG = "FOREIGN_KEY_FLAG"
...
class Constraint:
    ...
    def default(default_value: _ENABLED_UNION):
        ...
        if type(default_value) in _ENABLED_TYPES:
            return _Constraint(value=f'DEFAULT {default_value}')  # noqa
        raise TypeError(
    ...
    def check(check_condition: str):
        ...
        if type(check_condition) == str:
            return _Constraint(value=f'CHECK({check_condition})')  # noqa
        raise TypeError(
    ...
    def foreign_key(parent_entity, parent_field, delete_link=None, update_link=None):
        ...
        if delete_link is None and update_link is None:
            foreign_key = f"FOREIGN KEY ({_FOREIGN_KEY_FLAG}) " \
    ...
    def comment(comment: str):
        ...
        if type(comment) == str:
            # 目前仅支持sqlite的注释格式
            return _Constraint(value=f'-- {comment}\n')  # noqa
        raise TypeError(
