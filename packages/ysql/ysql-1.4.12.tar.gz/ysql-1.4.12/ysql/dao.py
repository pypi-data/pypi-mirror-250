...
def Dao(entity, return_type=tuple, auto_fetch_one: bool = True, enable_substitute: bool = True):
    ...
    Args:
        entity: 该数据访问类对应的数据类。
        return_type: 全局设置查询结果中 !单条记录! 的数据类型。
...
def Sql(sql: str, return_type=None, fetch_one: bool = False):
    ...
    Args:
        sql: 固定字符串的sql语句。
        return_type: 单独设置查询结果中 !单条记录! 的数据类型，若Dao和Sql同时设置该参数，则遵循Sql的return_type。
                    应属于[tuple, dict, dataclass, namedtuple, your_entity, int, str, float, bytes, bool]
        fetch_one: 设置是否直接返回单条记录，默认使用列表存储并返回查询结果，若Dao和Sql同时设置该参数，则遵循Sql的fetch_one。

    Return:
        使用select时，将结合return_type和fetch_one自动返回指定格式的查询结果。
...
class DaoFunc:
    ...
    def execute(self, sql: str, args=None, return_type=None, fetch_one: bool = False):
        ...
        Args:
            sql: 固定字符串sql语句。
            args: 该sql需要的参数。
            return_type: 单独设置查询结果中 !单条记录! 的数据类型，若Dao和execute同时设置该参数，则遵循execute的return_type。
                        应属于[tuple, dict, dataclass, namedtuple, your_entity, int, str, float, bytes, bool]
...
def execute(self, sql: str, args=None, return_type=None, fetch_one: bool = False):
    ...
    return __fetch_result(self=self, sql=sql, return_type=return_type, fetch_one=fetch_one)
...
def __fetch_result(self, sql, return_type, fetch_one: bool):
    """确定返回类型"""
    cursor = self.cursor
    lower_sql = sql.lower()
