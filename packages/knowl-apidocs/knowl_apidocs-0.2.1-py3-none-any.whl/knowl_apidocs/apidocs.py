...
def main(repo):
    ...
    if os.path.exists(".env"):
        ...
        for line in lines:
            ...
            env_key = line_split[0]
            env_value = remove_quotes(line_split[1])
    ...
    for v in MANDATORY_ENV_VARIABLE:
        if v not in os.environ.keys():
            raise RuntimeError(f"{v} not found in ENVIRONMENT VARIABLES. Either add it to .env files or it add permanently as Environment variable")
