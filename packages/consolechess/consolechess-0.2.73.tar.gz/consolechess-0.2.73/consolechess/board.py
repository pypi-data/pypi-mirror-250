...
NestedDict = dict[str | None, Union["NestedDict", str, None]]
...
def make_openings_tree(openings: list[dict[str, str]]) -> NestedDict:
    """
    ...
    for opening in openings:
        ...
        if last_move not in cursor:
            cursor[last_move] = {}
        cursor[last_move][None] = {  # type: ignore
...
with (
    ...
    OPENINGS: NestedDict = pickle.loads(file.read())
...
class ChessBoard:
    ...
    def opening(self) -> Opening | None:
        """Get the ECO opening."""
        cursor: NestedDict = OPENINGS
        opening: NestedDict | None = None
        for move in self._moves:
            try:
                cur: NestedDict = cursor[move]  # type: ignore
                cursor = cur
                if None in cursor:
                    op = cursor[None]
