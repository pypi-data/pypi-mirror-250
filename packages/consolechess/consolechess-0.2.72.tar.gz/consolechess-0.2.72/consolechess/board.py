...
def make_openings_tree(openings: list[dict[str, str]]) -> dict:  # type: ignore
    """
    ...
    for opening in openings:
        if "bare_moves" not in opening:
            opening.update({"bare_moves": re.sub(r"\d+\.+\s+", "", opening["moves"])})
        index_chain = []
        cursor = tree
...
with (
    Path(f"{Path(__file__).parent}/openings.pkl")
    if "__file__" in globals()
    else Path("consolechess/openings.pkl")
).open("rb") as file:
    OPENINGS_TREE = pickle.loads(file.read())
