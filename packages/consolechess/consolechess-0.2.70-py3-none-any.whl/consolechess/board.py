...
with (
    ...
    else Path("consolechess/openings.csv")
).open() as file:
    OPENINGS = list(DictReader(file))
...
class ChessBoard:
    ...
    def opening(self) -> Opening | None:
        ...
        for opening in OPENINGS:
            if opening["moves"] in moves:
                return Opening(
                    eco=opening["eco"], name=opening["name"], moves=opening["moves"]
                )
        return None
    ...
    def export_moves(
        ...
        *,
        ...
        while True:
            ...
            moves = moves[2:]
        st = self.status if (st := self.status).game_over else self._status
