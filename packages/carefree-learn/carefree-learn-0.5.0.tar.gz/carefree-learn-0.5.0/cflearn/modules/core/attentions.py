...
from ...toolkit import sdp_attn
from ...toolkit import gradient_checkpoint


attentions = PrefixModules("attention")


def register_attention(name: str, **kwargs: Any) -> Callable[[TModule], TModule]:
    return attentions.register(name, **kwargs)
...
def build_attention(
    ...
    **kwargs: Any,
    ...
    return attentions.build(name, config=config, **kwargs)
...
class Attention(Module, IAttention, IHijackMixin):
...
def make_attention(in_channels: int, attention_type: str, **kwargs: Any) -> Module:
    ...
    if attentions.has(attention_type):
        return safe_execute(attentions.get(attention_type), kwargs)
