...
from ....cv.ae.vq import AttentionAutoEncoderVQ
...
class DDIMQSampler(DDPMQSampler):
    def reset_buffers(self, discretize: str, total_step: int) -> None:  # type: ignore
        if discretize == "uniform":
            t = self.model.t
            span = t // (total_step + int(t % total_step == 0) - 1)
            q_timesteps = np.array(list(range(0, t, span)) + [t - 1])
...
class DDIMMixin(ISampler, UncondSamplerMixin, metaclass=ABCMeta):
    ...
    def _get_pred_x0(
        ...
        if quantize_denoised:
            ...
            if not isinstance(first_stage, AttentionAutoEncoderVQ):
                raise ValueError(
                    err_fmt.format(
                        "model with `AttentionAutoEncoderVQ` as the `first_stage` model"
                    )
                )
