    ...
    *,
    ...
    for module in modules:
        for _ in range(num_repeat):
            experiment.add_task(module=module, **task_kw)  # type: ignore
...
def make_toy_ml_model(
    ...
    **kwargs: Any,
    ...
    module_config.update(dict(input_dim=x_np.shape[1], output_dim=output_dim))
    if module in ("fcnn", "tree_dnn"):
        module_config.update(dict(hidden_units=[100], batch_norm=False, dropout=0.0))
    # re-assign
    config.num_epoch = 2
    config.max_epoch = 4
