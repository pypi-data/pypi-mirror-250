...
class VariantFile(object):
        ...
        for info_or_format, spec in zip(
            ...
            for field, props in spec.items():
                ...
                if sz > 1:
                    ...
                    miss_value = None
                self.missing_values[info_or_format].setdefault(
                    field, miss_value
                )
    ...
    def gt_to_numpy(values):
        ...
        ar = np.zeros(
            shape=(len(gt_homogen), len(gt_homogen[0])), dtype=GT_DTYPE
        )
        ar[:] = gt_homogen
        return ar
    ...
    def tokenize_info(self, INFO):
        ...
        for field in INFO.split(";"):
            ...
            if len(keyval) == 1:
                ...
                field = key
            if (
                field not in self.info_spec
                and field not in self.fields_warned["info"]
            ):
