...
class Metadata(object):
    ...
    metadata = {}
    reason = None
    # HACK: Adding type and id, correct way is to define another metadata_class_name.
    type = None
    id = None
    ...
    def from_dict(cls: Type[M], namespace: str, metadata_dict: dict) -> M:
        ...
        # print("UNSKRIPT: metadata dict " + json.dumps(metadata_dict))
        metadata_class_name = 'unskript.secrets.elyra_metadata_metadata.Metadata'
        ...
        try:
            ...
            #print("UNSKRIPT: metadata instance " + instance.to_json())
    ...
    def to_dict(self, trim: bool = False) -> dict:
        ...
        d = dict(name=self.name, display_name=self.display_name, metadata=self.metadata, schema_name=self.schema_name, type=self.type, id=self.id)
        if not trim:
            if self.resource:
