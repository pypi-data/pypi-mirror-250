...
class RunTask(AbstractEvent):
    ...
    async def task_execution(self):
        ...
        try:
            ...
            raise TaskNotFound(
                f'Task: {self.task}: {err}'
            ) from err
        except TaskFailed as err:
            raise TaskFailed(
                f'Task {self.task} failed: {err}'
            ) from err
        except FileNotFound:
            raise
        except FileError as err:
            raise FileError(
                f'Task {self.task}, File Not Found error: {err}'
            ) from err
        except Exception as err:
            raise TaskFailed(
                f'Error: Task {self.task} failed: {err}'
            ) from err

    async def __call__(self, *args, **kwargs):
        self._logger.debug(
            f':: Running Task: {self.program}.{self.task}'
        )
