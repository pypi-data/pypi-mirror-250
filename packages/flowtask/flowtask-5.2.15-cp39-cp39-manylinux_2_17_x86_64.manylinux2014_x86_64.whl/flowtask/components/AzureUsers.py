...
class AzureUsers(Azure):
    ...
    async def get_users(self, most_recent: str = None):
        ...
        all_users = []
        if most_recent:
            utc_datetime = iso8601.parse_date(most_recent).isoformat().replace('+00:00', 'Z')
    ...
    async def run(self):
        ...
        try:
            ...
            self._logger.info(
                f'<{__name__}>: Successfully got Users Info.'
            )
            self._result = self.create_dataframe(result)
            ...
            raise ComponentError(
                f'{__name__}: Error in AzureUsers: {err}'
            ) from err
