...
#from flowtask.data_integration.task import Task
    ...
    async def get(self):
        ...
        try:
            ...
            try:
                ...
                return self.error(request=self.request, response=msg, exception=e, state=400)
            if result:
                data = []
                for row in result:
                ...
                for elements in data:
                    for key, value in elements.items():
                        elements[key] = str(value) if isinstance(
                            value, (UUID, datetime.time, datetime.datetime)) else value
                headers = {
                    'X-STATUS': 'OK',
                    'X-MESSAGE': 'Tasks List'
                }
                return self.json_response(response=data, headers=headers)
            else:
                if self._lasterr:
                    msg = {
                        "error": str(self._lasterr)
                    }
                    return self.error(request=self.request, response=msg, exception=self._lasterr, state=400)
                headers = {
                    'X-STATUS': 'EMPTY',
    ...
    async def post(self):
        ...
        try:
            ...
            try:
                ...
                return self.error(request=self.request, response=msg, headers=headers, state=400)
        try:
        ...
        try:
            ...
            try:
                ...
                headers = {
                    'X-STATUS': 'Task OK',
                    'X-MESSAGE': 'Execution of Task {}.{} on {}'.format(program, task_id, action)
                }
                ...
                print(err)
                error = 'Error on Task {} File Not Found: {}'.format(
                    task_id, str(err))
                msg = {
                    "message": error
                }
