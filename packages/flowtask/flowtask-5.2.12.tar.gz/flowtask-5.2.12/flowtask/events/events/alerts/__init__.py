...
class Alert(Notification, AbstractEvent):
    ...
    async def __call__(self, *args, **kwargs):
        ...
        if self.system_checks:
            rst = await self.process_checks(
                self.system_checks, stats, task_name
            )
            errors += rst
        ...
        if self.result_checks:
            ...
            rst = await self.process_checks(
                self.result_checks, data, task_name
            )
            errors += rst
    ...
    def get_pandas_function(self, payload: dict):
        ...
        try:
            ...
            self._logger.warning(
                f"Function {fname} does not exist on Alert System"
            )
    ...
    async def process_checks(self, checks, data, task_name):
        ...
        for check in checks:
            ...
            if result is False:
                ...
                errors.append(
                    {"function": fname, "column": colname, "value": value}
                )
        return errors
