...
class Rsync(RunSSH):
    ...
    def __init__(
        ...
        self.flags: str = 'azrv'
        super(Rsync, self).__init__(loop, job, stat, **kwargs)
    ...
    async def start(self, **kwargs):
        ...
        if hasattr(self, 'destination'):
            ...
            self.dest_server = self.mask_replacement(
                self.destination['server']
            )
            self.dest_user = self.mask_replacement(
                self.destination['user']
            )
            try:
                self.dest_port = self.mask_replacement(
    ...
    async def run(self):
        ...
        command = rsync.format(flags=self.flags, destination=destination, source=self.source_dir)
        try:
