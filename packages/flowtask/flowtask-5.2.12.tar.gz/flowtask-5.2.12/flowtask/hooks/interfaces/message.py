...
class Notification(MaskInterface):
    ...
    def get_notify(self, **kwargs):
        if self.provider == 'telegram':
            recipient = Chat(
                **{"chat_id": EVENT_CHAT_ID, "chat_name": self._channel}
            )
            ...
            _id = self._channel_id if self._channel_id else MS_TEAMS_NAVIGATOR_CHANNEL_ID
        ...
        logging.debug(
            f"Created Notify Component {provider}"
        )
