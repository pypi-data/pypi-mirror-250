...
class kidLoggerFileArrange():
    ...
    def getCreatDate(self, filepath):
        ...
        modified_datetime = datetime.strptime(f"{year}-{month}-{day}","%Y-%B-%d")
        if month == "December" and modified_datetime > datetime.now():
            modified_date = datetime.strptime(f"{int(year)-1}-{month}-{day}","%Y-%B-%d").date()
        else:
