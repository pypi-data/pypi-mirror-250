...
class RunOracle:
    ...
    def getTNS(self, dbJson):
        ...
        sqlStr = f"WITH tmp_data AS (SELECT pid, user_name, user_pwd, tns_content FROM DB_CONFIG WHERE host_name = '{host_name}' AND port = {port} AND container = '{container}' AND tb_space = '{tb_space}'),
                    tmp_root AS
                    (SELECT tns_content FROM DB_CONFIG WHERE ID = (SELECT pid FROM tmp_data))
                    SELECT a.user_name, a.user_pwd, NVL(a.tns_content, b.tns_content) AS tns_content
                    FROM tmp_data a
                    LEFT JOIN tmp_root b
                    ON 1 = 1
                    "
