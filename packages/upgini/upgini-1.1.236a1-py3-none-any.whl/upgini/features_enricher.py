...
class FeaturesEnricher(TransformerMixin):
    ...
    def __sample_balanced(
        ...
        if EVAL_SET_INDEX in original_df_sampled.columns:
            X_sampled = original_df_sampled.query(f"{EVAL_SET_INDEX} == 0")
        else:
            X_sampled = original_df_sampled
        X_sampled = drop_existing_columns(X_sampled, [SYSTEM_RECORD_ID, EVAL_SET_INDEX, TARGET])
        search_keys = self.fit_search_keys
        ...
        if eval_set is not None:
            ...
            for idx in range(len(eval_set)):
                ...
                eval_X_sampled = original_df_sampled.query(f"{EVAL_SET_INDEX} == {idx + 1}")
                eval_X_sampled = drop_existing_columns(eval_X_sampled, [SYSTEM_RECORD_ID, EVAL_SET_INDEX, TARGET])
