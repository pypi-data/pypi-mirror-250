...
class LayerNextClient:
    ...
    def create_or_update_job(
        ...
        int, float)) or not isinstance(JobStatus(status), JobStatus) or not isinstance(JobType(job_type), JobType)):
    ...
    def retrieve_documents(
        ...
        if doc_list and isinstance(doc_list, list) and len(doc_list) > 0: # Check if the list is not empty and get the score of the first document
        ...
        for doc in filtered_doc_list:
            doc['chunk_list'] = chunk_dict.get(doc['uniqueName'], [])
            # Remove the 'score' field from the document if it exists
            if 'score' in doc:
                del doc['score']
