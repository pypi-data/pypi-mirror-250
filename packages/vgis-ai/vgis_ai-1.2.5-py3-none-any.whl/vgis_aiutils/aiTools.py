...
class AIHelper:
    ...
    # image_shape:(width,height)
    # image_map_env:(map_minx,map_miny,map_maxx,map_maxy)
    def convert_mmdet_pred_to_shape(mmdet_prediction,names, image_path, image_map_env, map_epsg, out_boxs_shp_path,out_polys_shp_path):
        img = cv2.imread(image_path)
        image_shape = img.shape
        map_box_coords_list = []
        ...
        for index in range(len(mmdet_prediction["labels"])):
            ...
            if out_polys_shp_path is not None:
                map_poly_coords_list.append([poly, conf, cls, name])
    ...
    def draw_polys_on_image(base_image_path, polys, merge_image_path):
        ...
        for poly in polys:
            polys_points = np.array([poly], dtype=np.int8)
    ...
    def convert_polys_to_masks(polys, width, height):
        masks = np.zeros((width, height), dtype=np.int8)
        obj = np.array(polys, dtype=np.int8)
    ...
    # mask二值图转shp
    # 通过gdal的栅格转矢量的方法
    def convert_mask_image_to_shape(mask_image_path: str, mask_image_tranform: list, file_prefix: str, shp_file_path: str,
                              logger: object) -> str:
        """
