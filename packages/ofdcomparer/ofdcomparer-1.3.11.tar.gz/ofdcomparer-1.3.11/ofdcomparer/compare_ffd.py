...
class ComparerOfd:
    ...
    def compare_etalon_fn_ofd(self, etalon: dict, changes: dict = None,
        ...
        logging.info(f"etalon: {etalon=}")
        logging.info(f"comparable: {comparable=}")
        logging.info(f"compare_result: {self.result=}")
    ...
    def compare_last_fd_in_fn_and_ofd(
        ...
        logging.debug(f"compare_result: {self.result}")
        self.output_result_to_log()
        logging.debug(f"compare_result: {self.result}")
    ...
    def compare_tags(
        ...
        logging.debug(f"compare_tags() < {rnm=}, {fn=}, {fd=}")
        try:
            for _ in range(200):
    ...
    def __expand_stlv(self, key, comparable, etalon):
        ...
        if self.__is_list(comparable, etalon):
            ...
            for i in range(len(etalon)):
                logging.debug("iteration ", count)
                count += 1
                logging.debug("comparable[i] ", comparable[i])
                logging.debug("etalon[i] ", etalon[i])
                self.compare(comparable=comparable[i], etalon=etalon[i])
            return True
        if isinstance(comparable, dict) and isinstance(etalon, dict):
            logging.debug("isinstance(comparable, dict) and isinstance(etalon, dict)")
            count = 0
            for key, value in etalon.items():
                logging.debug("iteration ", count)
                count += 1
                logging.debug("comparable ", comparable[key])
                logging.debug("etalon ", etalon[key])
