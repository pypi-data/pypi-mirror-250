...
class DTO10Helper:
    def __init__(self, sigma: bool = False, ip: str = ""):
        while True:
            try:
                if platform.system() == 'Linux':
                    self.fptr = IFptr(lib_path=os.path.join(CURRENT_PATH, 'linux'))  # Создание объекта ДТО
                elif platform.system() == 'Darwin':
                    self.fptr = IFptr(lib_path=os.path.join(CURRENT_PATH, 'mac'))  # Создание объекта ДТО
                else:
                    self.fptr = IFptr(lib_path=os.path.join(CURRENT_PATH, 'windows'))  # Создание объекта ДТО
            except Exception:
        ...
        if sigma:
            self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_MODEL, str(99999))
            self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_PORT, str(IFptr.LIBFPTR_PORT_TCPIP))
            self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_IPADDRESS,
                                       str(ip))
            self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_IPPORT, '15000')
            self.fptr.setSingleSetting(IFptr.LIBFPTR_SETTING_AUTO_RECONNECT, str(True))
        self.fptr.applySingleSettings()  # Запись настроек
