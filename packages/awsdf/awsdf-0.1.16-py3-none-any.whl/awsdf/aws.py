...
class Account(object):
    ...
    def quicksight_get_datasources(self) -> pd.DataFrame:
        ...
        Q_DATASOURCE_COLS = ['Arn', 'DataSourceId', 'Name', 'Type', 'Status',
                             'CreatedTime', 'LastUpdatedTime', 'DataSourceParameters']
        lst_datasources = wr.quicksight.list_data_sources(
            boto3_session=self.session)
    ...
    def quicksight_get_datasets(self, includeDetails: bool = False) -> pd.DataFrame:
        ...
        COL_LIST = ['Arn', 'DataSetId', 'Name', 'CreatedTime',
                    'LastUpdatedTime', 'ImportMode']
        if includeDetails:
            datasets_df['dataset_details_dict'] = datasets_df.progress_apply(
                lambda row: self.quicksight_get_dataset_details(row['DataSetId']), axis=1)
            """
            ...
            """
            datasets_df['ConsumedSpiceCapacityInBytes'] = datasets_df['dataset_details_dict'].apply(
                lambda x: x.get('ConsumedSpiceCapacityInBytes'))
            COL_LIST.append("ConsumedSpiceCapacityInBytes")

            # Get dataset permissions
            account_id = self.iam_get_accountid()
            datasets_df['Permissions'] = datasets_df.progress_apply(lambda row: self.quicksight_get_dataset_permissions(
                AwsAccountId=account_id, DataSetId=row['DataSetId']), axis=1)
            datasets_df['Owner'] = datasets_df['Permissions'].apply(
                lambda x: ','.join([str(owner['Principal']).split("/")[-1] for owner in x]))
    ...
    def quicksight_get_dataset_permissions(self, AwsAccountId: str, DataSetId: str):
        ...
        client_quicksight: boto3.client = self.session.client(
            AWS_QUICKSIGHT_CLIENT)
    ...
    def quicksight_get_dataset_details(self, datasetId: str) -> dict:
        ...
        try:
            ...
            if str(e).__contains__("DataSet type flatFile is not supported through API yet"):
                logger.warning(
                    f"Skipping flat file dataset type. {datasetId=}")
            elif str(e).__contains__("The data set type is not supported through API yet"):
                logger.warning(
                    f"Skipping unsupported dataset type {datasetId=}")
            elif str(e).__contains__("DataSet type genericTable is not supported through API yet"):
                logger.warning(
                    f"Skipping genericTable dataset type {datasetId=}")
            else:
                raise Exception(e)
    ...
    def quicksight_get_dashboards(self, includeDetails: bool = False) -> pd.DataFrame:
        ...
        lst_dashboards = wr.quicksight.list_dashboards(
            boto3_session=self.session)
        dashboards_df = pd.DataFrame(lst_dashboards)
        COL_LIST = ['Arn', 'DashboardId', 'Name', 'CreatedTime',
                    'LastUpdatedTime', 'PublishedVersionNumber', 'LastPublishedTime']
        # if includeDetails:
