...
file_map = {} or Redis(**os.getenv("REDIS_CLIENT_PARAMS", {}))  # redis_client_params
...
async def get_files(
    ...
    try:
        ...
        response = minio_client.get_object(OPENAI_BUCKET, file_name)
        data = response.read()
...
async def get_files_content(
    ...
    try:
        file_name = file_map.get(file_id)
        response = minio_client.get_object(OPENAI_BUCKET, file_name)  # 获取链接
        data = response.data
...
async def upload_files(
    ...
    if purpose == Purpose.fine_tune:
        ...
        minio_client.put_object(OPENAI_BUCKET, file.filename, data=file.file, length=file.size)

        # fileid2filename
        file_id = f"file-{hash(file.filename)}"
        file_map[file_id] = file.filename  # shortuuid.uuid('a')
        logger.debug(file_map)

        # 按次计费
        _ = get_ppu_usage(api_key=api_key, model='ppu-file')
