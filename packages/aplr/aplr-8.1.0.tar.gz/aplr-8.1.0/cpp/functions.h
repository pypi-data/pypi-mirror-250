...
double calculate_group_mse_by_prediction_validation_error(const VectorXd &y, const VectorXd &predictions, const VectorXd &sample_weight,
                                                          const std::set<int> &unique_prediction_groups,
                                                          double dispersion_parameter, double quantile)
{
    VectorXi group{create_groups_for_group_mse_sorted_by_vector(predictions, unique_prediction_groups)};
    return calculate_mean_error(calculate_errors(y, predictions, sample_weight, "group_mse_cycle",
                                                 dispersion_parameter, group, unique_prediction_groups, quantile),
                                sample_weight);
}

double calculate_tuning_metric(const std::string &validation_tuning_metric, const std::string &loss_function, const VectorXd &y,
                               const VectorXd &predictions, const VectorXd &sample_weight, const VectorXi &group, const MatrixXd &other_data,
                               double dispersion_parameter, const std::set<int> &unique_groups, double quantile,
                               const std::set<int> &unique_prediction_groups,
                               const std::function<double(const VectorXd &y, const VectorXd &predictions, const VectorXd &sample_weight, const VectorXi &group, const MatrixXd &other_data)> &calculate_custom_validation_error_function,
                               const std::function<double(const VectorXd &y, const VectorXd &predictions, const VectorXd &sample_weight, const VectorXi &group, const MatrixXd &other_data)> &calculate_custom_loss_function)
{
    if (validation_tuning_metric == "default")
    {
        if (loss_function == "custom_function")
        {
            try
            {
                return calculate_custom_loss_function(y, predictions, sample_weight, group, other_data);
            }
            catch (const std::exception &e)
            {
        ...
        {
            return calculate_group_mse_by_prediction_validation_error(y, predictions, sample_weight, unique_prediction_groups, dispersion_parameter, quantile);
        }
        else
            return calculate_mean_error(calculate_errors(y, predictions, sample_weight, loss_function, dispersion_parameter, group, unique_groups, quantile), sample_weight);
    }
    else if (validation_tuning_metric == "mse")
        return calculate_mean_error(calculate_errors(y, predictions, sample_weight, MSE_LOSS_FUNCTION), sample_weight);
    else if (validation_tuning_metric == "mae")
        return calculate_mean_error(calculate_errors(y, predictions, sample_weight, "mae"), sample_weight);
    else if (validation_tuning_metric == "negative_gini")
        return -calculate_gini(y, predictions, sample_weight);
    else if (validation_tuning_metric == "group_mse")
    {
        bool group_is_not_provided{group.rows() == 0};
        ...
        return calculate_mean_error(calculate_errors(y, predictions, sample_weight, "group_mse", dispersion_parameter, group, unique_groups, quantile), sample_weight);
    }
    ...
    else if (validation_tuning_metric == "custom_function")
    {
        try
        {
            return calculate_custom_validation_error_function(y, predictions, sample_weight, group, other_data);
        }
        catch (const std::exception &e)
        {
