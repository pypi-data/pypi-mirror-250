...
public:
    ...
    double cv_error;
    bool use_tuning_metric_for_variable_selection;
    ...
    APLRRegressor(size_t m = 1000, double v = 0.1, uint_fast32_t random_state = std::numeric_limits<uint_fast32_t>::lowest(), std::string loss_function = "mse",
                  ...
                  bool use_tuning_metric_for_variable_selection = false);
...
APLRRegressor::APLRRegressor(size_t m, double v, uint_fast32_t random_state, std::string loss_function, std::string link_function, size_t n_jobs,
    ...
    : reserved_terms_times_num_x{reserved_terms_times_num_x}, intercept{NAN_DOUBLE}, m{m}, v{v},
      ...
      use_tuning_metric_for_variable_selection{use_tuning_metric_for_variable_selection}
{
}
...
APLRRegressor::APLRRegressor(const APLRRegressor &other)
    : reserved_terms_times_num_x{other.reserved_terms_times_num_x}, intercept{other.intercept}, terms{other.terms}, m{other.m}, v{other.v},
      ...
      use_tuning_metric_for_variable_selection{other.use_tuning_metric_for_variable_selection}
{
}
...
void APLRRegressor::estimate_split_point_for_each_term(std::vector<Term> &terms, std::vector<size_t> &terms_indexes)
{
    ...
    bool do_variable_selection_based_on_validation_tuning_metric{use_tuning_metric_for_variable_selection && validation_tuning_metric != "default"};
    if (do_variable_selection_based_on_validation_tuning_metric)
    {
        for (size_t i = 0; i < terms_indexes.size(); ++i)
        {
            VectorXd term_contribution_to_linear_predictor{terms[terms_indexes[i]].calculate_contribution_to_linear_predictor(X_train)};
            VectorXd linear_predictor{linear_predictor_current + term_contribution_to_linear_predictor};
            VectorXd predictions_train{transform_linear_predictor_to_predictions(linear_predictor, link_function, calculate_custom_transform_linear_predictor_to_predictions_function)};
            terms[terms_indexes[i]].training_loss = calculate_tuning_metric(validation_tuning_metric, loss_function, y_train, predictions_train,
                                                                            sample_weight_train, group_train, other_data_train,
                                                                            dispersion_parameter, unique_groups_train, quantile,
                                                                            unique_prediction_groups, calculate_custom_validation_error_function, calculate_custom_loss_function);
        }
    }
}
...
size_t APLRRegressor::find_best_term_index(std::vector<Term> &terms, std::vector<size_t> &terms_indexes)
{
    ...
    for (auto &term_index : terms_indexes)
    {
        ...
        if (term_is_eligible)
        {
            if (std::isless(terms[term_index].training_loss, lowest_errors_sum))
            {
                best_term_index = term_index;
                lowest_errors_sum = terms[term_index].training_loss;
            }
        }
    }
...
VectorXi APLRRegressor::find_indexes_for_terms_to_consider_as_interaction_partners()
{
    ...
    VectorXd training_losses(terms.size());
    ...
    for (size_t i = 0; i < terms.size(); ++i)
    {
        ...
        {
            training_losses[count] = terms[i].training_loss;
            indexes_for_terms_to_consider_as_interaction_partners[count] = i;
            ++count;
        }
    }
    training_losses.conservativeResize(count);
    ...
    {
        VectorXi sorted_indexes{sort_indexes_ascending(training_losses)};
...
void APLRRegressor::find_sorted_indexes_for_errors_for_interactions_to_consider()
{
    ...
    {
        errors_for_interactions_to_consider[i] = interactions_to_consider[i].training_loss;
    }
    ...
    for (Eigen::Index j = 0; j < sorted_indexes_of_errors_for_interactions_to_consider.size(); ++j) // For each interaction to consider starting from lowest to highest error
    {
        ...
        if (allowed_to_add_one_interaction)
        {
            if (best_term_before_interactions_was_not_selected)
                error_is_less_than_for_best_term_before_interactions = std::isless(interactions_to_consider[sorted_indexes_of_errors_for_interactions_to_consider[j]].training_loss, neg_gradient_nullmodel_errors_sum);
            else
                error_is_less_than_for_best_term_before_interactions = std::isless(interactions_to_consider[sorted_indexes_of_errors_for_interactions_to_consider[j]].training_loss, terms_eligible_current[best_term_before_interactions].training_loss);
...
{
    validation_error_steps.col(0)[boosting_step] = calculate_tuning_metric(validation_tuning_metric, loss_function, y_validation,
                                                                           predictions_current_validation, sample_weight_validation,
                                                                           group_validation, other_data_validation, dispersion_parameter,
                                                                           unique_groups_validation, quantile, unique_prediction_groups,
...
void APLRRegressor::update_term_eligibility()
{
    ...
    {
        VectorXd terms_eligible_current_training_losses(terms_eligible_current.size());
        for (size_t i = 0; i < terms_eligible_current.size(); ++i)
        {
            terms_eligible_current_training_losses[i] = terms_eligible_current[i].training_loss;
        }
        VectorXi sorted_training_losses_indexes{sort_indexes_ascending(terms_eligible_current_training_losses)};

        size_t count{0};
        for (size_t i = 0; i < terms_eligible_current.size(); ++i)
        {
            bool term_is_eligible_now{terms_eligible_current[sorted_training_losses_indexes[i]].ineligible_boosting_steps == 0};
            if (term_is_eligible_now)
            {
                ...
                if (term_should_receive_added_boosting_steps)
                    terms_eligible_current[sorted_training_losses_indexes[i]].ineligible_boosting_steps += ineligible_boosting_steps_added;
            }
            else
            {
                terms_eligible_current[sorted_training_losses_indexes[i]].ineligible_boosting_steps -= 1;
            }
        }
    }
