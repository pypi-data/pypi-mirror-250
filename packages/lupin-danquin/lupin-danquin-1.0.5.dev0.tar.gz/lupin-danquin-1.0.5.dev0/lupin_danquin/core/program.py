...
def remove_indesirable_spaces(input_txt: str) -> str:
    return input_txt.replace(" -", "-").replace("    -", "   -")
...
def get_program_description(program_path: str, program_file: str) -> str:
    ...
    for line in input_txt:
        if start_doc:
            doc_line = line.split(comment_txt)[-1].rstrip()
            doc_lines.append(remove_indesirable_spaces(doc_line))
        else:
            break
        # first code line
