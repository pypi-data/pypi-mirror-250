...
# Show regression statistics when putting a linear regression on a plot
...
class MapUI(object):
	...
	def add_menu(self, table_object, column_headers):
		...
		def toggle_show_regr_stats():
			global show_regression_stats
			dlg = OneCheckboxDialog("Hide/Show Regression Stats", "Show regression statistics on plots", show_regression_stats)
			rv = dlg.show()
			if rv is not None:
				show_regression_stats = rv == "1"
      ...
      Copyright %s, R Dreas Nielsen
		...
		plot_menu.add_command(label="Regr. stats", command = toggle_show_regr_stats, underline=6)
...
class PlotDialog(object):
				...
				if self.linreg and self.x_var.get() in self.numeric_columns:
					...
					if show_regression_stats:
						...
						statdlg = MsgDialog(title="Linear Regression", message = "Slope: %s\nIntercept: %s\nR-square: %s\nN: %s" % \
								(round_figs(slope, 4), round_figs(intercept, 4), round_figs(exp_ss/total_ss, 4), N))
						statdlg.show()
	...
	def set_alpha(self, *args):
		...
		if new_alpha is not None:
			...
			if self.type_var.get() in ("Scatter plot", "Line plot", "Categorical stripchart", "Categorical KD plot"):
				self.q_redraw()
...
class UnivarStatsDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.numeric_columns = [c[0] for c in self.column_specs if c[1] in ("int", "float")]
		self.numeric_columns.sort()
		self.dnames = ["Variable", "N", "Min", "Max", "Mean", "Median", "Mode", "Std.Dev.", "Log10 mean", "Log10 std.dev."]
		self.statdata = []
	...
	def recalc(self):
		...
		# Statistics are: 1) N (non-missing), 2) minimum, 3) maximum, 4) mean, 5) median,
		# 6) mode, 7) standard deviation, 8) mean of Log-10 tranforme (if applicable), and 9) std. dev. of Log-10 transform.
		...
		if self.data_labels is not None:
			for i in range(len(self.data_labels)):
				...
				if len(d) > 0:
					...
					if log_error:
						logd = []
					dd = [self.data_labels[i], len(d), min(d), max(d), statistics.fmean(d), statistics.median(d), \
							statistics.mode(d), statistics.stdev(d)]
					if len(logd) > 0:
						dd.extend([statistics.fmean(logd), statistics.stdev(logd)])
					else:
						dd.extend([None, None])
					self.statdata.append(dd)
		#if len(self.statdata) > 0:
		tframe, tdata = treeview_table(self.content_frame, self.statdata, self.dnames)
		tframe.grid(row=0, column=0, sticky=tk.NSEW)
	...
	def clear_output(self):
		...
		self.dlg.bind("<Control-s>")
		self.dataset = None
		tframe, tdata = treeview_table(self.content_frame, [], self.dnames)
		tframe.grid(row=0, column=0, sticky=tk.NSEW)
	def save_table(self, *args):
		export_data_table(self.dnames, self.statdata, sheetname="Selected map items")
	def show(self):
		self.dlg.wait_window(self.dlg)
	def do_close(self, *args):
	...
	def recalc(self):
		...
		if self.dataset is not None:
			...
			if N > 1:
				...
				self.statdata.append(["Pearson's r", round_figs(statistics.correlation(self.dataset[0], self.dataset[1]), 3)])
				try:
					self.statdata.append(["Spearman's rho", round_figs(statistics.correlation(self.dataset[0], self.dataset[1], method="ranked"), 3)])
				except TypeError:
					pass
				ns, stats = Polynomial.fit(np.array(self.dataset[0]), np.array(self.dataset[1]), 1, full=True)
				intercept, slope = ns.convert().coef
				self.statdata.append(["Regression slope", round_figs(slope, 3)])
				self.statdata.append(["Regression intercept", round_figs(intercept, 2)])
				total_ss = sum([(self.dataset[1][i] - ymean)**2 for i in range(N)])
				resid_ss = list(stats[0])[0]
				exp_ss = total_ss - resid_ss
				rsquare = exp_ss / total_ss
				self.statdata.append(["R square", round_figs(rsquare, 3)])
				self.statdata.append(["Total SS", round_figs(total_ss, 3)])
				self.statdata.append(["Explained SS", round_figs(exp_ss, 3)])
				self.statdata.append(["Residual SS", round_figs(resid_ss, 3)])
				self.plot_axes.scatter(self.dataset[0], self.dataset[1], alpha=self.alpha)
				self.plot_axes.set_xlabel(self.data_labels[0])
				self.plot_axes.set_ylabel(self.data_labels[1])
				self.plot_axes.axline((xmean, ymean), slope=slope, clip_on=True, label="Linear fit", color="black", linestyle="dotted", alpha=0.65)
				self.plotfig_canvas.draw()
				self.plot_nav.update()
				self.dlg.bind("<Alt-a>", self.set_alpha)
		if len(self.statdata) > 0:
...
class CategCorrespDialog(object):
	...
	def recalc(self):
		...
		# Create the summary
		sqlcmd = """select %s, count(*) as data_rows, 100 * count(*)/total_rows as percent
