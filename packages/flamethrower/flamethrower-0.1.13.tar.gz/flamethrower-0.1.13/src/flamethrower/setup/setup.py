...
def setup_zsh_env() -> dict | None:
    ...
    setup_source_files = os.path.join(os.getcwd(), 'src', 'flamethrower', 'setup', 'src')

    zshrc_path = get_zsh_config_path()
    if not os.path.exists(zshrc_path):
        source_zshrc_path = os.path.join(setup_source_files, '.zshrc')
        shutil.copy(source_zshrc_path, zshrc_path)
    
    flamethrower_gitignore_path = get_flamethrower_gitignore_path()
    if not os.path.exists(flamethrower_gitignore_path):
        source_gitignore_path = os.path.join(setup_source_files, '.sample.gitignore')
        shutil.copy(source_gitignore_path, flamethrower_gitignore_path)
