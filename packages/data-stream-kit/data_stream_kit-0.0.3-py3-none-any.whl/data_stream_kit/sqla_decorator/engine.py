...
from .utils.tool import md5string
...
def get_default_kwargs(url, kwargs):
    if 'sqlite:' in url:
        # sqlite 设置连接池大小等参数会报错
        remove_keys = ['pool_size', 'max_overflow', 'pool_recycle', 'pool_pre_ping']
        for key in remove_keys:
            if key in kwargs:
                kwargs.pop(key)

    if 'create_table_metadata' in kwargs:
        kwargs.pop('create_table_metadata')

    return kwargs


def disconnect(alias=DEFAULT_CONNECTION_NAME):
    engine = _dbs.get(alias)
    if not engine:
        return
...
class ScopedSessionEngine(scoped_session):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self._last_item = None  # 防止重复提交rollback  commit close

    def __getattr__(self, item):
        logger.info('ScopedSessionEngine.__getattr__ %s', item)
        self._last_item = item
        return getattr(self, item)


class SqlaEngine(object):
    """
    sqlalchemy 连接
    """

    # listeners=[MyListener()]
    # pool_pre_ping 每次有一个连接从连接池检出的时候，
    # pre ping 会发出一个 “SELECT 1” 来检查 SQL 连接是否有效，如果有无效链接，则会重置此链接以及所有早于此连接的连接
    # listeners 监听报错
    # pool_recycle 连接池重连时间
    # max_overflow 连接池溢出大小
    # pool_reset_on_return 每次查询结束执行操作 默认rollback  查询的话可以不做任何操作(节约10m延时) 插入必须手动commit
    # echo 是否显示执行 sql
    ...
    """  # noqa
        ...
        self._url = url
        self._url_key = md5string(url)[:16]
        self._autoflush = autoflush
        self._autocommit = autocommit
        self._alias = alias
        self._expire_on_commit = expire_on_commit
        self._create_table_metadata = kwargs.get('create_table_metadata') or []
        self._kwargs = get_default_kwargs(url, kwargs=kwargs)
        self._engine = create_engine(url, **self._kwargs)

        self._session = ScopedSessionEngine(sessionmaker(autocommit=autocommit,
                                                         autoflush=autoflush,
                                                         expire_on_commit=expire_on_commit))
        self._session.configure(bind=self._engine)
        if create_table:
            for metadata in self._create_table_metadata:
                metadata.create_all(self._engine)

        self.add_alias_info()

    def add_alias_info(self):
        """
        将当前engine 放入全局变量中
        :return:
        """
        uri_key = self._url_key
        if self._alias in _dbs and self._alias == DEFAULT_CONNECTION_NAME:
            warnings.warn('A different connection with alias `{}` was already'
                          "registered. Use disconnect() first".format(self._alias))

        _dbs[uri_key] = self
        if self._alias not in _dbs:
            _dbs[self._alias] = self

    @property
    def session(self) -> Session:
        _session = self._session()
        setattr(_session, 'engine', self)
        return _session
...
def create_db_engine(url,
    ...
    :param db_uri:
    :param alias: 连接别名  后续db_session 可以根据 alias进行切换
    :param pool_size: 连接池大小
    :param max_overflow: 连接池溢出大小
    :param pool_recycle: 连接池重连时间
    :param pool_pre_ping: 每次有一个连接从连接池检出的时候，
    :param pool_reset_on_return: 每次查询结束执行操作 默认rollback  查询的话可以不做任何操作(节约10m延时)
                                 插入必须手动commit
    :param echo: 是否显示执行 sql
    :param create_table: 是否创建表
