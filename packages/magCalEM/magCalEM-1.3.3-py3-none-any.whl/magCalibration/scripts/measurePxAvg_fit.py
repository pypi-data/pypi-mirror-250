...
def method2(img, lattice_res, pxGuess, savgol_window, supervised_angle, NWPS_dir, filename, write_dir, supervised_smooth, aliased):
    ...
    minradi = int((size[0]*(pxGuess-pxGuess*leeway_minus))/lattice_res)
    maxradi = int((size[0]*(pxGuess+pxGuess*leeway_plus))/lattice_res)
    if aliased:
        str_alias = 'y'
        minradi = int((size_alias*(pxGuess-pxGuess*leeway_minus))/lattice_res)
        maxradi = int((size_alias*(pxGuess+pxGuess*leeway_plus))/lattice_res)   
    ...
    if accept == "True" and end != "True":
        if supervised_smooth != True:
            ...
            radi = int((size_alias*(px-px*leeway_minus))/lattice_res)
            radi2 = int((size_alias*(px+px*leeway_plus))/lattice_res) 
            #bounds
            ...
            #plt.savefig(f"{write_dir}test.png")
