...
class MCNeuralNetwork:
    ## Default Functions ## 
    def __init__(self, hiddenCounts:list = [100], activations: any = 'DEFAULT'):
        """
...
class MCSimpleEnsemble:
    def __init__(self, n_estimators = 10, hidden_counts = [100,], activations = "DEFAULT"):
        ...
        self.weights = []
        self.fitted = False
        self.base_model = MCNeuralNetwork(hiddenCounts=hidden_counts, activations=activations)
...
def generate_model(X, Y, hidden_layers = [100], 
        ...
        model = MCNeuralNetwork(hiddenCounts=hidden_layers, activations=af_start)
...
def run_module_tests() -> None:
    ...
    model.fit(xt, yt, Ieta=100, Beta=10, Gamma=50, verbose=False)
...
def build_model(X, Y, max_hidden_layers=2, base_height=50, 
    ...
    if test_ends:
        ...
        for i, l1 in enumerate(activation_tests):
            # Make test model
            model = MCNeuralNetwork(hiddenCounts=[], activations=[l1, 'lin'])
    ...
    if test_ends:
        ...
        for i, l1 in enumerate(activation_tests[1:]):
            # Make test model
            model = MCNeuralNetwork(hiddenCounts=[], activations=[input_activation, l1])
    ...
    for layer in range(max_hidden_layers):
        ...
        for i, l1 in enumerate(activation_tests):
            # Make test model
            model = MCNeuralNetwork(hiddenCounts=model_layer_sizes, activations=[input_activation] + model_activations + [l1] + [output_activation])
    ...
    model = MCNeuralNetwork(hiddenCounts=model_layer_sizes, activations=[input_activation] + model_activations + [output_activation])
