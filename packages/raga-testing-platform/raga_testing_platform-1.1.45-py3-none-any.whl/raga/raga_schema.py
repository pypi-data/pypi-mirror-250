...
class CategoricalFeatureSchemaElement(FeatureSchemaElement):
    def __init__(self):
        super().__init__()
...
class FMA_LLMRules():
    ...
    def add(self, metric:str, metric_threshold:float, eval_metric:str, threshold:float, score_column:str):
        from raga import REQUIRED_ARG_V2
        assert isinstance(metric, str) and metric, f"{REQUIRED_ARG_V2.format('metric', 'str')}"
        assert isinstance(metric_threshold, float) and metric_threshold, f"{REQUIRED_ARG_V2.format('metric_threshold', 'float')}"
        assert isinstance(threshold, float) and threshold, f"{REQUIRED_ARG_V2.format('threshold', 'float')}"
        assert isinstance(eval_metric, str) and eval_metric, f"{REQUIRED_ARG_V2.format('eval_metric', 'str')}"
        assert isinstance(score_column, str) and score_column, f"{REQUIRED_ARG_V2.format('score_column', 'str')}"
        self.rules.append({"metric":metric, "threshold":metric_threshold, "evalMetric":eval_metric, "evalThreshold":threshold, "scoreColName": score_column})
    def get(self):
        return self.rules
