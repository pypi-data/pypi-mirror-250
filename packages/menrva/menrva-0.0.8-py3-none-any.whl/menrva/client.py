...
class MenrvaClient(distributed.Client):
    ...
    @staticmethod
    def instantiate_module(plugin, *args, **kwargs):
    ...
    def load_plugin(self, directory, plugin, name, *args, **kwargs):
        plugin_file = ".".join((plugin, "py"))
        if pathlib.Path(directory).joinpath(plugin_file).exists():
            plugin = MenrvaClient.instantiate_module(directory + "." + plugin, *args, **kwargs)
...
def local_client(
    ...
    **Example Usage**
    ...
    path = str(pathlib.Path(__file__).parent.resolve().joinpath("plugins/"))

    if local_cache or autorestrictor:
        dask.config.set({
            "distributed.scheduler.preload": os.path.join(path, 'plugins/scheduler.py')
        })
    ...
    if local_cache or _worker_log_params:
        client.load_plugin(
            ...
            plugin="worker",
            name="worker_logger",
            local_chace=local_cache,
            log_params=_worker_log_params
        )
...
def slurm_cluster_client(
    ...
    #    "--local_cache",local_cache,
    #    "--log_to_term",_worker_log_params["log_to_term"],
    #    "--log_to_file",_worker_log_params["log_to_file"],
    #    "--log_file",_worker_log_params["log_file"],
    #    "--log_level",_worker_log_params["log_level"]]
    #    })
    #
