...
def getImageFromOut(output,size,codes):
    vocabs=get_codes(codes)
    mask = np.array(output.cpu())
    mask2=mask.copy()

    for vocab in vocabs.keys():
        mask2[mask==vocabs[vocab][0]]=vocabs[vocab][1]
...
def omniData(path, learn, transformations, size=(480,640)):
    images = sorted(glob.glob(path+os.sep+'unlabeled_images' + os.sep + "*"))
    i = path.rfind(os.sep)
    if i != -1:
        newPath = path[:i] + "_tmp"
