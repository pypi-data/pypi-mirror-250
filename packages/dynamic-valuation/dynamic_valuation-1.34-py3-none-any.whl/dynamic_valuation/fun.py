...
class Solow_simple:
    ...
    def __init__(self,
        ...
        self.eq=root(lambda s: self.sdot(s,self.x(s)),bracket=[self.s_min+1e-1,self.s_max],method='brentq').root
    ...
    def x(self,s):
        return (1-self.mps)*s
    def sdot(self, s,x):
        return self.A *(s-x)**self.alpha-self.ϕ*s-self.x(s)
...
def T(cl=None,v=[],s_grid=[],policy=None):
  if not list(s_grid):
    s_grid=cl.s_grid
  if not list(v):
     v=s_grid
