...
class SheetHandler:
    ...
    def import_sheet(self, source_sheet: gspread.Worksheet, new_title: str = "") -> None:
        """
        Imports a sheet from another spreadsheet
        Arguments:
            source_sheet (gspread.Worksheet): The sheet to import
            new_title (str) [Optional]: The name of the imported sheet in the spreadsheet
        """
        if new_title == "":
            new_title = source_sheet.title
        
        counter = 1
        prob_title = new_title
        while any(sheet.title == prob_title for sheet in self.sheet_connection.worksheets()):
            prob_title = f"{new_title} ({counter})"
            counter += 1
        
        source_id = source_sheet.id
        body = {
            "requests": [
                {
                    "duplicateSheet": {
        ...
        self.sheet_connection.batch_update(body)
        dest_sheet = self.sheet_connection.worksheet(new_title)
        self.worksheets[new_title] = dest_sheet
