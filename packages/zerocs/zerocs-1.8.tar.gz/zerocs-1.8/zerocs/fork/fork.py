...
class _Fork:
    ...
    def fork_logger(config: dict) -> None:
        ...
        cmd = f'{_python} {_script} ' \
              f'--RABBITMQ_CONFIG {config.get("RABBITMQ_CONFIG")} ' \
              f'--LOGS_PATH {config.get("LOGS_PATH")} ' \
              f'--SERVICE_IP {Utils.get_ipaddr()}'
    ...
    def fork_service(service_list: list, config: dict) -> None:
        for service in service_list:
            ...
            cmd = f'{_python} {_script} ' \
                  f'--SERVICE_PATH {service.__name__} ' \
                  f'--RABBITMQ_CONFIG {config.get("RABBITMQ_CONFIG")} ' \
                  f'--LOGS_PATH {config.get("LOGS_PATH")}'
    ...
    def fork_work(work_list: list, config: dict) -> None:
        for work in work_list:
            ...
            cmd = f'{_python} {_script} ' \
                  f'--SERVICE_PATH {work.__name__} ' \
                  f'--RABBITMQ_CONFIG {config.get("RABBITMQ_CONFIG")} ' \
                  f'--LOGS_PATH {config.get("LOGS_PATH")} ' \
                  f'--MONGODB_CONFIG {config.get("MONGODB_CONFIG")}'
