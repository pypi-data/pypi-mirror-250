...
class DependencyBuilder:
    ...
    def build_hi_ts_data(self):
        ...
        if not os.path.exists(hi_ts_map_path):
            ...
            hi_ts_mapped_df.to_csv(os.path.join(self.data_dir, 'hi_ts_map.csv'), index = False)
    ...
    def build_clinvar_db(self):
        ...
        if not os.path.exists(clinvar_short_path):
            ...
            # Download the vcf
            cmd = f"""
            wget https://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/clinvar.vcf.gz -P {self.data_dir}
            """
            p = self.worker(cmd,  stdout=subprocess.PIPE, shell = True)

            # Create an index file
            # cmd = f"""
            # {config['tabix']} -f {clinvar_short_path}
            # """
            cmd = f"""
