...
from boulderopal._nodes.base import NodeFactory
from boulderopal._nodes.validation import ArrayLike
...
class PwcFactory(NodeFactory):
    ...
    durations: np.ndarray = field(converter=ArrayT.REAL("durations").ndim(1).ge(0))
    values: np.ndarray | Tensor = field(converter=ArrayLike("values"))
    time_dimension: int = field(default=0, converter=ScalarT.INT("time_dimension"))
    name: Optional[str] = None

    @time_dimension.validator
    def _check(self, _, time_dimension: int) -> None:
        ...
        Checker.VALUE(
            ...
            {"time_dimension": time_dimension, "value_shape": value_shape},
        )
        Checker.VALUE(
            value_shape[time_dimension] == len(self.durations),
            "The size of the `time_dimension` of `values` must be equal to the length"
            " of `durations`.",
            {"time_dimension": time_dimension, "len(durations)": len(self.durations)},
        )
    ...
    def create_node(self, graph):
        time_dimension = (
            ...
            else self.time_dimension + len(self.values.shape)
        )
        return Pwc(
            self.get_operation(graph),
            value_shape=self.values.shape[time_dimension + 1 :],
            durations=self.durations,
            batch_shape=self.values.shape[:time_dimension],
        )

    @classmethod
    def get_doc_category(cls) -> list[Category]:
        return [Category.BUILDING_PIECEWISE_CONSTANT_HAMILTONIANS]
