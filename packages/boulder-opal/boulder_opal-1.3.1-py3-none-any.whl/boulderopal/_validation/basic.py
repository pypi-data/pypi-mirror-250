...
def _is_integer(val: Any) -> bool:
    return isinstance(val, ScalarT.INT.value.types)


def _is_real(val: Any) -> bool:
    return _is_integer(val) or isinstance(val, ScalarT.REAL.value.types)
...
def _is_strict_complex(val: Any) -> bool:
    return isinstance(val, ScalarT.COMPLEX.value.types)
