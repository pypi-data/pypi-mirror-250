...
def check_optimization_node_in_graph(graph: "Graph") -> None:
    ...
    for operation in graph.operations.values():
        if PRIMARY_NODE_REGISTRY.is_optimization_node(operation):
            return
    raise ValueError(
...
def check_cost_node_for_optimization_graph(
    ...
    def _validate_node_from_operation(operation: Operation) -> None:
        if check_gradient_nodes:
            Checker.VALUE(
                PRIMARY_NODE_REGISTRY.supports_gradient(operation),
    ...
    for name in output_node_names:
        if PRIMARY_NODE_REGISTRY.is_optimization_node(graph.operations[name]):
            Checker.VALUE(
