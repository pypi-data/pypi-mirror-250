...
class StfGraph:  # pylint: disable=unused-argument
    ...
    def sinc_convolution_kernel(
        self, cutoff_frequency: Union[Annotated[float, ScalarT.REAL().gt(0)], Tensor]
    ) -> ConvolutionKernel:
        r"""
    ...
    def gaussian_convolution_kernel(
        self,
        std: Union[Annotated[float, ScalarT.REAL().gt(0)], Tensor],
        offset: Union[Annotated[float, ScalarT.REAL().ge(0)], Tensor] = 0,
    ) -> ConvolutionKernel:
        r"""
        Create a convolution kernel representing a normalized Gaussian.
