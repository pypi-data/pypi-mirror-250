...
import subprocess
import sys
# from seleniumbase import config as SBConfig
from seldom.recorder import page_utils, shared_utils


class SBConfig:
    rec_subprocess_p = None
    rec_subprocess_used = False
...
def do_recording(file_name, url, overwrite_enabled, use_chrome, window):
    poll = None
    if SBConfig.rec_subprocess_used:
        poll = SBConfig.rec_subprocess_p.poll()
    if not SBConfig.rec_subprocess_used or poll is not None:
        pass
    else:
    ...
    if not page_utils.is_valid_url(url):
        ...
        if not SBConfig.rec_subprocess_used or poll is not None:
            print("222", command)
            SBConfig.rec_subprocess_p = subprocess.Popen(command, shell=True)
            SBConfig.rec_subprocess_used = True
        else:
            print("333")
            show_already_recording_warning()
        send_window_to_front(window)
