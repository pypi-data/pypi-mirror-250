...
def form_list_without_pagination(req):
    ...
    forms = Form.objects.filter(account_id=account_id, status=FORM_STATUS[1][0]).order_by('-created_date')
...
def form_submission_list(req, form_id):
    ...
    if int(page) > page_obj.paginator.num_pages:
        ...
        for form_submission in page_obj.object_list:
            ...
            form_type = form_submission.form.type
            form_name = form_submission.form.name
...
def form_submission_details2(req, form_id, form_submission_id):
    ...
    # default - "Status ID & Status" at the last index for each submission
    single_row_entry.append({"key": "Status ID", "value": form_submission.status})
    single_row_entry.append({"key": "Status", "value": FORM_SUBMISSION_STATUS_DETAILS[form_submission.status]})

    return Response({"status": "success", "data": single_row_entry}, status=status.HTTP_200_OK)
