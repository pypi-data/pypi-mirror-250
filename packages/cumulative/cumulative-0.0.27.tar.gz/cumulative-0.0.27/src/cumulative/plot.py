...
class Plot:
    ...
    def heatmap(self, src=None, figure=None, show=True, ms=4, k=60, score="idx", style="."):
        src = options.default_if_null(src, "transforms.source")
        tmp = options.get("transforms.tmp")
        ...
        with options.option_context({"transforms": {"source": tmp, "destination": tmp}}):
            ...
            self.c.plot.scatter(figure=figure, show=show, ms=ms, alpha=tmp_score, score=tmp_score, style=style)
            self.c.drop()
