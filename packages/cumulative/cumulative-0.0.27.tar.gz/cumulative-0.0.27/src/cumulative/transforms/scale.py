...
class Scale(Transform):
    ...
    def transform_row(self, row, src, default_value=0):
        ...
        if s.shape[0] == 1:
            log.warning(f"idx={idx}: Length equal to 1, defaulting `y` to {default_value}")
            s[:] = default_value
        elif s.nunique() < 2:
            log.warning(f"idx={idx}: Less than 2 distinct `y` values, defaulting `y` to {default_value}")
            s[:] = default_value
        else:
            s -= s.min()
            s /= s.max()
        ...
        if s.shape[0] == 1:
            log.warning(f"idx={idx}: Length equal to 1, defaulting `x` to zero")
            s.index = [0]
        else:
            if s.index.nunique() < 2:
                log.warning(f"idx={idx}: Less than 2 distinct `x` values, defaulting to [0,1,...]/N")
