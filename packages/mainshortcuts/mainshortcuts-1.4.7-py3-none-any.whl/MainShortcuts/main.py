...
class _m_proc: # Операции с текущим или другими процессами
...
class _m_path: # Операции с путями к файлам/папкам
  ...
  def info(path,listdir=False,listlinks=False): # Информация о пути
    ...
    info["path"]=path
    info["split"]=_m_path.split(path)
    info["dir"]=_m_path.merge(info["split"][:-1])
    ...
    info["exists"]=_m_path.exists(path)
    if info["exists"]:
  ...
  def link(fr,to,force=False): # Сделать символическую ссылку
    if _m_path.exists(to) and force:
      _m_path.delete(to)
    _os.symlink(fr,to)
  ln=_m_path.link
...
class _m_file: # Операции с файлами
  ...
  def write(path,text="",encoding="utf-8",force=False): # Записать текстовый файл
    if _m_path.info(path)["type"]=="dir" and force:
      _os.remove(path)
  ...
  def rename(fr,to,force=False):
    ...
    if type=="file":
      ...
      raise Exception("Unknown type: "+type)
class _m_dir: # Операции с папками
  def create(path,force=False): # Создать папку
    if _m_path.exists(path):
      type=_m_path.info(path)["type"]
      if type=="dir":
        return True
      elif force:
        _m_path.delete(path)
  ...
  def delete(path):
    ...
    if type=="dir":
      ...
      raise Exception("Unknown type: "+type)
  def copy(fr,to,force=False):
    type=_m_path.info(fr)["dir"]
    if type=="dir":
      if _m_path.info(to)["type"]!="dir" and force:
        try:
          _m_path.delete(to)
        except:
          pass
    ...
    for i in a:
      ...
      if c:
        if files and info["type"]=="file":
          ...
          b.append(i)
    return b
class _m_str: # Операции с текстом
  def array2str(a):
    b=[]
    for i in a:
      b.append(str(i))
...
class _m_json: # Операции с JSON
  ...
  def write(path,data,encoding="utf-8",mode="c",indent=2,sort=True,force=False): # Данные в файл
    if _m_path.info(path)["type"]=="dir" and force:
      _os.remove(path)
    with open(path,"w",encoding=encoding) as f:
      f.write(_m_json.encode(data,mode=mode,indent=indent,sort=sort))
    return True
  def read(path,encoding="utf-8"): # Данные из файла
    with open(path,"r",encoding=encoding) as f:
      return _json.load(f)
  def sort(data): # Сортировать ключи словарей ({"b":1,"c":2,"a":3} -> {"a":3,"b":1,"c":2})
    return _m_json.decode(_m_json.encode(data,mode="c",sort=True))
  def rebuild(data,mode="c",indent=2,sort=True): # Перестроить JSON в тексте
    return _m_json.encode(_m_json.decode(data),mode=mode,indent=indent,sort=sort)
  def rewrite(path,encoding="utf-8",mode="c",indent=2,sort=True): # Перестроить JSON в файле
    return _m_json.write(path,_m_json.read(path,encoding=encoding),encoding=encoding,mode=mode,indent=indent,sort=sort)
# Команды для разных ОС
if _m_os.platform=="Windows": # Windows
  def clear():
    _os.system("cls")
  cls=clear
elif _m_os.platform=="Linux": # Linux
  def clear():
    _os.system("clear")
  cls=clear
elif _m_os.platform=="Darwin": # MacOS
  pass
else: # Неизвестный тип
  print("MainShortcuts WARN: Unknown OS \""+_m_os.platform+"\"")
