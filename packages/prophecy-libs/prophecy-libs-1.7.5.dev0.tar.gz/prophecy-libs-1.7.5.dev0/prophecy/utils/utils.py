...
def collectMetrics(
    ...
    conf_str = None
    if config is not None:
        conf_str = json.dumps(config.to_dict())
...
class SecretManager:
    ...
    def __init__(self, spark: SparkSession):
        self.jvm = spark.sparkContext._jvm
        self.spark = spark
        self.secret_manager = self.jvm.io.prophecy.libs.package.SecretManager
        self.secrets_provider_enum = self.jvm.io.prophecy.libs.secrets.package.SecretsProvider

    def secrets_provider(self, input: str):
        self.secrets_provider_enum.withNameInsensitive(input)

    def get(self, scope: str, key: str, provider: str):
        self.secret_manager.get(scope, key, self.secrets_provider(provider))
