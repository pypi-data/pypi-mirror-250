...
class Builder:
    ...
    def set_nodes(self, data: dict):
        ...
        for ld in data["linkDataArray"]:
            ...
            # ! Agregar parents solo si el nodo existe
            if ld["to"] in self.nodes:
                ...
                # ! Agregar childs solo si el nodo existe
                if ld["from"] in self.nodes:
                    if not self.nodes[ld["from"]]["childs"]:
                        self.nodes[ld["from"]]["childs"] = [child]
                    else:
                        self.nodes[ld["from"]]["childs"].append(child)
                else:
                    print(f'link {ld["key"]} hace referencia a nodo hijo inexistente ({ld["from"]})')
            else:
                print(f'link {ld["key"]} hace referencia a nodo padre inexistente ({ld["to"]})')
