...
def triggered_w_load(cpu_intensity=0, max_primes=1, memory_mb=1):
    ...
    print(
        f"Starting triggered workload.  CPU Intensity: {cpu_intensity} ({get_cpu_load(cpu_intensity)}), max_primes: {max_primes}, memory_mb: {memory_mb}")
    start = time.time()
    ...
    print(
        f"Completed triggered workload.  CPU Intensity: {cpu_intensity} ({get_cpu_load(cpu_intensity)}), max_primes: {max_primes}. Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    return f"Done. Completed in {end-start}s"


def triggered_no_load(sleep_time=1):
    print(f"Starting triggered no-load process.  Sleep time: {sleep_time}")
    start = time.time()
    time.sleep(sleep_time)
    end = time.time()
    print(
        f"Completed triggered no-load process.  Sleep time: {sleep_time}.  Completed in {end-start} seconds.")
    return f"Done. Completed in {end-start}s"
...
class LoadGen:
    ...
    def __init__(self, work_break, cpu_intensity, deplete_after, max_primes, randomize_work_break=False, memory_mb=1):
        """Constructor.
    ...
    def start_workloads(self):
        ...
        while True:
            ...
            print(f"Workload took {end-start} seconds, using {round(sys.getsizeof(memory_holder)/1024/1024,2)}MB Memory")
    ...
    def run(self):
        ...
        while True:
            ...
            print(
                f"Starting a work loop: work: {self.work_break} (Random: {self.randomize_work_break}), CPU Intensity: {self.cpu_intensity} ({get_cpu_load(self.cpu_intensity)}), max_primes: {self.max_primes}, memory_mb: {self.memory_mb}")
            self.start_workloads()
            print(
                f"Waiting for {self.work_break} seconds before starting next work loop"
            )
...
def constant_loadgen(**kwargs):
    ...
    work_break = int(os.environ.get(
        "LOADGEN_WORK_BREAK", kwargs.setdefault('work_break', work_break_default)))
    cpu_intensity = float(os.environ.get(
        "LOADGEN_CPU_INTENSITY", kwargs.setdefault('cpu_intensity', cpu_intensity_default)))
    deplete_after = int(os.environ.get(
        "LOADGEN_DEPLETE_AFTER", kwargs.setdefault('deplete_after', deplete_after_default)))
    max_primes = int(os.environ.get(
        "LOADGEN_MAX_PRIMES", kwargs.setdefault('max_primes', max_primes_default)))
    memory_mb = float(os.environ.get(
        "LOADGEN_MEMORY_MB", kwargs.setdefault('memory_mb', memory_mb_default)))
        
    loadgen_worker = LoadGen(work_break=work_break, randomize_work_break=randomize_work_break,
