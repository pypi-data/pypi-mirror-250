...
gpkg_file_1 = str(Path.joinpath(root_dir, "test_files/UK_Earthquakes.gpkg"))


class TestGeoreader_GPKG(TestCase):
    def test_data_type(self):
        geo_reader_1 = GeoReader(gpkg_file_1)
        self.assertEqual(geo_reader_1.data_type["name"], "gpkg")

    def test_list_layers(self):
        geo_reader_1 = GeoReader(gpkg_file_1)
        layers = geo_reader_1.list_layers()
        self.assertListEqual(
            layers, ["UK_Modern_Earthquakes", "UK_Historical_Earthquakes"]
        )

    def test_layer_count(self):
        geo_reader_1 = GeoReader(gpkg_file_1)
        self.assertEqual(len(geo_reader_1.list_layers()), 2)
    ...
    def test_get_layer(self):
        ...
        self.assertIsInstance(layer_2, LayerHandler)
        self.assertEqual(layer_2.get_layer_name(), "UK_Modern_Earthquakes")
