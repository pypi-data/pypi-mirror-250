...
class OGR_Layer:
    ...
    def write_to_postgis_db(
        ...
        if db_datastore is None:
            raise ValueError(
                "Something went wrong trying to connect to database. please connection string is correct"
            )
        ...
        try:
            geom_type = (
                ...
                else self.get_normalized_geom_type()
            )
            ...
            out_layer = db_datastore.CreateLayer(
                full_table_name, srs, geom_type, layer_creation_opts
            )
