...
def is_valid_type(_type: str) -> bool:
    if (
        is_valid_db_type(_type)
        or is_valid_zip_type(_type)
        or is_valid_service_type(_type)
        or is_valid_file_type(_type)
    ):
        return True
...
def get_handler_file(_type: str):
    if is_valid_file_type(_type):
        from georeader.handlers.helpers.handler_getters.file_handlers import (
            file_handlers,
        )
...
def get_handler_service(_type: str):
    if is_valid_service_type(_type):
        from georeader.handlers.helpers.handler_getters.service_handlers import (
            service_handlers,
        )
