...
markupAdmin = types.ReplyKeyboardMarkup(keyboard=[[types.KeyboardButton(text='⬅️ Prev'), types.KeyboardButton(text='↩️ Menu'),
            ...
            try:
                ...
                try:
                    ...
                    if MSG_TYPE == 'photo':
                        ...
                        res = await extra_bot.send_video(chat_id=OWNER_TID, video=MEDIA, width=width, height=height, disable_notification=True)
                        ...
                        if IS_LINK and FILE_NAME.lower().endswith('.mp4'):
                            MEDIA = types.FSInputFile(await download_file_my(MEDIA, BOT_TID, 'mp4'))

                        file_id, file_name_video = await send_audio_my(extra_bot, BOT_TID, OWNER_TID, MEDIA, FILE_NAME, BASE_D, EXTRA_D)
                        if file_name_video:
                        ...
                        file_id_note, file_name_video = await send_audio_my(extra_bot, BOT_TID, OWNER_TID, MEDIA, FILE_NAME, BASE_D, EXTRA_D)
                        if file_name_video:
                            if os.path.exists(FILE_NAME): os.remove(FILE_NAME)
                            print(f'del {FILE_NAME=}, остался {file_name_video=}')
                            FILE_NAME = file_name_video
                        ...
                        F_NAME = FILE_NAME
                        if IS_LINK:
                            F_NAME = await download_file_my(MEDIA, BOT_TID, FILE_NAME.split(".")[-1])
...
async def return_tgph_link(FILE_NAME, MSG_TYPE, IS_LINK):
    ...
    try:
        ...
        if IS_LINK:
            ...
            if res:
                result = res
            # print(f'new return_tgph_link: {result}')
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
    ...
    try:
        ...
        while cnt > 0:
            try:
                ...
                res = await extra_bot.send_audio(chat_id=OWNER_TID, audio=MEDIA, thumbnail=thumb, title=title, performer=performer, disable_notification=True)
                await extra_bot.delete_message(OWNER_TID, res.message_id)
                ...
                if not result or os.path.getsize(FILE_NAME) < 5242880:
                    ...
                    res = await extra_bot.send_audio(chat_id=OWNER_TID, audio=types.FSInputFile(file_name_video), thumbnail=thumb, title=title, performer=performer)
                    await extra_bot.delete_message(OWNER_TID, res.message_id)
