...
class TCI(object):
    ...
    def __init__(self, settings: dict):
        ...
        # private properties
        self._channelsList: list = settings.get('channels')
        ...
        self._run = False
        self._sendMsgThreadName = f"{time.time()}_{self._user}_sendMsgThread"
        self._getMsgThreadName = f"{time.time()}_{self._user}_getMsgThread"
        self._sendMessagesThread = threading.Thread(target=self._emptyMsgQ, name=self._sendMsgThreadName, daemon=True)
        self._getMessagesTread = threading.Thread(target=self._getMsgs, name=self._getMsgThreadName, daemon=True) 
    ...
    def updateSettings(self, settings: dict):
        self._channelsList = settings.get('channels')
    ...
    @property       
    def getThreads(self) -> tuple:
        return(self._sendMessagesThread, self._getMessagesTread)
    
    @property
    def sendMsgThreadName(self) -> str:
        return self._sendMsgThreadName
    
    @sendMsgThreadName.setter
    def sendMsgThreadName(self, name) -> None:
        self._sendMsgThreadName = name

    @property
    def getMsgThreadName(self)->str:
        return self._getMsgThreadName
    
    @getMsgThreadName.setter
    def getMsgThreadName(self, name):
        self._getMsgThreadName = name
