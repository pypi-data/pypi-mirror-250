...
class s3adapter:
    ...
    def exists(self, file_key):
        ...
        try:
            ...
            #response = self.s3_client.list_objects_v2(Bucket=self.bucket_name, Prefix=file_key)
            #for obj in response.get('Contents', []):
            #    if obj['Key'] == file_key:
            #        print(f"The object '{file_key}' exists in the bucket '{self.bucket_name}'.")
            #        object_exists=True
            #        break
            #if !object_exists=False               
            self.log.debug(f'Sucess to get S3Bucket, object [{file_key}], bucket [{self.bucket_name}]. Status [{False}]')
            return object_exists
        except ClientError as ce:
           # When object not exists boto return AccessDenied and we handle it as not existence of object           
           self.log.debug(f'Sucess to get S3Bucket, object [{file_key}], bucket [{self.bucket_name}]. Status [{object_exists}]')
           return False     
        except Exception as e:
            self.log.exception(e)
