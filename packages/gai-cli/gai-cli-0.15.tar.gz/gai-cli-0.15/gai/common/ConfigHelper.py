...
def get_api_config():
    rc = get_rc()
    app_dir = rc["app_dir"]
    api_config = os.path.join(os.path.expanduser(app_dir), "api_config.json")
...
def get_default_generator():
    ...
    return api_config["default_generator"]

def get_generator_url(generator):
    api_config_json = get_api_config()
    if generator not in api_config_json['generators']:
        raise Exception(f"Generator {generator} not found supported.")
    return api_config_json['generators'][generator]["url"]