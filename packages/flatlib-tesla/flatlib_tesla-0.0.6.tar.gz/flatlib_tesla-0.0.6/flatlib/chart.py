...
class Chart:
    ...
    def __init__(self, date, pos, **kwargs):
        ...
        self.pos = pos
        self.hsys = hsys
        self.objects = ephem.getObjectList(IDs, date, pos,hsys,self.FLAGS)
        self.houses, self.angles = ephem.getHouses(date, pos, hsys,self.FLAGS)
    ...
    def getFixedStar(self, ID):
        ...
        return ephem.getFixedStar(ID, self.date,self.FLAGS)
    
    def getFixedStars(self):
        """ Returns a list with all fixed stars. """
        IDs = const.LIST_FIXED_STARS
        return ephem.getFixedStarList(IDs, self.date,self.FLAGS)
