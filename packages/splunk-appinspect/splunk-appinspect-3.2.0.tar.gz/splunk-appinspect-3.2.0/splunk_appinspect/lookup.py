...
class LookupHelper:
    ...
    There are a few restrictions to the kinds of CSV files that can be used for CSV lookups:
     ...
     - The table represented by the CSV file should have at least two columns. One of those columns should
        represent a field with a set of values that includes those belonging to a field in your events. The column
        does not have to have the same name as the event field. Any column can have multiple instances of the same
        value, as this represents a multivalued field.
     - The CSV file cannot contain non-utf-8 characters. Plain ascii text is supported, as is any character
        set that is also valid utf-8.
    ...
    @staticmethod
    def is_valid_csv(full_filepath: Path) -> tuple[bool, str]:
        """
        Valid whether file is valid csv according to
        http://docs.splunk.com/Documentation/SplunkCloud/6.6.0/Knowledge/ConfigureCSVlookups
        and additional constraints - should have at least one row and number of
        columns must match for all rows.

        Args:
            full_filepath: Local system filepath to lookup to be examined.
