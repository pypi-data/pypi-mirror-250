...
def test_sighting_all_required_properties():
    ...
    sighting = misp_stix2.v21.Sighting(
        type='sighting',
        id=SIGHTING_ID,
        created=now,
        modified=now,
...
def test_sighting_bad_where_sighted_refs():
    ...
    assert excinfo.value.cls == misp_stix2.v21.Sighting
    assert excinfo.value.prop_name == "where_sighted_refs"


def test_sighting_type_must_be_sightings():
    with pytest.raises(misp_stix2.exceptions.InvalidValueError) as excinfo:
        misp_stix2.v21.Sighting(type='xxx', **SIGHTING_KWARGS)
...
def test_invalid_kwarg_to_sighting():
    with pytest.raises(misp_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_stix2.v21.Sighting(my_custom_property="foo", **SIGHTING_KWARGS)
...
def test_create_sighting_from_objects_rather_than_ids(malware):  # noqa: F811
    rel = misp_stix2.v21.Sighting(sighting_of_ref=malware)
...
def test_parse_sighting(data):
    sighting = misp_stix2.parse(data, version="2.1")
