...
import misp_stix2
import misp_stix2.base
import misp_stix2.registration
import misp_stix2.registry
import misp_stix2.v21
...
IDENTITY_CUSTOM_PROP = misp_stix2.v21.Identity(
    name="John Smith",
...
def _register_extension(ext, props):
    ...
    misp_stix2.v21.CustomExtension(
        ext_def_id,
        props,
    )(ext)
    ...
    try:
        ...
        del misp_stix2.registry.STIX2_OBJ_MAPS["2.1"]["extensions"][ext_def_id]


def test_identity_custom_property():
    identity = misp_stix2.v21.Identity(
        id=IDENTITY_ID,
    ...
    with pytest.raises(misp_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_stix2.v21.Identity(
            id=IDENTITY_ID,
            created="2015-12-21T19:59:11Z",
            modified="2015-12-21T19:59:11Z",
    ...
    with pytest.raises(misp_stix2.exceptions.InvalidValueError) as excinfo:
        misp_stix2.v21.Identity(
            id=IDENTITY_ID,
            created="2015-12-21T19:59:11Z",
            modified="2015-12-21T19:59:11Z",
...
def test_identity_custom_property_invalid():
    ...
    assert excinfo.value.cls == misp_stix2.v21.Identity
...
def test_parse_identity_custom_property(data):
    with pytest.raises(misp_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_stix2.parse(data, version="2.1")
    assert issubclass(excinfo.value.cls, misp_stix2.v21.Identity)
    assert excinfo.value.properties == ['foo']
    assert "Unexpected properties for" in str(excinfo.value)

    identity = misp_stix2.parse(data, version="2.1", allow_custom=True)
    assert identity.foo == "bar"


def test_custom_property_object_in_bundled_object():
    bundle = misp_stix2.v21.Bundle(IDENTITY_CUSTOM_PROP, allow_custom=True)
...
def test_custom_properties_object_in_bundled_object():
    ...
    bundle = misp_stix2.v21.Bundle(obj, allow_custom=True)
...
def test_custom_property_dict_in_bundled_object():
    ...
    with pytest.raises(InvalidValueError):
        misp_stix2.v21.Bundle(custom_identity)

    bundle = misp_stix2.v21.Bundle(custom_identity, allow_custom=True)
    assert bundle.objects[0].x_foo == "bar"
    assert '"x_foo": "bar"' in str(bundle)
...
def test_custom_property_in_observed_data():
    artifact = misp_stix2.v21.File(
        allow_custom=True,
        name='test',
        x_foo='bar',
    )
    observed_data = misp_stix2.v21.ObservedData(
        allow_custom=True,
...
def test_identity_custom_property_edit_markings():
    marking_obj = misp_stix2.v21.MarkingDefinition(
        id=MARKING_DEFINITION_ID,
        definition_type="statement",
        definition=misp_stix2.v21.StatementMarking(statement="Copyright 2016, Example Corp"),
    )
    marking_obj2 = misp_stix2.v21.MarkingDefinition(
        id=MARKING_DEFINITION_ID,
        definition_type="statement",
        definition=misp_stix2.v21.StatementMarking(statement="Another one"),
    )
...
def test_invalid_custom_property_in_marking():
    with pytest.raises(ValueError) as excinfo:
        @misp_stix2.v21.CustomMarking(
            'x-new-obj', [
                ('9property1', misp_stix2.properties.StringProperty(required=True)),
            ],
        )
        class NewObj():
            pass
...
def test_custom_marking_no_init_1():
    @misp_stix2.v21.CustomMarking(
        'x-new-obj', [
            ('property1', misp_stix2.properties.StringProperty(required=True)),
        ],
    )
    class NewObj():
        pass
...
@misp_stix2.v21.CustomObject(
    'x-new-type', [
        ('property1', misp_stix2.properties.StringProperty(required=True)),
        ('property2', misp_stix2.properties.IntegerProperty()),
    ],
)
...
def test_custom_object_type():
    ...
    with pytest.raises(misp_stix2.exceptions.MissingPropertiesError) as excinfo:
...
def test_custom_object_ref_property_containing_identifier():
    @misp_stix2.v21.CustomObject(
        'x-new-obj-with-ref', [
            ('property_ref', misp_stix2.properties.ReferenceProperty(invalid_types=[])),
        ],
    )
    class NewObs():
        pass
...
def test_custom_object_refs_property_containing_identifiers():
    @misp_stix2.v21.CustomObject(
        'x-new-obj-with-refs', [
            ('property_refs', misp_stix2.properties.ListProperty(misp_stix2.properties.ReferenceProperty(invalid_types=[]))),
        ],
    )
    class NewObs():
        pass
    ...
    with pytest.raises(ValueError, match=r"not a subclass of 'ReferenceProperty"):
        @misp_stix2.v21.CustomObject(
            'x-new-obj-with-objref', [
                ('property_ref', misp_stix2.properties.ObjectReferenceProperty()),
            ],
        )
        class NewObs():
            pass
    ...
    with pytest.raises(ValueError, match=r"not a 'ListProperty' containing a subclass of 'ReferenceProperty"):
        @misp_stix2.v21.CustomObject(
            'x-new-obj-with-objrefs', [
                ('property_refs', misp_stix2.properties.ListProperty(misp_stix2.properties.ObjectReferenceProperty())),
            ],
        )
        class NewObs():
            pass
...
def test_custom_object_invalid_ref_property():
    with pytest.raises(ValueError) as excinfo:
        @misp_stix2.v21.CustomObject(
            'x-new-obj', [
                ('property_ref', misp_stix2.properties.StringProperty()),
            ],
        )
        class NewObs():
            pass
...
def test_custom_object_invalid_refs_property():
    with pytest.raises(ValueError) as excinfo:
        @misp_stix2.v21.CustomObject(
            'x-new-obj', [
                ('property_refs', misp_stix2.properties.StringProperty()),
            ],
        )
        class NewObs():
            pass
...
def test_custom_object_invalid_refs_list_property():
    with pytest.raises(ValueError) as excinfo:
        @misp_stix2.v21.CustomObject(
            'x-new-obj', [
                ('property_refs', misp_stix2.properties.ListProperty(misp_stix2.properties.StringProperty)),
            ],
        )
        class NewObs():
            pass
...
def test_custom_subobject_dict():
    ...
    obj = misp_stix2.parse(obj_dict, allow_custom=True)
    assert obj["objects"][0]["x_foo"] == 123
    assert obj.has_custom

    with pytest.raises(InvalidValueError):
        misp_stix2.parse(obj_dict, allow_custom=False)


def test_custom_subobject_obj():
    ident = misp_stix2.v21.Identity(
...
def test_parse_custom_object_type():
    ...
    nt = misp_stix2.parse(nt_string, allow_custom=True)
    assert nt["property1"] == 'something'
...
def test_parse_unregistered_custom_object_type():
    ...
    with pytest.raises(misp_stix2.exceptions.ParseError) as excinfo:
        misp_stix2.parse(nt_string, version="2.1")
...
def test_parse_unregistered_custom_object_type_w_allow_custom():
    ...
    custom_obj = misp_stix2.parse(nt_string, version="2.1", allow_custom=True)
    assert custom_obj["type"] == "x-foobar-observable"

# Custom SCOs


@misp_stix2.v21.CustomObservable(
    'x-new-observable', [
        ('property1', misp_stix2.properties.StringProperty(required=True)),
        ('property2', misp_stix2.properties.IntegerProperty()),
        ('x_property3', misp_stix2.properties.BooleanProperty()),
    ],
)
class NewObservable():
    def __init__(self, property2=None, **kwargs):
...
def test_custom_observable_object_no_init_1():
    @misp_stix2.v21.CustomObservable(
        'x-new-observable-2', [
            ('property1', misp_stix2.properties.StringProperty()),
        ],
    )
    class NewObs():
        pass
...
def test_invalid_custom_property_in_custom_observable_object():
    with pytest.raises(ValueError) as excinfo:
        @misp_stix2.v21.CustomObservable(
            'x-new-sco', [
                ('5property1', misp_stix2.properties.StringProperty()),
            ],
        )
        class NewObs(object):
            pass  # pragma: no cover
...
def test_custom_no_properties_raises_exception():
    with pytest.raises(TypeError):
        ...
        @misp_stix2.v21.CustomObject('x-new-object-type')
...
def test_custom_wrong_properties_arg_raises_exception():
    with pytest.raises(ValueError):
        ...
        @misp_stix2.v21.CustomObservable('x-new-object-type', (("prop", misp_stix2.properties.BooleanProperty())))
...
def test_parse_custom_observable_object():
    ...
    nt = misp_stix2.parse(nt_string, [], version='2.1')
    assert isinstance(nt, misp_stix2.base._STIXBase)
    assert nt.property1 == 'something'
...
def test_parse_unregistered_custom_observable_object():
    ...
    with pytest.raises(misp_stix2.exceptions.ParseError) as excinfo:
        misp_stix2.parse(nt_string, version='2.1')
    assert "Can't parse unknown object type" in str(excinfo.value)
    parsed_custom = misp_stix2.parse(nt_string, allow_custom=True, version='2.1')
    assert parsed_custom['property1'] == 'something'
    with pytest.raises(AttributeError) as excinfo:
        assert parsed_custom.property1 == 'something'
    assert not isinstance(parsed_custom, misp_stix2.base._STIXBase)
...
def test_parse_unregistered_custom_observable_object_with_no_type():
    ...
    with pytest.raises(misp_stix2.exceptions.ParseError) as excinfo:
        misp_stix2.parse(nt_string, allow_custom=True, version='2.1')
...
def test_parse_observed_data_with_custom_observable():
    ...
    parsed = misp_stix2.parse(input_str, version="2.1", allow_custom=True)
...
def test_observed_data_with_custom_observable_object():
    no = NewObservable(property1='something')
    ob_data = misp_stix2.v21.ObservedData(
...
def test_custom_extension_with_list_and_dict_properties_observable_type(data):
    @misp_stix2.v21.CustomExtension(
        'x-some-extension-ext', [
                ('keys', misp_stix2.properties.ListProperty(misp_stix2.properties.DictionaryProperty, required=True)),
        ],
    )
...
def test_custom_extension_no_properties():
    with pytest.raises(ValueError):
        @misp_stix2.v21.CustomExtension('x-new2-ext', None)
        class BarExtension():
            pass
...
def test_custom_extension_empty_properties():
    with pytest.raises(ValueError):
        @misp_stix2.v21.CustomExtension('x-new2-ext', [])
        class BarExtension():
            pass
...
def test_invalid_custom_property_in_extension():
    with pytest.raises(ValueError) as excinfo:
        @misp_stix2.v21.CustomExtension(
            'x-new3-ext', [
                    ('6property1', misp_stix2.properties.StringProperty(required=True)),
            ],
        )
        class NewExt():
            pass
...
def test_parse_observable_with_custom_extension():
    ...
    parsed = misp_stix2.parse(input_str, version='2.1')
...
def test_parse_observable_with_custom_extension_property():
    ...
    with pytest.raises(InvalidValueError):
        misp_stix2.parse(input_str, version="2.1", allow_custom=False)
...
def test_custom_and_spec_extension_mix():
    ...
    assert isinstance(
        file_obs.extensions["raster-image-ext"], misp_stix2.v21.RasterImageExt,
    )
    assert isinstance(
        file_obs.extensions["ntfs-ext"], misp_stix2.v21.NTFSExt,
    )
...
def test_parse_observable_with_unregistered_custom_extension(data):
    ...
    parsed_ob = misp_stix2.parse(data, allow_custom=True, version='2.1')
    assert parsed_ob['extensions']['x-foobar-ext']['property1'] == 'foo'
    assert not isinstance(parsed_ob['extensions']['x-foobar-ext'], misp_stix2.base._STIXBase)
...
def test_unregistered_new_style_extension():
    ...
    f = misp_stix2.parse(f_dict, allow_custom=False)
    ...
    f = misp_stix2.parse(f_dict, allow_custom=True)
...
def test_register_custom_object():
    ...
    with pytest.raises(ValueError) as excinfo:
        misp_stix2.registration._register_object(CustomObject2, version="2.1")
    assert '@CustomObject decorator' in str(excinfo)


def test_extension_property_location():
    assert 'extensions' in misp_stix2.v21.OBJ_MAP_OBSERVABLE['x-new-observable']._properties
    assert 'extensions' not in misp_stix2.v21.EXT_MAP['x-new-ext']._properties
...
def test_custom_object_nested_dictionary(data):
    @misp_stix2.v21.CustomObject(
        'x-example', [
            ('dictionary', misp_stix2.properties.DictionaryProperty()),
        ],
    )
...
def test_register_custom_object_with_version():
    ...
    cust_obj_1 = misp_stix2.parsing.dict_to_stix2(custom_obj_1, version='2.1')

    assert cust_obj_1.type in misp_stix2.registry.STIX2_OBJ_MAPS['2.1']['objects']
...
def test_register_observable():
    ...
    assert custom_obs.type in misp_stix2.registry.STIX2_OBJ_MAPS['2.1']['observables']
...
def test_register_observable_custom_extension():
    ...
    assert 'domain-name' in misp_stix2.registry.STIX2_OBJ_MAPS['2.1']['observables']
    assert example._type in misp_stix2.registry.STIX2_OBJ_MAPS['2.1']['extensions']
...
def test_unregistered_top_level_extension_passes_with_allow_custom_false():
    indicator = misp_stix2.v21.Indicator(
...
def test_registered_top_level_extension_passes_with_allow_custom_false():
    @misp_stix2.v21.CustomExtension(
        'extension-definition--d83fce45-ef58-4c6c-a3f4-1fbc32e98c6e', [
            ('rank', misp_stix2.properties.IntegerProperty(required=True)),
            ('toxicity', misp_stix2.properties.IntegerProperty(required=True)),
        ],
    )
    class ExtensionFoo1:
        extension_type = 'toplevel-property-extension'
...
def test_registered_new_extension_sdo_allow_custom_false():
    @misp_stix2.v21.CustomObject(
        'my-favorite-sdo', [
            ('name', misp_stix2.properties.StringProperty(required=True)),
            ('some_property_name1', misp_stix2.properties.StringProperty(required=True)),
            ('some_property_name2', misp_stix2.properties.StringProperty()),
    ...
    sdo_object = misp_stix2.parse(my_favorite_sdo)
    assert isinstance(sdo_object, MyFavSDO)
    assert isinstance(
        sdo_object.extensions['extension-definition--d83fce45-ef58-4c6c-a3f4-1fbc32e9999'],
        misp_stix2.v21.EXT_MAP['extension-definition--d83fce45-ef58-4c6c-a3f4-1fbc32e9999'],
    )
...
def test_registered_new_extension_sro_allow_custom_false():
    ...
    sro_object = misp_stix2.parse(my_favorite_sro)
    assert isinstance(sro_object, MyFavSRO)
    assert isinstance(
        sro_object.extensions['extension-definition--e96690a5-dc13-4f27-99dd-0f2188ad74ce'],
        misp_stix2.v21.EXT_MAP['extension-definition--e96690a5-dc13-4f27-99dd-0f2188ad74ce'],
    )
...
def test_registered_new_extension_sco_allow_custom_false():
    @misp_stix2.v21.CustomObservable(
        'my-favorite-sco', [
            ('name', misp_stix2.properties.StringProperty(required=True)),
            ('some_network_protocol_field', misp_stix2.properties.StringProperty(required=True)),
    ...
    sco_object = misp_stix2.parse(my_favorite_sco)
    assert isinstance(sco_object, MyFavSCO)
    assert isinstance(
        sco_object.extensions['extension-definition--a932fcc6-e032-177c-126f-cb970a5a1fff'],
        misp_stix2.v21.EXT_MAP['extension-definition--a932fcc6-e032-177c-126f-cb970a5a1fff'],
    )
...
def test_registered_new_extension_marking_allow_custom_false():
    ...
    props = {
        'some_marking_field': misp_stix2.properties.StringProperty(required=True),
    }
    ...
    with _register_extension(MyFavMarking, props) as ext_def_id:
        ...
        marking_object = misp_stix2.parse(my_favorite_marking)
        assert isinstance(marking_object, misp_stix2.v21.MarkingDefinition)
        assert isinstance(
            marking_object.extensions[ext_def_id],
            misp_stix2.v21.EXT_MAP[ext_def_id],
        )
...
def test_custom_marking_toplevel_properties():
    ...
    props = {
        "foo": misp_stix2.properties.StringProperty(required=True),
    }
    ...
    with _register_extension(CustomMarking, props) as ext_def_id:
        ...
        marking = misp_stix2.parse(marking_dict)
...
def test_nested_ext_prop_meta():
    ...
    props = {
        "intprop": misp_stix2.properties.IntegerProperty(required=True),
        "strprop": misp_stix2.properties.StringProperty(
...
def test_toplevel_extension_includes_extensions():
    ...
    ext_props = {
        "extensions": misp_stix2.properties.ExtensionsProperty(spec_version="2.1"),
    }
    ...
    with _register_extension(ExtensionsExtension, ext_props) as ext_id:
        ...
        misp_stix2.parse(obj_dict)


def test_invalid_extension_prop_name():

    with pytest.raises(ValueError):
        @misp_stix2.v21.common.CustomExtension(
            "extension-definition--0530fdbd-0fa3-42ab-90cf-660e0abad370",
            [
                ("7foo", misp_stix2.properties.StringProperty()),
            ],
        )
        class CustomExt:
            extension_type = "property-extension"
