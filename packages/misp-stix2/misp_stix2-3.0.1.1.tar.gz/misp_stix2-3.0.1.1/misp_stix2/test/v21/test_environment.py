...
import misp_stix2
import misp_stix2.environment
import misp_stix2.equivalence.graph
import misp_stix2.equivalence.object
import misp_stix2.exceptions
...
@pytest.fixture
def ds():
    cam = misp_stix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    idy = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    ind = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    mal = misp_stix2.v21.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    rel1 = misp_stix2.v21.Relationship(ind, 'indicates', mal, id=RELATIONSHIP_IDS[0])
    rel2 = misp_stix2.v21.Relationship(mal, 'targets', idy, id=RELATIONSHIP_IDS[1])
    rel3 = misp_stix2.v21.Relationship(cam, 'uses', mal, id=RELATIONSHIP_IDS[2])
    reprt = misp_stix2.v21.Report(
    ...
    yield misp_stix2.MemoryStore(stix_objs)
...
def ds2_objects():
    cam = misp_stix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    idy = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    ind = misp_stix2.v21.Indicator(id=INDICATOR_ID, created_by_ref=idy.id, **INDICATOR_KWARGS)
    indv2 = ind.new_version(
        ...
        object_marking_refs=[misp_stix2.v21.TLP_WHITE],
    )
    mal = misp_stix2.v21.Malware(id=MALWARE_ID, created_by_ref=idy.id, **MALWARE_KWARGS)
...
@pytest.fixture
def ds2(ds2_objects):
    yield misp_stix2.MemoryStore(ds2_objects)


@pytest.fixture
def fs():
    yield misp_stix2.FileSystemSource(FS_PATH)


def test_object_factory_created_by_ref_str():
    factory = misp_stix2.ObjectFactory(created_by_ref=IDENTITY_ID)
    ind = factory.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID


def test_object_factory_created_by_ref_obj():
    id_obj = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    factory = misp_stix2.ObjectFactory(created_by_ref=id_obj)
    ind = factory.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID
...
def test_object_factory_override_default():
    ...
    ind = factory.create(misp_stix2.v21.Indicator, created_by_ref=new_id, **INDICATOR_KWARGS)
    assert ind.created_by_ref == new_id


def test_object_factory_created():
    factory = misp_stix2.ObjectFactory(created=FAKE_TIME)
    ind = factory.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
...
def test_object_factory_external_reference():
    ext_ref = misp_stix2.v21.ExternalReference(
        source_name="ACME Threat Intel",
        description="Threat report",
    )
    factory = misp_stix2.ObjectFactory(external_references=ext_ref)
    ind = factory.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
    assert ind.external_references[0].source_name == "ACME Threat Intel"
    assert ind.external_references[0].description == "Threat report"

    ind2 = factory.create(misp_stix2.v21.Indicator, external_references=None, **INDICATOR_KWARGS)
    assert 'external_references' not in ind2


def test_object_factory_obj_markings():
    stmt_marking = misp_stix2.v21.StatementMarking("Copyright 2016, Example Corp")
    mark_def = misp_stix2.v21.MarkingDefinition(
        definition_type="statement",
        definition=stmt_marking,
    )
    factory = misp_stix2.ObjectFactory(object_marking_refs=[mark_def, misp_stix2.v21.TLP_AMBER])
    ind = factory.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
    assert mark_def.id in ind.object_marking_refs
    assert misp_stix2.v21.TLP_AMBER.id in ind.object_marking_refs

    factory = misp_stix2.ObjectFactory(object_marking_refs=misp_stix2.v21.TLP_RED)
    ind = factory.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
    assert misp_stix2.v21.TLP_RED.id in ind.object_marking_refs
...
def test_object_factory_list_append():
    ...
    ext_ref2 = misp_stix2.v21.ExternalReference(
        source_name="Yet Another Threat Report",
        description="Threat report from YATR",
    )
    ext_ref3 = misp_stix2.v21.ExternalReference(
        source_name="Threat Report #3",
        description="One more threat report",
    )
    factory = misp_stix2.ObjectFactory(external_references=ext_ref)
    ind = factory.create(misp_stix2.v21.Indicator, external_references=ext_ref2, **INDICATOR_KWARGS)
    assert ind.external_references[1].source_name == "Yet Another Threat Report"

    ind = factory.create(misp_stix2.v21.Indicator, external_references=[ext_ref2, ext_ref3], **INDICATOR_KWARGS)
...
def test_object_factory_list_replace():
    ...
    factory = misp_stix2.ObjectFactory(external_references=ext_ref, list_append=False)
    ind = factory.create(misp_stix2.v21.Indicator, external_references=ext_ref2, **INDICATOR_KWARGS)
...
def test_environment_functions():
    env = misp_stix2.Environment(
        misp_stix2.ObjectFactory(created_by_ref=IDENTITY_ID),
        misp_stix2.MemoryStore(),
    )

    # Create a STIX object
    ind = env.create(misp_stix2.v21.Indicator, id=INDICATOR_ID, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID
    ...
    query = [misp_stix2.Filter('type', '=', 'vulnerability')]
    resp = env.query(query)
    assert len(resp) == 0

    # See different results after adding filters to the environment
    env.add_filters([
        misp_stix2.Filter('type', '=', 'indicator'),
        misp_stix2.Filter('created_by_ref', '=', IDENTITY_ID),
    ])
    env.add_filter(misp_stix2.Filter('labels', '=', 'benign'))  # should be 'malicious-activity'
    resp = env.get(INDICATOR_ID)
...
def test_environment_source_and_sink():
    ind = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    env = misp_stix2.Environment(source=misp_stix2.MemorySource([ind]), sink=misp_stix2.MemorySink([ind]))
    assert env.get(INDICATOR_ID).indicator_types[0] == 'malicious-activity'


def test_environment_datastore_and_sink():
    with pytest.raises(ValueError) as excinfo:
        misp_stix2.Environment(
            factory=misp_stix2.ObjectFactory(),
            store=misp_stix2.MemoryStore(), sink=misp_stix2.MemorySink,
        )
    assert 'Data store already provided' in str(excinfo.value)


def test_environment_no_datastore():
    env = misp_stix2.Environment(factory=misp_stix2.ObjectFactory())

    with pytest.raises(AttributeError) as excinfo:
        env.add(misp_stix2.v21.Indicator(**INDICATOR_KWARGS))
...
def test_environment_datastore_and_no_object_factory():
    ...
    env = misp_stix2.Environment(store=misp_stix2.MemoryStore())
    ind = env.create(misp_stix2.v21.Indicator, id=INDICATOR_ID, **INDICATOR_KWARGS)
...
def test_creator_of():
    identity = misp_stix2.v21.Identity(**IDENTITY_KWARGS)
    factory = misp_stix2.ObjectFactory(created_by_ref=identity.id)
    env = misp_stix2.Environment(store=misp_stix2.MemoryStore(), factory=factory)
    env.add(identity)

    ind = env.create(misp_stix2.v21.Indicator, **INDICATOR_KWARGS)
...
def test_creator_of_no_datasource():
    identity = misp_stix2.v21.Identity(**IDENTITY_KWARGS)
    factory = misp_stix2.ObjectFactory(created_by_ref=identity.id)
    env = misp_stix2.Environment(factory=factory)
...
def test_relationships(ds):
    env = misp_stix2.Environment(store=ds)
...
def test_object_similarity_on_same_attack_pattern1():
    ap1 = misp_stix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_PATTERN_KWARGS)
    ap2 = misp_stix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_PATTERN_KWARGS)
    env = misp_stix2.Environment().object_similarity(ap1, ap2)
...
def test_object_similarity_on_same_attack_pattern2():
    ...
    ap1 = misp_stix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_KWARGS)
    ap2 = misp_stix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_KWARGS)
    env = misp_stix2.Environment().object_similarity(ap1, ap2)
    assert round(env) == 100


def test_object_similarity_on_same_campaign1():
    camp1 = misp_stix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    camp2 = misp_stix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    env = misp_stix2.Environment().object_similarity(camp1, camp2)
    assert round(env) == 100
...
def test_object_similarity_on_same_campaign2():
    ...
    camp1 = misp_stix2.v21.Campaign(id=CAMPAIGN_ID, **CAMP_KWARGS)
    camp2 = misp_stix2.v21.Campaign(id=CAMPAIGN_ID, **CAMP_KWARGS)
    env = misp_stix2.Environment().object_similarity(camp1, camp2)
    assert round(env) == 100


def test_object_similarity_on_same_identity1():
    iden1 = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    iden2 = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    env = misp_stix2.Environment().object_similarity(iden1, iden2)
    assert round(env) == 100
...
def test_object_similarity_on_same_identity2():
    ...
    iden1 = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDEN_KWARGS)
    iden2 = misp_stix2.v21.Identity(id=IDENTITY_ID, **IDEN_KWARGS)
    env = misp_stix2.Environment().object_similarity(iden1, iden2)
    assert round(env) == 100


def test_object_similarity_on_same_indicator():
    ind1 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    ind2 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    env = misp_stix2.Environment().object_similarity(ind1, ind2)
    assert round(env) == 100


def test_object_similarity_on_same_location1():
    location_kwargs = dict(latitude=45, longitude=179)
    loc1 = misp_stix2.v21.Location(id=LOCATION_ID, **location_kwargs)
    loc2 = misp_stix2.v21.Location(id=LOCATION_ID, **location_kwargs)
    env = misp_stix2.Environment().object_similarity(loc1, loc2)
    assert round(env) == 100
...
def test_object_similarity_location_with_no_latlong():
    ...
    loc1 = misp_stix2.v21.Location(id=LOCATION_ID, **LOCATION_KWARGS)
    loc2 = misp_stix2.v21.Location(id=LOCATION_ID, **loc_kwargs)
    env = misp_stix2.Environment().object_similarity(loc1, loc2)
    assert round(env) != 100


def test_object_similarity_on_same_malware():
    malw1 = misp_stix2.v21.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    malw2 = misp_stix2.v21.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    env = misp_stix2.Environment().object_similarity(malw1, malw2)
    assert round(env) == 100


def test_object_similarity_on_same_threat_actor1():
    ta1 = misp_stix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_ACTOR_KWARGS)
    ta2 = misp_stix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_ACTOR_KWARGS)
    env = misp_stix2.Environment().object_similarity(ta1, ta2)
    assert round(env) == 100
...
def test_object_similarity_on_same_threat_actor2():
    ...
    ta1 = misp_stix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_KWARGS)
    ta2 = misp_stix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_KWARGS)
    env = misp_stix2.Environment().object_similarity(ta1, ta2)
    assert round(env) == 100


def test_object_similarity_on_same_tool():
    tool1 = misp_stix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = misp_stix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    env = misp_stix2.Environment().object_similarity(tool1, tool2)
    assert round(env) == 100


def test_object_similarity_on_same_vulnerability1():
    vul1 = misp_stix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
    vul2 = misp_stix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
    prop_scores = {}
    env = misp_stix2.Environment().object_similarity(vul1, vul2, prop_scores)
    assert round(env) == 100
...
def test_object_equivalence_on_same_vulnerability1():
    ...
    vul2 = misp_stix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
    prop_scores = {}
    env = misp_stix2.Environment().object_equivalence(vul1, vul2, prop_scores)
    assert env is True
    assert round(prop_scores["matching_score"]) == 30
    assert round(prop_scores["sum_weights"]) == 30
...
def test_object_similarity_on_same_vulnerability2():
    ...
    vul1 = misp_stix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULN_KWARGS1)
    vul2 = misp_stix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULN_KWARGS2)
    prop_scores = {}
    env = misp_stix2.Environment().object_similarity(vul1, vul2, prop_scores)
...
def test_object_similarity_on_unknown_object():
    ...
    def _x_foobar_checks(obj1, obj2, **weights):
        matching_score = 0.0
        sum_weights = 0.0
        if misp_stix2.equivalence.object.check_property_present("external_references", obj1, obj2):
            w = weights["external_references"]
            sum_weights += w
            matching_score += w * misp_stix2.equivalence.object.partial_external_reference_based(
                obj1["external_references"],
                obj2["external_references"],
            )
        if misp_stix2.equivalence.object.check_property_present("name", obj1, obj2):
            w = weights["name"]
            sum_weights += w
            matching_score += w * misp_stix2.equivalence.object.partial_string_based(obj1["name"], obj2["name"])
    ...
    cust1 = misp_stix2.parse(CUSTOM_KWARGS1, allow_custom=True)
    cust2 = misp_stix2.parse(CUSTOM_KWARGS2, allow_custom=True)
    env = misp_stix2.Environment().object_similarity(cust1, cust2, **weights)
    assert round(env) == 0
...
def test_object_similarity_different_type_raises():
    with pytest.raises(ValueError) as excinfo:
        vul1 = misp_stix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
        ind1 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
        misp_stix2.Environment().object_similarity(vul1, ind1)
...
def test_object_similarity_different_spec_version_raises():
    with pytest.raises(ValueError) as excinfo:
        ...
        ind1 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
        ind2 = misp_stix2.v20.Indicator(id=INDICATOR_ID, **V20_KWARGS)
        misp_stix2.Environment().object_similarity(ind1, ind2)
...
def test_object_similarity_zero_match():
    ...
    weights = {
        "indicator": {
            "indicator_types": (15, misp_stix2.equivalence.object.partial_list_based),
            "pattern": (80, misp_stix2.equivalence.object.custom_pattern_based),
            "valid_from": (5, misp_stix2.equivalence.object.partial_timestamp_based),
    ...
    ind1 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    ind2 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **IND_KWARGS)
    env = misp_stix2.Environment().object_similarity(ind1, ind2, **weights)
    assert round(env) == 0
    env = misp_stix2.Environment().object_similarity(ind2, ind1, **weights)
    assert round(env) == 0
...
def test_object_similarity_different_spec_version():
    ...
    ind1 = misp_stix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    ind2 = misp_stix2.v20.Indicator(id=INDICATOR_ID, **IND_KWARGS)
    env = misp_stix2.Environment().object_similarity(ind1, ind2, ignore_spec_version=True, **weights)
    assert round(env) == 0

    env = misp_stix2.Environment().object_similarity(ind2, ind1, ignore_spec_version=True, **weights)
    assert round(env) == 0
...
def test_object_similarity_external_references(refs1, refs2, ret_val):
    value = misp_stix2.equivalence.object.partial_external_reference_based(refs1, refs2)
...
def test_object_similarity_timestamp():
    ...
    assert misp_stix2.equivalence.object.partial_timestamp_based(t1, t2, 1) == 0.5


def test_object_similarity_exact_match():
    t1 = "2018-10-17T00:14:20.652Z"
    t2 = "2018-10-17T12:14:20.652Z"
    assert misp_stix2.equivalence.object.exact_match(t1, t2) == 0.0


def test_no_datastore_fallsback_list_based_check_for_refs_check():
    r1 = misp_stix2.v21.Report(id=REPORT_ID, **REPORT_KWARGS)
    r2 = misp_stix2.v21.Report(id=REPORT_ID, **REPORT_KWARGS)
    prop_scores = {}
    assert misp_stix2.Environment().object_similarity(r1, r2, prop_scores) == 100.0
...
def test_object_similarity_method_provided():
    ...
    weights = {
        "tool": {
            "tool_types": (20, misp_stix2.equivalence.object.partial_list_based),
            "name": (80, misp_stix2.equivalence.object.partial_string_based),
            "method": custom_semantic_equivalence_method,
        },
    }

    tool1 = misp_stix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = misp_stix2.v21.Tool(id=TOOL_ID, **TOOL2_KWARGS)
    env = misp_stix2.Environment().object_similarity(tool1, tool2, **weights)
    assert round(env) == 96
...
def test_object_similarity_prop_scores_method_provided():
    ...
    tool1 = misp_stix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = misp_stix2.v21.Tool(id=TOOL_ID, **TOOL2_KWARGS)
    env = misp_stix2.Environment().object_similarity(tool1, tool2, prop_scores, **weights)
    assert round(env) == 96
...
def test_versioned_checks(ds, ds2):
    ...
    weights = misp_stix2.equivalence.graph.WEIGHTS.copy()
    weights.update({
        "_internal": {
    ...
    score = misp_stix2.equivalence.object._versioned_checks(INDICATOR_ID, INDICATOR_ID, ds, ds2, **weights)
...
def test_semantic_check_with_versioning(ds, ds2):
    ...
    ds.add(ind)
    score = misp_stix2.equivalence.object.reference_check(ind.id, INDICATOR_ID, ds, ds2, **weights)
...
def test_list_semantic_check(ds, ds2):
    ...
    score = misp_stix2.equivalence.object.list_reference_check(
        object_refs1,
        object_refs2,
        ds,
        ds2,
...
def test_graph_similarity_raises_value_error(ds):
    with pytest.raises(ValueError):
        prop_scores1 = {}
        misp_stix2.Environment().graph_similarity(ds, ds2, prop_scores1, max_depth=-1)
...
def test_depth_limiting():
    ...
    mem_store1 = misp_stix2.MemorySource(g1)
    mem_store2 = misp_stix2.MemorySource(g2)

    custom_weights = {
        "foo": {
            "some1_ref": (33, misp_stix2.equivalence.object.reference_check),
            "some2_ref": (33, misp_stix2.equivalence.object.reference_check),
            "name": (34, misp_stix2.equivalence.object.partial_string_based),
        },
    }
    prop_scores1 = {}
    env1 = misp_stix2.equivalence.graph.graph_similarity(
    ...
    # Switching parameters
    prop_scores2 = {}
    env2 = misp_stix2.equivalence.graph.graph_similarity(
...
def test_graph_similarity_with_duplicate_graph(ds):
    prop_scores = {}
    env = misp_stix2.Environment().graph_similarity(ds, ds, prop_scores)
    assert round(env) == 100
...
def test_graph_similarity_with_versioning_check_on(ds2, ds):
    prop_scores1 = {}
    env1 = misp_stix2.Environment().graph_similarity(ds, ds2, prop_scores1, versioning_checks=True)
    ...
    # Switching parameters
    prop_scores2 = {}
    env2 = misp_stix2.Environment().graph_similarity(ds2, ds, prop_scores2, versioning_checks=True)
...
def test_graph_similarity_with_versioning_check_off(ds2, ds):
    prop_scores1 = {}
    env1 = misp_stix2.Environment().graph_similarity(ds, ds2, prop_scores1)
    assert round(env1) == 88
    assert round(prop_scores1["matching_score"]) == 789
    assert round(prop_scores1["len_pairs"]) == 9
    ...
    # Switching parameters
    prop_scores2 = {}
    env2 = misp_stix2.Environment().graph_similarity(ds2, ds, prop_scores2)
    assert round(env2) == 88
    assert round(prop_scores2["matching_score"]) == 789
    assert round(prop_scores2["len_pairs"]) == 9
...
def test_graph_equivalence_with_duplicate_graph(ds):
    prop_scores = {}
    env = misp_stix2.Environment().graph_equivalence(ds, ds, prop_scores)
    assert env is True
    assert round(prop_scores["matching_score"]) == 800
    assert round(prop_scores["len_pairs"]) == 8
...
def test_graph_equivalence_with_versioning_check_on(ds2, ds):
    prop_scores1 = {}
    env1 = misp_stix2.Environment().graph_equivalence(ds, ds2, prop_scores1, versioning_checks=True)

    # Switching parameters
    prop_scores2 = {}
    env2 = misp_stix2.Environment().graph_equivalence(ds2, ds, prop_scores2, versioning_checks=True)
...
def test_graph_equivalence_with_versioning_check_off(ds2, ds):
    prop_scores1 = {}
    env1 = misp_stix2.Environment().graph_equivalence(ds, ds2, prop_scores1)

    # Switching parameters
    prop_scores2 = {}
    env2 = misp_stix2.Environment().graph_equivalence(ds2, ds, prop_scores2)
