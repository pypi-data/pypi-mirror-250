...
def test_lockheed_martin_cyber_kill_chain():
    recon = misp_stix2.v21.KillChainPhase(
        kill_chain_name="lockheed-martin-cyber-kill-chain",
        phase_name="reconnaissance",
    )
...
def test_kill_chain_example():
    preattack = misp_stix2.v21.KillChainPhase(
        kill_chain_name="foo",
        phase_name="pre-attack",
    )
...
def test_kill_chain_required_properties():
    ...
    with pytest.raises(misp_stix2.exceptions.MissingPropertiesError) as excinfo:
        misp_stix2.v21.KillChainPhase()

    assert excinfo.value.cls == misp_stix2.v21.KillChainPhase
...
def test_kill_chain_required_property_chain_name():
    ...
    with pytest.raises(misp_stix2.exceptions.MissingPropertiesError) as excinfo:
        misp_stix2.v21.KillChainPhase(phase_name="weaponization")
...
def test_kill_chain_required_property_phase_name():
    ...
    with pytest.raises(misp_stix2.exceptions.MissingPropertiesError) as excinfo:
        misp_stix2.v21.KillChainPhase(kill_chain_name="lockheed-martin-cyber-kill-chain")
