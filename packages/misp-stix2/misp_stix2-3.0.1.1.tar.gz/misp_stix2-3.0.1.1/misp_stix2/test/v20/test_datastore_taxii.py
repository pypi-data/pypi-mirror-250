...
from misp_stix2.datastore.filters import Filter
from misp_stix2.utils import get_timestamp
...
class MockTAXIICollectionEndpoint(Collection):
    ...
    def get_objects(self, accept=MEDIA_TYPE_STIX_V20, start=0, per_request=0, **filter_kwargs):
        ...
        if objs:
            ...
            resp.encoding = "utf-8"
            resp._content = bytes(misp_stix2.v20.Bundle(objects=objs).serialize(ensure_ascii=False), resp.encoding)
            return resp
        else:
            resp = Response()
    ...
    def get_object(self, id, **filter_kwargs):
        ...
        if objects:
            ...
            filtered_objects = []
        if filtered_objects:
            return misp_stix2.v20.Bundle(objects=filtered_objects)
        else:
            resp = Response()
            resp.status_code = 404
            resp.raise_for_status()
...
def test_ds_taxii(collection):
    ds = misp_stix2.TAXIICollectionSource(collection)
    assert ds.collection is not None


def test_add_stix2_object(collection):
    tc_sink = misp_stix2.TAXIICollectionSink(collection)

    # create new STIX threat-actor
    ta = misp_stix2.v20.ThreatActor(
        name="Teddy Bear",
        labels=["nation-state"],
...
def test_add_stix2_with_custom_object(collection):
    tc_sink = misp_stix2.TAXIICollectionStore(collection, allow_custom=True)
...
def test_get_object_found(collection):
    tc_source = misp_stix2.TAXIICollectionSource(collection)
    result = tc_source.query([
        misp_stix2.Filter("id", "=", "indicator--00000000-0000-4000-8000-000000000001"),
    ])
    assert result
...
def test_add_stix2_bundle_object(collection):
    ...
    tc_sink.add(misp_stix2.v20.Bundle(objects=[ta]))
...
def test_get_stix2_object(collection):
    tc_sink = misp_stix2.TAXIICollectionSource(collection)
...
def test_get_all_versions(collection):
    ds = misp_stix2.TAXIICollectionStore(collection)
...
def test_can_read_error(collection_no_rw_access):
    ...
    with pytest.raises(DataSourceError) as excinfo:
        misp_stix2.TAXIICollectionSource(collection_no_rw_access)
...
def test_can_write_error(collection_no_rw_access):
    ...
    with pytest.raises(DataSourceError) as excinfo:
        misp_stix2.TAXIICollectionSink(collection_no_rw_access)
...
def test_get_404():
    ...
    ds = misp_stix2.TAXIICollectionSource(TAXIICollection404())
