...
import misp_stix2
from misp_stix2.datastore import DataSourceError
...
def test_filesystem_sink_marking(fs_sink):
    marking = misp_stix2.v20.MarkingDefinition(
        id="marking-definition--34098fce-860f-48ae-8e50-ebd3cc5e41da",
        created="2017-01-20T00:00:00.000Z",
        definition_type="tlp",
        definition=misp_stix2.v20.TLPMarking(tlp="green"),
    )
...
def test_filesystem_store_query(fs_store):
    # query()
    tools = fs_store.query([misp_stix2.Filter("labels", "in", "tool")])
    assert len(tools) == 2
...
def test_filesystem_store_query_single_filter(fs_store):
    query = misp_stix2.Filter("labels", "in", "tool")
...
def test_filesystem_store_query_multiple_filters(fs_store):
    fs_store.source.filters.add(misp_stix2.Filter("labels", "in", "tool"))
...
def test_filesystem_object_with_custom_property_in_bundle(fs_store):
    ...
    bundle = misp_stix2.v20.Bundle(camp, allow_custom=True)
    fs_store.add(bundle)
