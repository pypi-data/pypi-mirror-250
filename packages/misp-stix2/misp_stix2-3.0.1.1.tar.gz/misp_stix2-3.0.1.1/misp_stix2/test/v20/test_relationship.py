...
def test_relationship_all_required_properties():
    ...
    rel = misp_stix2.v20.Relationship(
        type='relationship',
        id=RELATIONSHIP_ID,
        created=now,
        modified=now,
...
def test_relationship_type_must_be_relationship():
    with pytest.raises(misp_stix2.exceptions.InvalidValueError) as excinfo:
        misp_stix2.v20.Relationship(type='xxx', **RELATIONSHIP_KWARGS)

    assert excinfo.value.cls == misp_stix2.v20.Relationship
...
def test_relationship_id_must_start_with_relationship():
    with pytest.raises(misp_stix2.exceptions.InvalidValueError) as excinfo:
        misp_stix2.v20.Relationship(id='my-prefix--', **RELATIONSHIP_KWARGS)
...
def test_relationship_missing_some_required_properties():
    with pytest.raises(misp_stix2.exceptions.MissingPropertiesError) as excinfo:
        misp_stix2.v20.Relationship(relationship_type='indicates')
...
def test_invalid_kwarg_to_relationship():
    with pytest.raises(misp_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_stix2.v20.Relationship(my_custom_property="foo", **RELATIONSHIP_KWARGS)
...
def test_create_relationship_with_positional_args(indicator, malware):
    rel = misp_stix2.v20.Relationship(indicator, 'indicates', malware)
...
def test_parse_relationship(data):
    rel = misp_stix2.parse(data, version="2.0")
