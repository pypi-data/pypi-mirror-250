...
IDENTITY_CUSTOM_PROP = misp_stix2.v20.Identity(
    name="John Smith",
...
def test_identity_custom_property():
    ...
    identity = misp_stix2.v20.Identity(
        id=IDENTITY_ID,
        created="2015-12-21T19:59:11Z",
        modified="2015-12-21T19:59:11Z",
        name="John Smith",
...
def test_identity_custom_property_invalid():
    ...
    assert excinfo.value.cls == misp_stix2.v20.Identity
...
def test_parse_identity_custom_property(data):
    with pytest.raises(misp_stix2.exceptions.ExtraPropertiesError) as excinfo:
        misp_stix2.parse(data, version="2.0")
    assert issubclass(excinfo.value.cls, misp_stix2.v20.Identity)
    assert excinfo.value.properties == ['foo']
    assert "Unexpected properties for" in str(excinfo.value)

    identity = misp_stix2.parse(data, version="2.0", allow_custom=True)
    assert identity.foo == "bar"


def test_custom_property_object_in_bundled_object():
    bundle = misp_stix2.v20.Bundle(IDENTITY_CUSTOM_PROP, allow_custom=True)
...
def test_custom_properties_object_in_bundled_object():
    ...
    bundle = misp_stix2.v20.Bundle(obj, allow_custom=True)
...
def test_custom_property_dict_in_bundled_object():
    ...
    with pytest.raises(InvalidValueError):
        misp_stix2.v20.Bundle(custom_identity)

    bundle = misp_stix2.v20.Bundle(custom_identity, allow_custom=True)
    assert bundle.objects[0].x_foo == "bar"
    assert '"x_foo": "bar"' in str(bundle)
...
def test_custom_property_in_observed_data():
    artifact = misp_stix2.v20.File(
        allow_custom=True,
        name='test',
        x_foo='bar',
    )
    observed_data = misp_stix2.v20.ObservedData(
        allow_custom=True,
...
def test_identity_custom_property_edit_markings():
    marking_obj = misp_stix2.v20.MarkingDefinition(
        id=MARKING_DEFINITION_ID,
        definition_type="statement",
        definition=misp_stix2.v20.StatementMarking(statement="Copyright 2016, Example Corp"),
    )
    marking_obj2 = misp_stix2.v20.MarkingDefinition(
        id=MARKING_DEFINITION_ID,
        definition_type="statement",
        definition=misp_stix2.v20.StatementMarking(statement="Another one"),
    )
...
def test_parse_custom_object_type():
    ...
    nt = misp_stix2.parse(nt_string, version="2.0", allow_custom=True)
    assert nt["property1"] == 'something'
...
def test_parse_unregistered_custom_object_type():
    ...
    with pytest.raises(misp_stix2.exceptions.ParseError) as excinfo:
        misp_stix2.parse(nt_string, version="2.0")
...
def test_parse_unregistered_custom_object_type_w_allow_custom():
    ...
    custom_obj = misp_stix2.parse(nt_string, version="2.0", allow_custom=True)
    assert custom_obj["type"] == "x-foobar-observable"


@misp_stix2.v20.CustomObservable(
    'x-new-observable', [
        ('property1', misp_stix2.properties.StringProperty(required=True)),
        ('property2', misp_stix2.properties.IntegerProperty()),
...
def test_custom_observable_object_1():
    no = NewObservable(
        property1='something',
        extensions={
            'archive-ext': misp_stix2.v20.observables.ArchiveExt(
...
def test_custom_observable_object_invalid_valid_refs():
    @misp_stix2.v20.CustomObservable(
        'x-new-obs', [
            ('property1', misp_stix2.properties.StringProperty(required=True)),
            ('property_ref', misp_stix2.properties.ObjectReferenceProperty(valid_types='email-addr')),
        ],
    )
    class NewObs():
        pass
...
def test_custom_no_properties_raises_exception():
    with pytest.raises(TypeError):
        ...
        @misp_stix2.v20.CustomObject('x-new-object-type')
...
def test_custom_wrong_properties_arg_raises_exception():
    with pytest.raises(ValueError):
        ...
        @misp_stix2.v20.CustomObservable('x-new-object-type', (("prop", misp_stix2.properties.BooleanProperty())))
...
def test_parse_custom_observable_object():
    ...
    nt = misp_stix2.parse_observable(nt_string, [], version='2.0')
...
def test_parse_unregistered_custom_observable_object():
    ...
    with pytest.raises(misp_stix2.exceptions.ParseError) as excinfo:
        misp_stix2.parse_observable(nt_string, version='2.0')
    assert "Can't parse unknown observable type" in str(excinfo.value)

    parsed_custom = misp_stix2.parse_observable(nt_string, allow_custom=True, version='2.0')
...
def test_parse_unregistered_custom_observable_object_with_no_type():
    ...
    with pytest.raises(misp_stix2.exceptions.ParseError) as excinfo:
        misp_stix2.parse_observable(nt_string, allow_custom=True, version='2.0')
...
def test_parse_observed_data_with_custom_observable():
    ...
    parsed = misp_stix2.parse(input_str, version="2.0", allow_custom=True)
...
def test_observed_data_with_custom_observable_object():
    ...
    ob_data = misp_stix2.v20.ObservedData(
...
@misp_stix2.v20.CustomExtension(
    'x-new-ext', [
        ('property1', misp_stix2.properties.StringProperty(required=True)),
        ('property2', misp_stix2.properties.IntegerProperty()),
    ],
...
def test_custom_extension_no_properties():
    with pytest.raises(ValueError):
        @misp_stix2.v20.CustomExtension('x-new-ext2', None)
        class BarExtension():
            pass
...
def test_custom_extension_empty_properties():
    with pytest.raises(ValueError):
        @misp_stix2.v20.CustomExtension('x-new-ext2', [])
        class BarExtension():
            pass
...
def test_parse_observable_with_custom_extension():
    ...
    parsed = misp_stix2.parse_observable(input_str, version='2.0')
...
def test_parse_observable_with_custom_extension_property():
    ...
    with pytest.raises(InvalidValueError):
        misp_stix2.parse(input_str, version="2.0", allow_custom=False)
...
def test_custom_and_spec_extension_mix():
    ...
    assert isinstance(
        file_obs.extensions["raster-image-ext"], misp_stix2.v20.RasterImageExt,
    )
    assert isinstance(
        file_obs.extensions["ntfs-ext"], misp_stix2.v20.NTFSExt,
    )
...
def test_parse_observable_with_unregistered_custom_extension(data):
    with pytest.raises(InvalidValueError) as excinfo:
        misp_stix2.parse_observable(data, version='2.0')
    assert "Can't parse unknown extension type" in str(excinfo.value)

    parsed_ob = misp_stix2.parse_observable(data, allow_custom=True, version='2.0')
...
def test_register_custom_object():
    ...
    with pytest.raises(ValueError):
        misp_stix2.registration._register_object(CustomObject2, version="2.0")


def test_extension_property_location():
    assert 'extensions' in misp_stix2.v20.OBJ_MAP_OBSERVABLE['x-new-observable']._properties
    assert 'extensions' not in misp_stix2.v20.EXT_MAP['x-new-ext']._properties
...
def test_register_custom_object_with_version():
    ...
    cust_obj_1 = misp_stix2.parsing.dict_to_stix2(custom_obj_1, version='2.0')

    assert cust_obj_1.type in misp_stix2.registry.STIX2_OBJ_MAPS['2.0']['objects']
...
def test_register_observable_with_version():
    ...
    assert custom_obs.type in misp_stix2.registry.STIX2_OBJ_MAPS['2.0']['observables']
...
def test_register_marking_with_version():
    ...
    assert no._type in misp_stix2.registry.STIX2_OBJ_MAPS['2.0']['markings']


def test_register_observable_extension_with_version():
    @misp_stix2.v20.CustomExtension(
        'some-extension-2', [
                ('keys', misp_stix2.properties.StringProperty(required=True)),
        ],
    )
    ...
    assert example._type in misp_stix2.registry.STIX2_OBJ_MAPS['2.0']['extensions']
