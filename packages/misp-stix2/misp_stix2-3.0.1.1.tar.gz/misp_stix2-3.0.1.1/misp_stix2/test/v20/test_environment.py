...
@pytest.fixture
def ds():
    cam = misp_stix2.v20.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    idy = misp_stix2.v20.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    ind = misp_stix2.v20.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    mal = misp_stix2.v20.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    rel1 = misp_stix2.v20.Relationship(ind, 'indicates', mal, id=RELATIONSHIP_IDS[0])
    rel2 = misp_stix2.v20.Relationship(mal, 'targets', idy, id=RELATIONSHIP_IDS[1])
    rel3 = misp_stix2.v20.Relationship(cam, 'uses', mal, id=RELATIONSHIP_IDS[2])
    reprt = misp_stix2.v20.Report(
        name="Malware Report",
...
def ds2():
    cam = misp_stix2.v20.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    idy = misp_stix2.v20.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    ind = misp_stix2.v20.Indicator(id=INDICATOR_ID, created_by_ref=idy.id, **INDICATOR_KWARGS)
    ...
    mal = misp_stix2.v20.Malware(id=MALWARE_ID, created_by_ref=idy.id, **MALWARE_KWARGS)
...
def test_object_factory_created_by_ref_str():
    ...
    ind = factory.create(misp_stix2.v20.Indicator, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID


def test_object_factory_created_by_ref_obj():
    id_obj = misp_stix2.v20.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
...
def test_object_factory_override_default():
    ...
    ind = factory.create(misp_stix2.v20.Indicator, created_by_ref=new_id, **INDICATOR_KWARGS)
...
def test_object_factory_external_reference():
    ext_ref = misp_stix2.v20.ExternalReference(
    ...
    ind2 = factory.create(misp_stix2.v20.Indicator, external_references=None, **INDICATOR_KWARGS)
    assert 'external_references' not in ind2


def test_object_factory_obj_markings():
    stmt_marking = misp_stix2.v20.StatementMarking("Copyright 2016, Example Corp")
    mark_def = misp_stix2.v20.MarkingDefinition(
        definition_type="statement",
        definition=stmt_marking,
    )
    factory = misp_stix2.ObjectFactory(object_marking_refs=[mark_def, misp_stix2.v20.TLP_AMBER])
    ind = factory.create(misp_stix2.v20.Indicator, **INDICATOR_KWARGS)
    assert mark_def.id in ind.object_marking_refs
    assert misp_stix2.v20.TLP_AMBER.id in ind.object_marking_refs

    factory = misp_stix2.ObjectFactory(object_marking_refs=misp_stix2.v20.TLP_RED)
    ind = factory.create(misp_stix2.v20.Indicator, **INDICATOR_KWARGS)
    assert misp_stix2.v20.TLP_RED.id in ind.object_marking_refs
...
def test_object_factory_list_append():
    ...
    ext_ref2 = misp_stix2.v20.ExternalReference(
        source_name="Yet Another Threat Report",
        description="Threat report from YATR",
    )
    ext_ref3 = misp_stix2.v20.ExternalReference(
        source_name="Threat Report #3",
        description="One more threat report",
    )
    factory = misp_stix2.ObjectFactory(external_references=ext_ref)
    ind = factory.create(misp_stix2.v20.Indicator, external_references=ext_ref2, **INDICATOR_KWARGS)
    assert ind.external_references[1].source_name == "Yet Another Threat Report"

    ind = factory.create(misp_stix2.v20.Indicator, external_references=[ext_ref2, ext_ref3], **INDICATOR_KWARGS)
...
def test_environment_functions():
    ...
    ind = env.create(misp_stix2.v20.Indicator, id=INDICATOR_ID, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID
...
def test_environment_no_datastore():
    ...
    with pytest.raises(AttributeError) as excinfo:
        env.add(misp_stix2.v20.Indicator(**INDICATOR_KWARGS))
...
def test_creator_of():
    identity = misp_stix2.v20.Identity(**IDENTITY_KWARGS)
    ...
    ind = env.create(misp_stix2.v20.Indicator, **INDICATOR_KWARGS)
...
def test_semantic_check_with_versioning(ds, ds2):
    ...
    ind = misp_stix2.v20.Indicator(
        **dict(
            ...
            object_marking_refs=[misp_stix2.v20.TLP_WHITE],
        )
    )
    ds.add(ind)
