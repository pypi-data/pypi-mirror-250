...
import mispstix2
from mispstix2.datastore import DataSourceError
...
def test_filesystem_sink_marking(fs_sink):
    marking = mispstix2.v20.MarkingDefinition(
        id="marking-definition--34098fce-860f-48ae-8e50-ebd3cc5e41da",
        created="2017-01-20T00:00:00.000Z",
        definition_type="tlp",
        definition=mispstix2.v20.TLPMarking(tlp="green"),
    )
...
def test_filesystem_store_query(fs_store):
    # query()
    tools = fs_store.query([mispstix2.Filter("labels", "in", "tool")])
    assert len(tools) == 2
...
def test_filesystem_store_query_single_filter(fs_store):
    query = mispstix2.Filter("labels", "in", "tool")
...
def test_filesystem_store_query_multiple_filters(fs_store):
    fs_store.source.filters.add(mispstix2.Filter("labels", "in", "tool"))
...
def test_filesystem_object_with_custom_property_in_bundle(fs_store):
    ...
    bundle = mispstix2.v20.Bundle(camp, allow_custom=True)
    fs_store.add(bundle)
