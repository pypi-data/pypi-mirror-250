...
import mispstix2.versioning

from .constants import CAMPAIGN_MORE_KWARGS


def test_making_new_version():
    campaign_v1 = mispstix2.v20.Campaign(**CAMPAIGN_MORE_KWARGS)
...
def test_making_new_version_with_embedded_object():
    campaign_v1 = mispstix2.v20.Campaign(
        external_references=[{
            "source_name": "capec",
...
def test_versioning_error_invalid_property():
    ...
    with pytest.raises(mispstix2.exceptions.UnmodifiablePropertyError) as excinfo:
...
def test_versioning_error_bad_modified_value():
    ...
    assert excinfo.value.cls == mispstix2.v20.Campaign
    ...
    msg = msg.format(
        mispstix2.v20.Campaign.__name__, "modified",
...
def test_versioning_error_usetting_required_property():
    ...
    msg = msg.format(mispstix2.v20.Campaign.__name__, "name")
    assert str(excinfo.value) == msg
...
def test_versioning_error_new_version_of_revoked():
    ...
    with pytest.raises(mispstix2.exceptions.RevokeError) as excinfo:
...
def test_making_new_version_dict():
    ...
    campaign_v2 = mispstix2.versioning.new_version(CAMPAIGN_MORE_KWARGS, name="fred")
    ...
    assert campaign_v2['name'] == "fred"
    assert campaign_v1['description'] == campaign_v2['description']
    assert mispstix2.utils.parse_into_datetime(campaign_v1['modified'], precision='millisecond') < campaign_v2['modified']


def test_versioning_error_dict_bad_modified_value():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.versioning.new_version(CAMPAIGN_MORE_KWARGS, modified="2015-04-06T20:03:00.000Z")
...
def test_versioning_dict_unregistered_no_modified():
    ...
    new_d = mispstix2.versioning.new_version(d)
    assert "modified" in new_d
    assert new_d["modified"] > mispstix2.utils.parse_into_datetime(d["created"])

    new_d = mispstix2.versioning.new_version(d, modified="1996-11-20T01:19:29.134Z")
...
def test_versioning_dict_unregistered_unversionable():
    ...
    with pytest.raises(mispstix2.exceptions.ObjectNotVersionableError):
        mispstix2.versioning.new_version(d)

    with pytest.raises(mispstix2.exceptions.ObjectNotVersionableError):
        # should fail even if we provide a "created" kwarg.
        mispstix2.versioning.new_version(d, created="1985-06-29T06:09:51.157Z")


def test_versioning_custom_object():
    @mispstix2.v20.CustomObject(
        "x-versionable-all-optional-20", [
            ("created", mispstix2.properties.TimestampProperty()),
            ("modified", mispstix2.properties.TimestampProperty()),
            ("revoked", mispstix2.properties.BooleanProperty()),
        ],
    )
    class CustomSDO:
        pass
    ...
    new_obj = mispstix2.versioning.new_version(obj)
    ...
    with pytest.raises(mispstix2.exceptions.ObjectNotVersionableError):
        ...
        mispstix2.versioning.new_version(obj)
...
def test_versioning_error_dict_no_modified_value():
    ...
    campaign_v2 = mispstix2.versioning.new_version(campaign_v1, modified="2017-04-06T20:03:00.000Z")
...
def test_making_new_version_invalid_cls():
    ...
    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError) as excinfo:
        mispstix2.versioning.new_version(campaign_v1, name="fred")
...
def test_revoke_dict():
    campaign_v1 = CAMPAIGN_MORE_KWARGS
    campaign_v2 = mispstix2.versioning.revoke(campaign_v1)
...
def test_revoke_unversionable():
    sco = mispstix2.v20.File(name="data.txt")
    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        sco.revoke()
...
def test_versioning_error_revoke_of_revoked_dict():
    ...
    with pytest.raises(mispstix2.exceptions.RevokeError) as excinfo:
        mispstix2.versioning.revoke(campaign_v2)
...
def test_remove_custom_stix_property():
    ...
    mal_nc = mispstix2.versioning.remove_custom_stix(mal)

    assert "x_custom" not in mal_nc
    assert (
        mispstix2.utils.parse_into_datetime(mal["modified"], precision="millisecond") <
        mispstix2.utils.parse_into_datetime(mal_nc["modified"], precision="millisecond")
    )
...
def test_remove_custom_stix_object():
    @mispstix2.v20.CustomObject(
        "x-animal", [
            ("species", mispstix2.properties.StringProperty(required=True)),
            ("animal_class", mispstix2.properties.StringProperty()),
        ],
    )
    class Animal(object):
        pass
    ...
    nc = mispstix2.versioning.remove_custom_stix(animal)
...
def test_remove_custom_stix_no_custom():
    campaign_v1 = mispstix2.v20.Campaign(**CAMPAIGN_MORE_KWARGS)
    campaign_v2 = mispstix2.versioning.remove_custom_stix(campaign_v1)
...
def test_version_unversionable_dict():
    ...
    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        mispstix2.versioning.new_version(f)
...
def test_version_sco_with_modified():
    ...
    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        mispstix2.versioning.new_version(file_sco, name="newname.txt")

    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        mispstix2.versioning.revoke(file_sco)

    file_sco_obj = mispstix2.v20.File(
        name="data.txt",
        created="1973-11-23T02:31:37Z",
        modified="1991-05-13T19:24:57Z",
    )
    ...
    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        mispstix2.versioning.new_version(file_sco_obj, name="newname.txt")

    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        mispstix2.versioning.revoke(file_sco_obj)
...
def test_version_sco_with_custom():
    ...
    new_file_sco_obj = mispstix2.versioning.new_version(
        file_sco_obj, name="newname.txt",
    )

    assert new_file_sco_obj.name == "newname.txt"

    revoked_obj = mispstix2.versioning.revoke(new_file_sco_obj)
    ...
    new_d = mispstix2.versioning.new_version(d, size=1234)
    assert new_d["size"] == 1234

    revoked_d = mispstix2.versioning.revoke(new_d)
    assert revoked_d["revoked"]


def test_version_marking():
    m = mispstix2.v20.MarkingDefinition(
    ...
    with pytest.raises(mispstix2.exceptions.TypeNotVersionableError):
        mispstix2.versioning.new_version(m)
...
def test_version_disable_custom():
    ...
    # resulting object.
    m2 = mispstix2.versioning.new_version(m, x_custom=None, allow_custom=False)
    ...
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError):
        mispstix2.versioning.new_version(m, description=None, allow_custom=False)
...
def test_version_enable_custom():
    ...
    m2 = mispstix2.versioning.new_version(m, x_custom=123, allow_custom=True)
    assert "x_custom" in m2

    # Add a custom property without enabling it, make sure we get an error
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError):
        mispstix2.versioning.new_version(m, x_custom=123, allow_custom=False)
...
def test_version_propagate_custom():
    ...
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError):
        mispstix2.versioning.new_version(m, x_custom=123)

    m2 = mispstix2.versioning.new_version(m, description="Steals your identity!")
    assert "description" in m2
    assert m2.description == "Steals your identity!"

    m_custom = mispstix2.v20.Malware(
        name="foo", labels=["spyware"], x_custom=123, allow_custom=True,
    )

    # Remember custom-allowed setting from original; should work
    m2_custom = mispstix2.versioning.new_version(m_custom, x_other_custom="abc")
