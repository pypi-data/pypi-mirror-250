...
def test_observed_data_example_with_bad_refs():
    ...
    assert excinfo.value.cls == mispstix2.v20.ObservedData
...
def test_parse_observed_data(data):
    odata = mispstix2.parse(data, version="2.0")
...
def test_parse_artifact_valid(data):
    ...
    odata = mispstix2.parse(odata_str, version="2.0")
...
def test_parse_email_address(data):
    odata = mispstix2.parse_observable(data, {"0": "user-account"}, version='2.0')
    assert odata.type == "email-addr"

    odata_str = re.compile('"belongs_to_ref": "0"', re.DOTALL).sub('"belongs_to_ref": "3"', data)
    with pytest.raises(mispstix2.exceptions.InvalidObjRefError):
        mispstix2.parse_observable(odata_str, {"0": "user-account"}, version='2.0')
...
def test_parse_email_message(data):
    ...
    odata = mispstix2.parse_observable(data, valid_refs, version='2.0')
...
def test_parse_email_message_not_multipart(data):
    ...
    with pytest.raises(mispstix2.exceptions.DependentPropertiesError) as excinfo:
        mispstix2.parse_observable(data, valid_refs, version='2.0')

    assert excinfo.value.cls == mispstix2.v20.EmailMessage
...
def test_parse_basic_tcp_traffic(data):
    odata = mispstix2.parse_observable(
...
def test_parse_basic_tcp_traffic_with_error(data):
    with pytest.raises(mispstix2.exceptions.AtLeastOnePropertyError) as excinfo:
        mispstix2.parse_observable(data, {"4": "network-traffic"}, version='2.0')

    assert excinfo.value.cls == mispstix2.v20.NetworkTraffic
...
def test_artifact_mutual_exclusion_error():
    ...
    assert excinfo.value.cls == mispstix2.v20.Artifact
...
def test_directory_example_ref_error():
    with pytest.raises(mispstix2.exceptions.InvalidObjRefError) as excinfo:
        mispstix2.v20.Directory(
            _valid_refs=[],
            path='/usr/lib',
            created="2015-12-21T19:00:00Z",
            modified="2015-12-24T19:00:00Z",
    ...
    assert excinfo.value.cls == mispstix2.v20.Directory
    assert excinfo.value.prop_name == "contains_refs"


def test_domain_name_example():
    dn = mispstix2.v20.DomainName(
...
def test_domain_name_example_invalid_ref_type():
    with pytest.raises(mispstix2.exceptions.InvalidObjRefError) as excinfo:
        mispstix2.v20.DomainName(
            _valid_refs={"1": "file"},
            value="example.com",
            resolves_to_refs=["1"],
        )

    assert excinfo.value.cls == mispstix2.v20.DomainName
    assert excinfo.value.prop_name == "resolves_to_refs"


def test_file_example():
    f = mispstix2.v20.File(
        name="qwerty.dll",
        hashes={
            "SHA-256": "ceafbfd424be2ca4a5f0402cae090dda2fb0526cf521b60b60077c0f622b285a",
        },
...
def test_file_example_with_empty_NTFSExt():
    ...
    assert excinfo.value.cls == mispstix2.v20.File
...
def test_ip4_address_valid_refs():
    mac1 = mispstix2.v20.MACAddress(
        value="a1:b2:c3:d4:e5:f6",
    )
    mac2 = mispstix2.v20.MACAddress(
...
def test_network_traffic_http_request_example():
    h = mispstix2.v20.HTTPRequestExt(
        request_method="get",
...
def test_network_traffic_icmp_example():
    h = mispstix2.v20.ICMPExt(icmp_type_hex="08", icmp_code_hex="00")
    nt = mispstix2.v20.NetworkTraffic(
        _valid_refs={"0": "ipv4-addr"},
        protocols="tcp",
        src_ref="0",
...
def test_network_traffic_tcp_example():
    h = mispstix2.v20.TCPExt(src_flags_hex="00000002")
    nt = mispstix2.v20.NetworkTraffic(
        _valid_refs={"0": "ipv4-addr"},
        protocols="tcp",
        src_ref="0",
...
def test_mutex_example():
    m = mispstix2.v20.Mutex(name="barney")

    assert m.name == "barney"


def test_process_example():
    p = mispstix2.v20.Process(
        _valid_refs={"0": "file"},
        pid=1221,
        name="gedit-bin",
        created="2016-01-20T14:11:25.55Z",
...
def test_process_example_empty_error():
    with pytest.raises(mispstix2.exceptions.AtLeastOnePropertyError) as excinfo:
        mispstix2.v20.Process()

    assert excinfo.value.cls == mispstix2.v20.Process
    properties_of_process = list(mispstix2.v20.Process._properties.keys())
    ...
    msg = msg.format(
        mispstix2.v20.Process.__name__,
...
def test_process_example_with_WindowsProcessExt_Object():
    p = mispstix2.v20.Process(
        extensions={
            "windows-process-ext": mispstix2.v20.WindowsProcessExt(
                aslr_enabled=True,
                dep_enabled=True,
                priority="HIGH_PRIORITY_CLASS",
                owner_sid="S-1-5-21-186985262-1144665072-74031268-1309",
...
def test_url_example():
    s = mispstix2.v20.URL(value="https://example.com/research/index.html")
...
def test_user_account_unix_account_ext_example():
    u = mispstix2.v20.UNIXAccountExt(
        gid=1001,
        groups=["wheel"],
        home_dir="/home/jdoe",
        shell="/bin/bash",
...
def test_windows_registry_key_example():
    with pytest.raises(InvalidValueError):
        mispstix2.v20.WindowsRegistryValueType(
            name="Foo",
            data="qwerty",
            data_type="string",
        )

    v = mispstix2.v20.WindowsRegistryValueType(
        name="Foo",
        data="qwerty",
        data_type="REG_SZ",
    )
    w = mispstix2.v20.WindowsRegistryKey(
...
def test_x509_certificate_example():
    x509 = mispstix2.v20.X509Certificate(
...
def test_new_version_with_related_objects():
    data = mispstix2.v20.ObservedData(
