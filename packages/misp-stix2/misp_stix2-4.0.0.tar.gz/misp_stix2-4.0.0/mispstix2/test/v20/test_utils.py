...
import mispstix2.serialization
...
@pytest.mark.parametrize(
    'dttm, timestamp', [
        ...
        (mispstix2.utils.STIXdatetime(2017, 7, 1, 0, 0, 0, 1, precision='millisecond'), '2017-07-01T00:00:00.000Z'),
        (mispstix2.utils.STIXdatetime(2017, 7, 1, 0, 0, 0, 1, precision='second'), '2017-07-01T00:00:00Z'),
    ],
)
def test_timestamp_formatting(dttm, timestamp):
    assert mispstix2.utils.format_datetime(dttm) == timestamp
...
def test_parse_datetime(timestamp, dttm):
    assert mispstix2.utils.parse_into_datetime(timestamp) == dttm
...
def test_parse_datetime_precision(timestamp, dttm, precision):
    assert mispstix2.utils.parse_into_datetime(timestamp, precision) == dttm
...
def test_parse_datetime_invalid(ts):
    with pytest.raises(ValueError):
        mispstix2.utils.parse_into_datetime('foobar')
...
def test_get_dict(data):
    assert mispstix2.utils._get_dict(data)
...
def test_get_dict_invalid(data):
    with pytest.raises(ValueError):
        mispstix2.utils._get_dict(data)
...
def test_get_type_from_id(stix_id, type):
    assert mispstix2.utils.get_type_from_id(stix_id) == type


def test_deduplicate(stix_objs1):
    unique = mispstix2.utils.deduplicate(stix_objs1)
...
def test_find_property_index(object, tuple_to_find, expected_index):
    assert mispstix2.serialization.find_property_index(
        object,
        *tuple_to_find
    ) == expected_index
...
def test_iterate_over_values(dict_value, tuple_to_find, expected_index):
    assert mispstix2.serialization._find_property_in_seq(dict_value.values(), *tuple_to_find) == expected_index
...
def test_is_sdo_dict(type_):
    ...
    assert mispstix2.utils.is_sdo(d, "2.0")
...
def test_is_not_sdo_dict(dict_):
    assert not mispstix2.utils.is_sdo(dict_, "2.0")
...
def test_is_sco_dict():
    ...
    assert mispstix2.utils.is_sco(d, "2.0")
...
def test_is_not_sco_dict(dict_):
    assert not mispstix2.utils.is_sco(dict_, "2.0")
...
def test_is_sro_dict(dict_):
    assert mispstix2.utils.is_sro(dict_, "2.0")
...
def test_is_not_sro_dict(dict_):
    assert not mispstix2.utils.is_sro(dict_, "2.0")
...
def test_is_object_dict(dict_):
    assert mispstix2.utils.is_object(dict_, "2.0")
...
def test_is_not_object_dict(dict_):
    assert not mispstix2.utils.is_object(dict_, "2.0")
