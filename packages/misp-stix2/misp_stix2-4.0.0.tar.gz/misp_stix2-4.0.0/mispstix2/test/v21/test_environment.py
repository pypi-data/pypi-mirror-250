...
import mispstix2
import mispstix2.environment
import mispstix2.equivalence.graph
import mispstix2.equivalence.object
import mispstix2.exceptions
...
@pytest.fixture
def ds():
    cam = mispstix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    idy = mispstix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    ind = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    mal = mispstix2.v21.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    rel1 = mispstix2.v21.Relationship(ind, 'indicates', mal, id=RELATIONSHIP_IDS[0])
    rel2 = mispstix2.v21.Relationship(mal, 'targets', idy, id=RELATIONSHIP_IDS[1])
    rel3 = mispstix2.v21.Relationship(cam, 'uses', mal, id=RELATIONSHIP_IDS[2])
    reprt = mispstix2.v21.Report(
    ...
    yield mispstix2.MemoryStore(stix_objs)
...
def ds2_objects():
    cam = mispstix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    idy = mispstix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    ind = mispstix2.v21.Indicator(id=INDICATOR_ID, created_by_ref=idy.id, **INDICATOR_KWARGS)
    indv2 = ind.new_version(
        ...
        object_marking_refs=[mispstix2.v21.TLP_WHITE],
    )
    mal = mispstix2.v21.Malware(id=MALWARE_ID, created_by_ref=idy.id, **MALWARE_KWARGS)
...
@pytest.fixture
def ds2(ds2_objects):
    yield mispstix2.MemoryStore(ds2_objects)


@pytest.fixture
def fs():
    yield mispstix2.FileSystemSource(FS_PATH)


def test_object_factory_created_by_ref_str():
    factory = mispstix2.ObjectFactory(created_by_ref=IDENTITY_ID)
    ind = factory.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID


def test_object_factory_created_by_ref_obj():
    id_obj = mispstix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    factory = mispstix2.ObjectFactory(created_by_ref=id_obj)
    ind = factory.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID
...
def test_object_factory_override_default():
    ...
    ind = factory.create(mispstix2.v21.Indicator, created_by_ref=new_id, **INDICATOR_KWARGS)
    assert ind.created_by_ref == new_id


def test_object_factory_created():
    factory = mispstix2.ObjectFactory(created=FAKE_TIME)
    ind = factory.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
...
def test_object_factory_external_reference():
    ext_ref = mispstix2.v21.ExternalReference(
        source_name="ACME Threat Intel",
        description="Threat report",
    )
    factory = mispstix2.ObjectFactory(external_references=ext_ref)
    ind = factory.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
    assert ind.external_references[0].source_name == "ACME Threat Intel"
    assert ind.external_references[0].description == "Threat report"

    ind2 = factory.create(mispstix2.v21.Indicator, external_references=None, **INDICATOR_KWARGS)
    assert 'external_references' not in ind2


def test_object_factory_obj_markings():
    stmt_marking = mispstix2.v21.StatementMarking("Copyright 2016, Example Corp")
    mark_def = mispstix2.v21.MarkingDefinition(
        definition_type="statement",
        definition=stmt_marking,
    )
    factory = mispstix2.ObjectFactory(object_marking_refs=[mark_def, mispstix2.v21.TLP_AMBER])
    ind = factory.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
    assert mark_def.id in ind.object_marking_refs
    assert mispstix2.v21.TLP_AMBER.id in ind.object_marking_refs

    factory = mispstix2.ObjectFactory(object_marking_refs=mispstix2.v21.TLP_RED)
    ind = factory.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
    assert mispstix2.v21.TLP_RED.id in ind.object_marking_refs
...
def test_object_factory_list_append():
    ...
    ext_ref2 = mispstix2.v21.ExternalReference(
        source_name="Yet Another Threat Report",
        description="Threat report from YATR",
    )
    ext_ref3 = mispstix2.v21.ExternalReference(
        source_name="Threat Report #3",
        description="One more threat report",
    )
    factory = mispstix2.ObjectFactory(external_references=ext_ref)
    ind = factory.create(mispstix2.v21.Indicator, external_references=ext_ref2, **INDICATOR_KWARGS)
    assert ind.external_references[1].source_name == "Yet Another Threat Report"

    ind = factory.create(mispstix2.v21.Indicator, external_references=[ext_ref2, ext_ref3], **INDICATOR_KWARGS)
...
def test_object_factory_list_replace():
    ...
    factory = mispstix2.ObjectFactory(external_references=ext_ref, list_append=False)
    ind = factory.create(mispstix2.v21.Indicator, external_references=ext_ref2, **INDICATOR_KWARGS)
...
def test_environment_functions():
    env = mispstix2.Environment(
        mispstix2.ObjectFactory(created_by_ref=IDENTITY_ID),
        mispstix2.MemoryStore(),
    )

    # Create a STIX object
    ind = env.create(mispstix2.v21.Indicator, id=INDICATOR_ID, **INDICATOR_KWARGS)
    assert ind.created_by_ref == IDENTITY_ID
    ...
    query = [mispstix2.Filter('type', '=', 'vulnerability')]
    resp = env.query(query)
    assert len(resp) == 0

    # See different results after adding filters to the environment
    env.add_filters([
        mispstix2.Filter('type', '=', 'indicator'),
        mispstix2.Filter('created_by_ref', '=', IDENTITY_ID),
    ])
    env.add_filter(mispstix2.Filter('labels', '=', 'benign'))  # should be 'malicious-activity'
    resp = env.get(INDICATOR_ID)
...
def test_environment_source_and_sink():
    ind = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    env = mispstix2.Environment(source=mispstix2.MemorySource([ind]), sink=mispstix2.MemorySink([ind]))
    assert env.get(INDICATOR_ID).indicator_types[0] == 'malicious-activity'


def test_environment_datastore_and_sink():
    with pytest.raises(ValueError) as excinfo:
        mispstix2.Environment(
            factory=mispstix2.ObjectFactory(),
            store=mispstix2.MemoryStore(), sink=mispstix2.MemorySink,
        )
    assert 'Data store already provided' in str(excinfo.value)


def test_environment_no_datastore():
    env = mispstix2.Environment(factory=mispstix2.ObjectFactory())

    with pytest.raises(AttributeError) as excinfo:
        env.add(mispstix2.v21.Indicator(**INDICATOR_KWARGS))
...
def test_environment_datastore_and_no_object_factory():
    ...
    env = mispstix2.Environment(store=mispstix2.MemoryStore())
    ind = env.create(mispstix2.v21.Indicator, id=INDICATOR_ID, **INDICATOR_KWARGS)
...
def test_creator_of():
    identity = mispstix2.v21.Identity(**IDENTITY_KWARGS)
    factory = mispstix2.ObjectFactory(created_by_ref=identity.id)
    env = mispstix2.Environment(store=mispstix2.MemoryStore(), factory=factory)
    env.add(identity)

    ind = env.create(mispstix2.v21.Indicator, **INDICATOR_KWARGS)
...
def test_creator_of_no_datasource():
    identity = mispstix2.v21.Identity(**IDENTITY_KWARGS)
    factory = mispstix2.ObjectFactory(created_by_ref=identity.id)
    env = mispstix2.Environment(factory=factory)
...
def test_relationships(ds):
    env = mispstix2.Environment(store=ds)
...
def test_object_similarity_on_same_attack_pattern1():
    ap1 = mispstix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_PATTERN_KWARGS)
    ap2 = mispstix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_PATTERN_KWARGS)
    env = mispstix2.Environment().object_similarity(ap1, ap2)
...
def test_object_similarity_on_same_attack_pattern2():
    ...
    ap1 = mispstix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_KWARGS)
    ap2 = mispstix2.v21.AttackPattern(id=ATTACK_PATTERN_ID, **ATTACK_KWARGS)
    env = mispstix2.Environment().object_similarity(ap1, ap2)
    assert round(env) == 100


def test_object_similarity_on_same_campaign1():
    camp1 = mispstix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    camp2 = mispstix2.v21.Campaign(id=CAMPAIGN_ID, **CAMPAIGN_KWARGS)
    env = mispstix2.Environment().object_similarity(camp1, camp2)
    assert round(env) == 100
...
def test_object_similarity_on_same_campaign2():
    ...
    camp1 = mispstix2.v21.Campaign(id=CAMPAIGN_ID, **CAMP_KWARGS)
    camp2 = mispstix2.v21.Campaign(id=CAMPAIGN_ID, **CAMP_KWARGS)
    env = mispstix2.Environment().object_similarity(camp1, camp2)
    assert round(env) == 100


def test_object_similarity_on_same_identity1():
    iden1 = mispstix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    iden2 = mispstix2.v21.Identity(id=IDENTITY_ID, **IDENTITY_KWARGS)
    env = mispstix2.Environment().object_similarity(iden1, iden2)
    assert round(env) == 100
...
def test_object_similarity_on_same_identity2():
    ...
    iden1 = mispstix2.v21.Identity(id=IDENTITY_ID, **IDEN_KWARGS)
    iden2 = mispstix2.v21.Identity(id=IDENTITY_ID, **IDEN_KWARGS)
    env = mispstix2.Environment().object_similarity(iden1, iden2)
    assert round(env) == 100


def test_object_similarity_on_same_indicator():
    ind1 = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    ind2 = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    env = mispstix2.Environment().object_similarity(ind1, ind2)
    assert round(env) == 100


def test_object_similarity_on_same_location1():
    location_kwargs = dict(latitude=45, longitude=179)
    loc1 = mispstix2.v21.Location(id=LOCATION_ID, **location_kwargs)
    loc2 = mispstix2.v21.Location(id=LOCATION_ID, **location_kwargs)
    env = mispstix2.Environment().object_similarity(loc1, loc2)
    assert round(env) == 100
...
def test_object_similarity_location_with_no_latlong():
    ...
    loc1 = mispstix2.v21.Location(id=LOCATION_ID, **LOCATION_KWARGS)
    loc2 = mispstix2.v21.Location(id=LOCATION_ID, **loc_kwargs)
    env = mispstix2.Environment().object_similarity(loc1, loc2)
    assert round(env) != 100


def test_object_similarity_on_same_malware():
    malw1 = mispstix2.v21.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    malw2 = mispstix2.v21.Malware(id=MALWARE_ID, **MALWARE_KWARGS)
    env = mispstix2.Environment().object_similarity(malw1, malw2)
    assert round(env) == 100


def test_object_similarity_on_same_threat_actor1():
    ta1 = mispstix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_ACTOR_KWARGS)
    ta2 = mispstix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_ACTOR_KWARGS)
    env = mispstix2.Environment().object_similarity(ta1, ta2)
    assert round(env) == 100
...
def test_object_similarity_on_same_threat_actor2():
    ...
    ta1 = mispstix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_KWARGS)
    ta2 = mispstix2.v21.ThreatActor(id=THREAT_ACTOR_ID, **THREAT_KWARGS)
    env = mispstix2.Environment().object_similarity(ta1, ta2)
    assert round(env) == 100


def test_object_similarity_on_same_tool():
    tool1 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    env = mispstix2.Environment().object_similarity(tool1, tool2)
    assert round(env) == 100


def test_object_similarity_on_same_vulnerability1():
    vul1 = mispstix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
    vul2 = mispstix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
    prop_scores = {}
    env = mispstix2.Environment().object_similarity(vul1, vul2, prop_scores)
    assert round(env) == 100
...
def test_object_equivalence_on_same_vulnerability1():
    ...
    vul2 = mispstix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
    prop_scores = {}
    env = mispstix2.Environment().object_equivalence(vul1, vul2, prop_scores)
    assert env is True
    assert round(prop_scores["matching_score"]) == 30
    assert round(prop_scores["sum_weights"]) == 30
...
def test_object_similarity_on_same_vulnerability2():
    ...
    vul1 = mispstix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULN_KWARGS1)
    vul2 = mispstix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULN_KWARGS2)
    prop_scores = {}
    env = mispstix2.Environment().object_similarity(vul1, vul2, prop_scores)
...
def test_object_similarity_on_unknown_object():
    ...
    def _x_foobar_checks(obj1, obj2, **weights):
        matching_score = 0.0
        sum_weights = 0.0
        if mispstix2.equivalence.object.check_property_present("external_references", obj1, obj2):
            w = weights["external_references"]
            sum_weights += w
            matching_score += w * mispstix2.equivalence.object.partial_external_reference_based(
                obj1["external_references"],
                obj2["external_references"],
            )
        if mispstix2.equivalence.object.check_property_present("name", obj1, obj2):
            w = weights["name"]
            sum_weights += w
            matching_score += w * mispstix2.equivalence.object.partial_string_based(obj1["name"], obj2["name"])
    ...
    cust1 = mispstix2.parse(CUSTOM_KWARGS1, allow_custom=True)
    cust2 = mispstix2.parse(CUSTOM_KWARGS2, allow_custom=True)
    env = mispstix2.Environment().object_similarity(cust1, cust2, **weights)
    assert round(env) == 0
...
def test_object_similarity_different_type_raises():
    with pytest.raises(ValueError) as excinfo:
        vul1 = mispstix2.v21.Vulnerability(id=VULNERABILITY_ID, **VULNERABILITY_KWARGS)
        ind1 = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
        mispstix2.Environment().object_similarity(vul1, ind1)
...
def test_object_similarity_different_spec_version_raises():
    with pytest.raises(ValueError) as excinfo:
        ...
        ind1 = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
        ind2 = mispstix2.v20.Indicator(id=INDICATOR_ID, **V20_KWARGS)
        mispstix2.Environment().object_similarity(ind1, ind2)
...
def test_object_similarity_zero_match():
    ...
    weights = {
        "indicator": {
            "indicator_types": (15, mispstix2.equivalence.object.partial_list_based),
            "pattern": (80, mispstix2.equivalence.object.custom_pattern_based),
            "valid_from": (5, mispstix2.equivalence.object.partial_timestamp_based),
    ...
    ind1 = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    ind2 = mispstix2.v21.Indicator(id=INDICATOR_ID, **IND_KWARGS)
    env = mispstix2.Environment().object_similarity(ind1, ind2, **weights)
    assert round(env) == 0
    env = mispstix2.Environment().object_similarity(ind2, ind1, **weights)
    assert round(env) == 0
...
def test_object_similarity_different_spec_version():
    ...
    ind1 = mispstix2.v21.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS)
    ind2 = mispstix2.v20.Indicator(id=INDICATOR_ID, **IND_KWARGS)
    env = mispstix2.Environment().object_similarity(ind1, ind2, ignore_spec_version=True, **weights)
    assert round(env) == 0

    env = mispstix2.Environment().object_similarity(ind2, ind1, ignore_spec_version=True, **weights)
    assert round(env) == 0
...
def test_object_similarity_external_references(refs1, refs2, ret_val):
    value = mispstix2.equivalence.object.partial_external_reference_based(refs1, refs2)
...
def test_object_similarity_timestamp():
    ...
    assert mispstix2.equivalence.object.partial_timestamp_based(t1, t2, 1) == 0.5


def test_object_similarity_exact_match():
    t1 = "2018-10-17T00:14:20.652Z"
    t2 = "2018-10-17T12:14:20.652Z"
    assert mispstix2.equivalence.object.exact_match(t1, t2) == 0.0


def test_no_datastore_fallsback_list_based_check_for_refs_check():
    r1 = mispstix2.v21.Report(id=REPORT_ID, **REPORT_KWARGS)
    r2 = mispstix2.v21.Report(id=REPORT_ID, **REPORT_KWARGS)
    prop_scores = {}
    assert mispstix2.Environment().object_similarity(r1, r2, prop_scores) == 100.0
...
def test_object_similarity_method_provided():
    ...
    weights = {
        "tool": {
            "tool_types": (20, mispstix2.equivalence.object.partial_list_based),
            "name": (80, mispstix2.equivalence.object.partial_string_based),
            "method": custom_semantic_equivalence_method,
        },
    }

    tool1 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL2_KWARGS)
    env = mispstix2.Environment().object_similarity(tool1, tool2, **weights)
    assert round(env) == 96
...
def test_object_similarity_prop_scores():
    ...
    tool1 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL2_KWARGS)
    mispstix2.Environment().object_similarity(tool1, tool2, prop_scores)
...
def test_object_similarity_prop_scores_method_provided():
    ...
    tool1 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL_KWARGS)
    tool2 = mispstix2.v21.Tool(id=TOOL_ID, **TOOL2_KWARGS)
    env = mispstix2.Environment().object_similarity(tool1, tool2, prop_scores, **weights)
    assert round(env) == 96
...
def test_versioned_checks(ds, ds2):
    ...
    weights = mispstix2.equivalence.graph.WEIGHTS.copy()
    weights.update({
        "_internal": {
    ...
    score = mispstix2.equivalence.object._versioned_checks(INDICATOR_ID, INDICATOR_ID, ds, ds2, **weights)
...
def test_semantic_check_with_versioning(ds, ds2):
    ...
    ds.add(ind)
    score = mispstix2.equivalence.object.reference_check(ind.id, INDICATOR_ID, ds, ds2, **weights)
...
def test_list_semantic_check(ds, ds2):
    ...
    score = mispstix2.equivalence.object.list_reference_check(
        object_refs1,
        object_refs2,
        ds,
        ds2,
...
def test_graph_similarity_raises_value_error(ds):
    with pytest.raises(ValueError):
        prop_scores1 = {}
        mispstix2.Environment().graph_similarity(ds, ds2, prop_scores1, max_depth=-1)


def test_graph_similarity_with_filesystem_source(ds, fs):
    prop_scores1 = {}
    env1 = mispstix2.Environment().graph_similarity(
        fs, ds, prop_scores1,
        ignore_spec_version=True,
        versioning_checks=False,
        max_depth=1,
    ...
    prop_scores2 = {}
    env2 = mispstix2.Environment().graph_similarity(
        ds, fs, prop_scores2,
        ignore_spec_version=True,
        versioning_checks=False,
        max_depth=1,
...
def test_depth_limiting():
    ...
    mem_store1 = mispstix2.MemorySource(g1)
    mem_store2 = mispstix2.MemorySource(g2)

    custom_weights = {
        "foo": {
            "some1_ref": (33, mispstix2.equivalence.object.reference_check),
            "some2_ref": (33, mispstix2.equivalence.object.reference_check),
            "name": (34, mispstix2.equivalence.object.partial_string_based),
        },
    }
    prop_scores1 = {}
    env1 = mispstix2.equivalence.graph.graph_similarity(
    ...
    # Switching parameters
    prop_scores2 = {}
    env2 = mispstix2.equivalence.graph.graph_similarity(
...
def test_graph_similarity_with_duplicate_graph(ds):
    prop_scores = {}
    env = mispstix2.Environment().graph_similarity(ds, ds, prop_scores)
    assert round(env) == 100
...
def test_graph_similarity_with_versioning_check_on(ds2, ds):
    prop_scores1 = {}
    env1 = mispstix2.Environment().graph_similarity(ds, ds2, prop_scores1, versioning_checks=True)
    ...
    # Switching parameters
    prop_scores2 = {}
    env2 = mispstix2.Environment().graph_similarity(ds2, ds, prop_scores2, versioning_checks=True)
...
def test_graph_similarity_with_versioning_check_off(ds2, ds):
    prop_scores1 = {}
    env1 = mispstix2.Environment().graph_similarity(ds, ds2, prop_scores1)
    assert round(env1) == 88
    assert round(prop_scores1["matching_score"]) == 789
    assert round(prop_scores1["len_pairs"]) == 9
    ...
    # Switching parameters
    prop_scores2 = {}
    env2 = mispstix2.Environment().graph_similarity(ds2, ds, prop_scores2)
    assert round(env2) == 88
    assert round(prop_scores2["matching_score"]) == 789
    assert round(prop_scores2["len_pairs"]) == 9
...
def test_graph_equivalence_with_filesystem_source(ds, fs):
    prop_scores1 = {}
    env1 = mispstix2.Environment().graph_equivalence(fs, ds, prop_scores1, ignore_spec_version=True)

    # Switching parameters
    prop_scores2 = {}
    env2 = mispstix2.Environment().graph_equivalence(ds, fs, prop_scores2, ignore_spec_version=True)
...
def test_graph_equivalence_with_duplicate_graph(ds):
    prop_scores = {}
    env = mispstix2.Environment().graph_equivalence(ds, ds, prop_scores)
    assert env is True
    assert round(prop_scores["matching_score"]) == 800
    assert round(prop_scores["len_pairs"]) == 8
...
def test_graph_equivalence_with_versioning_check_on(ds2, ds):
    prop_scores1 = {}
    env1 = mispstix2.Environment().graph_equivalence(ds, ds2, prop_scores1, versioning_checks=True)

    # Switching parameters
    prop_scores2 = {}
    env2 = mispstix2.Environment().graph_equivalence(ds2, ds, prop_scores2, versioning_checks=True)
...
def test_graph_equivalence_with_versioning_check_off(ds2, ds):
    prop_scores1 = {}
    env1 = mispstix2.Environment().graph_equivalence(ds, ds2, prop_scores1)

    # Switching parameters
    prop_scores2 = {}
    env2 = mispstix2.Environment().graph_equivalence(ds2, ds, prop_scores2)
