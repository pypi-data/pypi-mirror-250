...
def test_observed_data_example():
    ...
    assert observed_data.objects['0'] == mispstix2.v21.File(name="foo.exe")
...
def test_observed_data_example_with_refs():
    ...
    assert observed_data.number_observed == 50
    assert observed_data.objects['0'] == mispstix2.v21.File(name="foo.exe")
    assert observed_data.objects['1'] == mispstix2.v21.Directory(path="/usr/home", contains_refs=["file--5956efbb-a7b0-566d-a7f9-a202eb05c70f"])
...
def test_observed_data_object_constraint():
    with pytest.raises(mispstix2.exceptions.MutuallyExclusivePropertiesError):
        mispstix2.v21.ObservedData(
            id="observed-data--b67d30ff-02ac-498a-92f9-32f845f448cf",
            created_by_ref="identity--f431f809-377b-45e0-aa1c-6a4751cae5ff",
            created="2016-04-06T19:58:16.000Z",
            modified="2016-04-06T19:58:16.000Z",
...
def test_observed_data_example_with_bad_refs():
    ...
    assert excinfo.value.cls == mispstix2.v21.Directory
...
def test_observed_data_example_with_non_dictionary():
    ...
    assert excinfo.value.cls == mispstix2.v21.ObservedData
...
def test_parse_observed_data(data):
    odata = mispstix2.parse(data, version="2.1")
...
def test_parse_artifact_valid(data):
    ...
    odata = mispstix2.parse(odata_str, version="2.1")
...
def test_parse_artifact_invalid(data):
    odata_str = OBJECTS_REGEX.sub('"objects": { %s }' % data, EXPECTED)
    with pytest.raises(mispstix2.exceptions.InvalidValueError):
        mispstix2.parse(odata_str, version="2.1")


def test_artifact_example_dependency_error():
    with pytest.raises(mispstix2.exceptions.DependentPropertiesError) as excinfo:
        mispstix2.v21.Artifact(url="http://example.com/sirvizio.exe")
...
def test_parse_email_message_not_multipart(data):
    ...
    assert excinfo.value.cls == mispstix2.v21.EmailMessage
...
def test_parse_basic_tcp_traffic_with_error(data):
    with pytest.raises(mispstix2.exceptions.AtLeastOnePropertyError) as excinfo:
        mispstix2.parse(data, version='2.1')

    assert excinfo.value.cls == mispstix2.v21.NetworkTraffic
...
def test_artifact_mutual_exclusion_error():
    with pytest.raises(mispstix2.exceptions.MutuallyExclusivePropertiesError) as excinfo:
        mispstix2.v21.Artifact(
            mime_type="image/jpeg",
            url="https://upload.wikimedia.org/wikipedia/commons/b/b4/JPEG_example_JPG_RIP_100.jpg",
            hashes={
                "MD5": "6826f9a05da08134006557758bb3afbb",
    ...
    assert excinfo.value.cls == mispstix2.v21.Artifact
...
def test_directory_example():
    f = mispstix2.v21.File(
        name="penguin.exe",
    )

    dir1 = mispstix2.v21.Directory(
        path='/usr/lib',
...
def test_domain_name_example():
    dn1 = mispstix2.v21.DomainName(
        value="mitre.org",
    )

    dn2 = mispstix2.v21.DomainName(
        value="example.com",
...
def test_domain_name_example_invalid_ref_type():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.DomainName(
            value="example.com",
            resolves_to_refs=["file--44a431e6-764b-5556-a3f5-bf655930a581"],
        )

    assert excinfo.value.cls == mispstix2.v21.DomainName
...
def test_file_example_with_empty_NTFSExt():
    ...
    assert excinfo.value.cls == mispstix2.v21.File
...
def test_file_with_archive_ext_object():
    ad = mispstix2.v21.Directory(path="archived/path")
    f_obj = mispstix2.v21.File(
        name="foo", extensions={
            "archive-ext": {
                "contains_refs": [ad],
            },
...
def test_file_example_encryption_error():
    with pytest.raises(mispstix2.exceptions.AtLeastOnePropertyError) as excinfo:
        mispstix2.v21.File(magic_number_hex="010b")
...
def test_ipv4_address_valid_refs():
    mac1 = mispstix2.v21.MACAddress(
        value="a1:b2:c3:d4:e5:f6",
    )
    mac2 = mispstix2.v21.MACAddress(
...
def test_network_traffic_http_request_example():
    h = mispstix2.v21.HTTPRequestExt(
        request_method="get",
...
def test_network_traffic_icmp_example():
    h = mispstix2.v21.ICMPExt(icmp_type_hex="08", icmp_code_hex="00")
    nt = mispstix2.v21.NetworkTraffic(
        protocols=["tcp"],
        src_ref="ipv4-addr--29a591d9-533a-5ecd-a5a1-cadee4411e88",
...
def test_network_traffic_tcp_example():
    h = mispstix2.v21.TCPExt(src_flags_hex="00000002")
    nt = mispstix2.v21.NetworkTraffic(
        protocols=["tcp"],
        src_ref="ipv4-addr--29a591d9-533a-5ecd-a5a1-cadee4411e88",
...
def test_mutex_example():
    m = mispstix2.v21.Mutex(name="barney")

    assert m.name == "barney"


def test_process_example():
    p = mispstix2.v21.Process(
        pid=1221,
        created_time="2016-01-20T14:11:25.55Z",
        command_line="./gedit-bin --new-window",
...
def test_process_example_empty_error():
    with pytest.raises(mispstix2.exceptions.AtLeastOnePropertyError) as excinfo:
        mispstix2.v21.Process()

    assert excinfo.value.cls == mispstix2.v21.Process
    properties_of_process = mispstix2.v21.Process._properties.keys()
    ...
    msg = msg.format(
        mispstix2.v21.Process.__name__,
...
def test_process_example_with_WindowsProcessExt_Object():
    p = mispstix2.v21.Process(
        extensions={
            "windows-process-ext": mispstix2.v21.WindowsProcessExt(
                aslr_enabled=True,
                dep_enabled=True,
                priority="HIGH_PRIORITY_CLASS",
                owner_sid="S-1-5-21-186985262-1144665072-74031268-1309",
...
def test_url_example():
    s = mispstix2.v21.URL(value="https://example.com/research/index.html")
...
def test_user_account_unix_account_ext_example():
    u = mispstix2.v21.UNIXAccountExt(
        gid=1001,
        groups=["wheel"],
        home_dir="/home/jdoe",
        shell="/bin/bash",
...
def test_windows_registry_key_example():
    with pytest.raises(mispstix2.exceptions.InvalidValueError):
        mispstix2.v21.WindowsRegistryValueType(
            name="Foo",
            data="qwerty",
            data_type="string",
        )

    v = mispstix2.v21.WindowsRegistryValueType(
        name="Foo",
        data="qwerty",
        data_type="REG_SZ",
    )
    w = mispstix2.v21.WindowsRegistryKey(
...
def test_x509_certificate_example():
    x509 = mispstix2.v21.X509Certificate(
...
def test_x509_certificate_error():
    ...
    with pytest.raises(mispstix2.exceptions.PropertyPresenceError) as excinfo:
        mispstix2.v21.X509Certificate(
            defanged=True,
        )

    assert excinfo.value.cls == mispstix2.v21.X509Certificate
    assert "At least one of the" in str(excinfo.value)
    assert "properties for X509Certificate must be populated." in str(excinfo.value)


def test_new_version_with_related_objects():
    data = mispstix2.v21.ObservedData(
...
def test_objects_deprecation():
    with pytest.warns(mispstix2.exceptions.STIXDeprecationWarning):
