...
import mispstix2.canonicalization.Canonicalize
import mispstix2.exceptions
from mispstix2.properties import (
...
from mispstix2.v21.vocab import HASHING_ALGORITHM
...
def test_no_contrib_props_defined():
    ...
    class SomeSCO(mispstix2.v21._Observable):
        _type = "some-sco"
...
def test_json_compatible_prop_values():
    ...
    can_json = mispstix2.canonicalization.Canonicalize.canonicalize(obj, utf8=False)
...
def test_embedded_object():
    class SubObj(mispstix2.base._STIXBase):
        _type = "sub-object"
        _properties = OrderedDict((
...
def test_json_unescaping(json_escaped, expected_unescaped):
    actual_unescaped = mispstix2.base._un_json_escape(json_escaped)
    assert actual_unescaped == expected_unescaped


def test_json_unescaping_bad_escape():
    with pytest.raises(ValueError):
        mispstix2.base._un_json_escape(r"\x")


def test_deterministic_id_same_extra_prop_vals():
    email_addr_1 = mispstix2.v21.EmailAddress(
        value="john@example.com",
        display_name="Johnny Doe",
    )

    email_addr_2 = mispstix2.v21.EmailAddress(
        value="john@example.com",
        display_name="Johnny Doe",
    )
...
def test_deterministic_id_no_contributing_props():
    email_msg_1 = mispstix2.v21.EmailMessage(
        is_multipart=False,
    )

    email_msg_2 = mispstix2.v21.EmailMessage(
        is_multipart=False,
    )
...
def test_id_gen_recursive_dict_conversion_1():
    file_observable = mispstix2.v21.File(
        name="example.exe",
        size=68 * 1000,
        ...
        extensions={
            "windows-pebinary-ext": mispstix2.v21.WindowsPEBinaryExt(
                pe_type="exe",
                machine_hex="014c",
                sections=[
                    mispstix2.v21.WindowsPESection(
                        name=".data",
                        size=4096,
                        entropy=7.980693,
...
def test_id_gen_recursive_dict_conversion_2():
    wrko = mispstix2.v21.WindowsRegistryKey(
        values=[
