...
import mispstix2
from mispstix2.v21 import TLP_WHITE
...
def test_marking_def_example_with_statement_positional_argument():
    marking_definition = mispstix2.v21.MarkingDefinition(
        id=MARKING_DEFINITION_ID,
        created="2017-01-20T00:00:00.000Z",
        definition_type="statement",
        definition=mispstix2.StatementMarking(statement="Copyright 2016, Example Corp"),
    )
...
def test_marking_def_example_with_kwargs_statement():
    ...
    marking_definition = mispstix2.v21.MarkingDefinition(
        ...
        created="2017-01-20T00:00:00.000Z",
        definition_type="statement",
        definition=mispstix2.StatementMarking(**kwargs),
    )
...
def test_marking_def_invalid_type():
    with pytest.raises(ValueError):
        mispstix2.v21.MarkingDefinition(
            id=MARKING_DEFINITION_ID,
            created="2017-01-20T00:00:00.000Z",
            definition_type="my-definition-type",
            definition=mispstix2.StatementMarking("Copyright 2016, Example Corp"),
        )


def test_campaign_with_markings_example():
    campaign = mispstix2.v21.Campaign(
        id="campaign--8e2e2d2b-17d4-4cbf-938f-98ee46b3cd3f",
...
def test_granular_example():
    granular_marking = mispstix2.v21.GranularMarking(
...
def test_granular_example_with_bad_selector():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.GranularMarking(
            marking_ref=MARKING_DEFINITION_ID,
            selectors=["abc[0]"],   # missing "."
        )

    assert excinfo.value.cls == mispstix2.v21.GranularMarking
...
def test_parse_marking_definition(data):
    gm = mispstix2.parse(data, version="2.1")
...
def test_registered_custom_marking():
    ...
    marking_def = mispstix2.v21.MarkingDefinition(
...
def test_marking_wrong_type_construction():
    with pytest.raises(ValueError):
        ...
        @mispstix2.v21.CustomMarking('x-new-marking-type2', ("a", "b"))
