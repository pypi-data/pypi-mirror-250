...
def test_observed_data_example_with_bad_refs():
    ...
    assert excinfo.value.cls == misp_stix2.v20.ObservedData
...
def test_parse_observed_data(data):
    odata = misp_stix2.parse(data, version="2.0")
...
def test_parse_artifact_valid(data):
    ...
    odata = misp_stix2.parse(odata_str, version="2.0")
...
def test_parse_email_address(data):
    odata = misp_stix2.parse_observable(data, {"0": "user-account"}, version='2.0')
    assert odata.type == "email-addr"

    odata_str = re.compile('"belongs_to_ref": "0"', re.DOTALL).sub('"belongs_to_ref": "3"', data)
    with pytest.raises(misp_stix2.exceptions.InvalidObjRefError):
        misp_stix2.parse_observable(odata_str, {"0": "user-account"}, version='2.0')
...
def test_parse_email_message(data):
    ...
    odata = misp_stix2.parse_observable(data, valid_refs, version='2.0')
...
def test_parse_email_message_not_multipart(data):
    ...
    with pytest.raises(misp_stix2.exceptions.DependentPropertiesError) as excinfo:
        misp_stix2.parse_observable(data, valid_refs, version='2.0')

    assert excinfo.value.cls == misp_stix2.v20.EmailMessage
...
def test_parse_basic_tcp_traffic(data):
    odata = misp_stix2.parse_observable(
...
def test_parse_basic_tcp_traffic_with_error(data):
    with pytest.raises(misp_stix2.exceptions.AtLeastOnePropertyError) as excinfo:
        misp_stix2.parse_observable(data, {"4": "network-traffic"}, version='2.0')

    assert excinfo.value.cls == misp_stix2.v20.NetworkTraffic
...
def test_artifact_mutual_exclusion_error():
    ...
    assert excinfo.value.cls == misp_stix2.v20.Artifact
...
def test_directory_example_ref_error():
    with pytest.raises(misp_stix2.exceptions.InvalidObjRefError) as excinfo:
        misp_stix2.v20.Directory(
            _valid_refs=[],
            path='/usr/lib',
            created="2015-12-21T19:00:00Z",
            modified="2015-12-24T19:00:00Z",
    ...
    assert excinfo.value.cls == misp_stix2.v20.Directory
    assert excinfo.value.prop_name == "contains_refs"


def test_domain_name_example():
    dn = misp_stix2.v20.DomainName(
...
def test_domain_name_example_invalid_ref_type():
    with pytest.raises(misp_stix2.exceptions.InvalidObjRefError) as excinfo:
        misp_stix2.v20.DomainName(
            _valid_refs={"1": "file"},
            value="example.com",
            resolves_to_refs=["1"],
        )

    assert excinfo.value.cls == misp_stix2.v20.DomainName
    assert excinfo.value.prop_name == "resolves_to_refs"


def test_file_example():
    f = misp_stix2.v20.File(
        name="qwerty.dll",
        hashes={
            "SHA-256": "ceafbfd424be2ca4a5f0402cae090dda2fb0526cf521b60b60077c0f622b285a",
        },
...
def test_file_example_with_empty_NTFSExt():
    ...
    assert excinfo.value.cls == misp_stix2.v20.File
...
def test_raster_image_ext_parse():
    obj = misp_stix2.parse(RASTER_IMAGE_EXT, version="2.0")
    assert obj.objects["0"].extensions['raster-image-ext'].image_width == 1024


def test_raster_images_ext_create():
    ext = misp_stix2.v20.RasterImageExt(image_width=1024)
...
def test_ip4_address_example():
    ip4 = misp_stix2.v20.IPv4Address(
...
def test_ip4_address_valid_refs():
    mac1 = misp_stix2.v20.MACAddress(
        value="a1:b2:c3:d4:e5:f6",
    )
    mac2 = misp_stix2.v20.MACAddress(
...
def test_ip4_address_example_cidr():
    ip4 = misp_stix2.v20.IPv4Address(value="198.51.100.0/24")

    assert ip4.value == "198.51.100.0/24"


def test_ip6_address_example():
    ip6 = misp_stix2.v20.IPv6Address(value="2001:0db8:85a3:0000:0000:8a2e:0370:7334")

    assert ip6.value == "2001:0db8:85a3:0000:0000:8a2e:0370:7334"


def test_mac_address_example():
    ip6 = misp_stix2.v20.MACAddress(value="d2:fb:49:24:37:18")

    assert ip6.value == "d2:fb:49:24:37:18"


def test_network_traffic_example():
    nt = misp_stix2.v20.NetworkTraffic(
...
def test_network_traffic_http_request_example():
    h = misp_stix2.v20.HTTPRequestExt(
        request_method="get",
...
def test_network_traffic_icmp_example():
    h = misp_stix2.v20.ICMPExt(icmp_type_hex="08", icmp_code_hex="00")
    nt = misp_stix2.v20.NetworkTraffic(
        _valid_refs={"0": "ipv4-addr"},
        protocols="tcp",
        src_ref="0",
...
def test_network_traffic_tcp_example():
    h = misp_stix2.v20.TCPExt(src_flags_hex="00000002")
    nt = misp_stix2.v20.NetworkTraffic(
        _valid_refs={"0": "ipv4-addr"},
        protocols="tcp",
        src_ref="0",
...
def test_mutex_example():
    m = misp_stix2.v20.Mutex(name="barney")

    assert m.name == "barney"


def test_process_example():
    p = misp_stix2.v20.Process(
        _valid_refs={"0": "file"},
        pid=1221,
        name="gedit-bin",
        created="2016-01-20T14:11:25.55Z",
...
def test_process_example_empty_error():
    with pytest.raises(misp_stix2.exceptions.AtLeastOnePropertyError) as excinfo:
        misp_stix2.v20.Process()

    assert excinfo.value.cls == misp_stix2.v20.Process
    properties_of_process = list(misp_stix2.v20.Process._properties.keys())
    ...
    msg = msg.format(
        misp_stix2.v20.Process.__name__,
...
def test_process_example_with_WindowsProcessExt_Object():
    p = misp_stix2.v20.Process(
        extensions={
            "windows-process-ext": misp_stix2.v20.WindowsProcessExt(
                aslr_enabled=True,
                dep_enabled=True,
                priority="HIGH_PRIORITY_CLASS",
                owner_sid="S-1-5-21-186985262-1144665072-74031268-1309",
...
def test_url_example():
    s = misp_stix2.v20.URL(value="https://example.com/research/index.html")
...
def test_user_account_unix_account_ext_example():
    u = misp_stix2.v20.UNIXAccountExt(
        gid=1001,
        groups=["wheel"],
        home_dir="/home/jdoe",
        shell="/bin/bash",
...
def test_windows_registry_key_example():
    with pytest.raises(InvalidValueError):
        misp_stix2.v20.WindowsRegistryValueType(
            name="Foo",
            data="qwerty",
            data_type="string",
        )

    v = misp_stix2.v20.WindowsRegistryValueType(
        name="Foo",
        data="qwerty",
        data_type="REG_SZ",
    )
    w = misp_stix2.v20.WindowsRegistryKey(
...
def test_x509_certificate_example():
    x509 = misp_stix2.v20.X509Certificate(
...
def test_new_version_with_related_objects():
    data = misp_stix2.v20.ObservedData(
