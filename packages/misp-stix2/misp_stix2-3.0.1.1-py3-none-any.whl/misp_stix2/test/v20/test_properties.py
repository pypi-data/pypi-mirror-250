...
import misp_stix2
from misp_stix2.exceptions import (
...
from misp_stix2.v20.common import MarkingProperty
...
def test_embedded_property():
    emb_prop = EmbeddedObjectProperty(type=misp_stix2.v20.EmailMIMEComponent)
    mime = misp_stix2.v20.EmailMIMEComponent(
...
def test_embedded_property_dict():
    ...
    result = emb_prop.clean(mime, False)
    assert isinstance(result[0], misp_stix2.v20.EmailMIMEComponent)
...
def test_extension_property_valid():
    ...
    assert isinstance(
        result[0]["windows-pebinary-ext"], misp_stix2.v20.WindowsPEBinaryExt,
    )
    assert not result[1]
...
def test_stix_property_not_compliant_spec():
    ...
    indicator = misp_stix2.v20.Indicator(spec_version="2.0", allow_custom=True, **constants.INDICATOR_KWARGS)
...
def test_observable_property_obj():
    ...
    obs = misp_stix2.v20.File(name="data.dat")
    obs_dict = {
        "0": obs,
    }
...
def test_observable_property_dict():
    ...
    result = prop.clean(obs_dict, False)
    assert isinstance(result[0]["0"], misp_stix2.v20.File)
...
def test_observable_property_obj_custom():
    ...
    obs = misp_stix2.v20.File(name="data.dat", foo=True, allow_custom=True)
    obs_dict = {
        "0": obs,
    }
...
def test_stix_object_property_custom_prop():
    ...
    assert isinstance(result[0], misp_stix2.v20.Identity)
