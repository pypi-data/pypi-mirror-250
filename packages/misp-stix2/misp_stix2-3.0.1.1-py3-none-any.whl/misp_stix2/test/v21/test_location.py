...
def test_location_with_some_required_properties():
    ...
    location = misp_stix2.v21.Location(
        id=LOCATION_ID,
        created=now,
        modified=now,
        latitude=48.8566,
...
def test_parse_location(data):
    location = misp_stix2.parse(data, version="2.1")
...
def test_location_complex_presence_constraint():
    with pytest.raises(misp_stix2.exceptions.PropertyPresenceError):
        misp_stix2.parse({
            "type": "location",
            "spec_version": "2.1",
            "id": LOCATION_ID,
