...
import misp_stix2
from misp_stix2.datastore.filesystem import (
    AuthSet, _find_search_optimizations, _get_matching_dir_entries,
    _timestamp2filename,
)
from misp_stix2.exceptions import STIXError
...
def fs_store():
    # create
    yield misp_stix2.FileSystemStore(FS_PATH)
...
def fs_source():
    # create
    fs = misp_stix2.FileSystemSource(FS_PATH)
...
def fs_sink():
    # create
    fs = misp_stix2.FileSystemSink(FS_PATH)
    assert fs.stix_dir == FS_PATH
    yield fs
...
def rel_fs_store():
    ...
    fs = misp_stix2.FileSystemStore(FS_PATH)
    for o in stix_objs:
        fs.add(o)
    yield fs
...
def test_filesystem_source_nonexistent_folder():
    with pytest.raises(ValueError):
        misp_stix2.FileSystemSource('nonexistent-folder')


def test_filesystem_sink_nonexistent_folder():
    with pytest.raises(ValueError):
        misp_stix2.FileSystemSink('nonexistent-folder')
...
def test_filesystem_source_query_single(fs_source):
    # query2
    is_2 = fs_source.query([misp_stix2.Filter("external_references.external_id", '=', "T1027")])
    assert len(is_2) == 1
...
def test_filesystem_source_query_multiple(fs_source):
    # query
    intrusion_sets = fs_source.query([misp_stix2.Filter("type", '=', "intrusion-set")])
...
def test_filesystem_source_backward_compatible(fs_source):
    ...
    results = fs_source.query([
        misp_stix2.Filter("type", "=", "malware"),
        misp_stix2.Filter("id", "=", "malware--6b616fc1-1505-48e3-8b2c-0d19337bff38"),
        misp_stix2.Filter("modified", "=", modified),
    ])
...
def test_filesystem_source_sco(fs_source):
    results = fs_source.query([misp_stix2.Filter("type", "=", "directory")])
...
def test_filesystem_sink_add_stix_object_dict(fs_sink, fs_source):
    ...
    # different name.
    camp2obj = misp_stix2.parse(camp2)
    filepath = os.path.join(
...
def test_filesystem_sink_add_stix_bundle_dict(fs_sink, fs_source):
    ...
    camp_obj = misp_stix2.parse(bund["objects"][0])
    filepath = os.path.join(
...
def test_filesystem_sink_add_json_stix_object(fs_sink, fs_source):
    ...
    camp4obj = misp_stix2.parse(camp4)
    filepath = os.path.join(
        FS_PATH, "campaign",
...
def test_filesystem_sink_json_stix_bundle(fs_sink, fs_source):
    ...
    bund2obj = misp_stix2.parse(bund2)
...
def test_filesystem_sink_add_objects_list(fs_sink, fs_source):
    ...
    camp7obj = misp_stix2.parse(camp7)
...
def test_filesystem_sink_marking(fs_sink):
    marking = misp_stix2.v21.MarkingDefinition(
        id="marking-definition--34098fce-860f-48ae-8e50-ebd3cc5e41da",
        created="2017-01-20T00:00:00.000Z",
        definition_type="tlp",
        definition=misp_stix2.v21.TLPMarking(tlp="green"),
    )
...
def test_filesystem_store_query(fs_store):
    # query()
    tools = fs_store.query([misp_stix2.Filter("tool_types", "in", "tool")])
    assert len(tools) == 2
...
def test_filesystem_store_query_single_filter(fs_store):
    query = misp_stix2.Filter("tool_types", "in", "tool")
...
def test_filesystem_store_query_multiple_filters(fs_store):
    fs_store.source.filters.add(misp_stix2.Filter("tool_types", "in", "tool"))
    tools = fs_store.query(misp_stix2.Filter("id", "=", "tool--242f3da3-4425-4d11-8f5c-b842886da966"))
    assert len(tools) == 1
...
def test_filesystem_store_query_dont_include_type_folder(fs_store):
    results = fs_store.query(misp_stix2.Filter("type", "!=", "tool"))
...
def test_filesystem_store_add_as_bundle():
    fs_store = misp_stix2.FileSystemStore(FS_PATH, bundlify=True)
...
def test_filesystem_store_add_sco(fs_store):
    sco = misp_stix2.v21.EmailAddress(
...
def test_filesystem_object_with_custom_property_in_bundle(fs_store):
    ...
    bundle = misp_stix2.v21.Bundle(camp, allow_custom=True)
    fs_store.add(bundle)
...
def test_optimize_types3():
    filters = [
        misp_stix2.Filter("type", "in", ["A", "B", "C"]),
        misp_stix2.Filter("type", "in", ["B", "C", "D"]),
    ]
...
def test_optimize_types4():
    filters = [
        misp_stix2.Filter("type", "in", ["A", "B", "C"]),
        misp_stix2.Filter("type", "in", ["D", "E", "F"]),
    ]
...
def test_optimize_types5():
    filters = [
        misp_stix2.Filter("type", "in", ["foo", "bar"]),
        misp_stix2.Filter("type", "!=", "bar"),
    ]
...
def test_optimize_types_ids1():
    filters = [
        misp_stix2.Filter("type", "in", ["foo", "bar"]),
        misp_stix2.Filter("id", "=", "foo--00000000-0000-0000-0000-000000000000"),
    ]
...
def test_optimize_types_ids2():
    filters = [
        misp_stix2.Filter("type", "=", "foo"),
        misp_stix2.Filter("id", "=", "bar--00000000-0000-0000-0000-000000000000"),
    ]
...
def test_optimize_types_ids5():
    filters = [
        ...
        misp_stix2.Filter("id", "!=", "D--00000000-0000-0000-0000-000000000000"),
    ]
...
def test_optimize_types_ids6():
    filters = [
        misp_stix2.Filter("id", "=", "A--00000000-0000-0000-0000-000000000000"),
    ]
