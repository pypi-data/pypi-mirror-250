...
if __name__ == '__main__':
    attack_pattern = misp_stix2.v21.AttackPattern(**ATTACK_PATTERN_KWARGS, interoperability=True)
    campaign = misp_stix2.v21.Campaign(**CAMPAIGN_KWARGS, interoperability=True)
    course_of_action = misp_stix2.v21.CourseOfAction(**COURSE_OF_ACTION_KWARGS, interoperability=True)
    identity = misp_stix2.v21.Identity(**IDENTITY_KWARGS, interoperability=True)
    indicator = misp_stix2.v21.Indicator(**INDICATOR_KWARGS, interoperability=True)
    intrusion_set = misp_stix2.v21.IntrusionSet(**INTRUSION_SET_KWARGS, interoperability=True)
    malware = misp_stix2.v21.Malware(**MALWARE_KWARGS, interoperability=True)
    marking_definition = misp_stix2.v21.MarkingDefinition(**MARKING_DEFINITION_KWARGS, interoperability=True)
    observed_data = misp_stix2.v21.ObservedData(**OBSERVED_DATA_KWARGS, interoperability=True)
    relationship = misp_stix2.v21.Relationship(**RELATIONSHIP_KWARGS, interoperability=True)
    sighting = misp_stix2.v21.Sighting(**SIGHTING_KWARGS, interoperability=True)
    threat_actor = misp_stix2.v21.ThreatActor(**THREAT_ACTOR_KWARGS, interoperability=True)
    tool = misp_stix2.v21.Tool(**TOOL_KWARGS)
    vulnerability = misp_stix2.v21.Vulnerability(**VULNERABILITY_KWARGS, interoperability=True)
    report = misp_stix2.v21.Report(**REPORT_KWARGS, interoperability=True)
    ...
    misp_stix2.parse(dict(bundle), interoperability=True)
