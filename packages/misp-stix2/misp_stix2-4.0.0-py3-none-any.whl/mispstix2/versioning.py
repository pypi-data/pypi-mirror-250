...
def _get_stix_version(data):
    ...
    if isinstance(data, Mapping):
        ...
        # work for dicts.
        if isinstance(data, mispstix2.v20._STIXBase20):
            stix_version = "2.0"
        elif isinstance(data, mispstix2.v21._STIXBase21):
            stix_version = "2.1"
...
def _is_versionable_type(data):
    ...
    if isinstance(data, Mapping):
        ...
        if isinstance(data, mispstix2.base._STIXBase):
            ...
            cls = mispstix2.registry.class_for_type(data.get("type"), stix_version)
            if cls:
                is_versionable = _VERSIONING_PROPERTIES.issubset(
                    cls._properties,
                )
...
def new_version(data, allow_custom=None, **kwargs):
    ...
    if is_sco(data, "2.1"):
        ...
        if uuid_.variant == uuid.RFC_4122 and uuid_.version == 5:
            if isinstance(data, mispstix2.base._Observable):
                cls = data.__class__
            else:
                cls = mispstix2.registry.class_for_type(
