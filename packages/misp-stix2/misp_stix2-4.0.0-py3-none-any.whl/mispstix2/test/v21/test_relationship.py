...
def test_relationship_all_required_properties():
    ...
    rel = mispstix2.v21.Relationship(
        type='relationship',
        id=RELATIONSHIP_ID,
        created=now,
        modified=now,
...
def test_relationship_type_must_be_relationship():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.Relationship(type='xxx', **RELATIONSHIP_KWARGS)

    assert excinfo.value.cls == mispstix2.v21.Relationship
...
def test_relationship_id_must_start_with_relationship():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.Relationship(id='my-prefix--', **RELATIONSHIP_KWARGS)
...
def test_relationship_missing_some_required_properties():
    with pytest.raises(mispstix2.exceptions.MissingPropertiesError) as excinfo:
        mispstix2.v21.Relationship(relationship_type='indicates')
...
def test_invalid_kwarg_to_relationship():
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError) as excinfo:
        mispstix2.v21.Relationship(my_custom_property="foo", **RELATIONSHIP_KWARGS)
...
def test_create_relationship_with_positional_args(indicator, malware):
    rel = mispstix2.v21.Relationship(indicator, 'indicates', malware)
...
def test_parse_relationship(data):
    rel = mispstix2.parse(data, version="2.1")
