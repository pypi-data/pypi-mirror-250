...
def test_indicator_type_must_be_indicator():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.Indicator(type='xxx', **INDICATOR_KWARGS)

    assert excinfo.value.cls == mispstix2.v21.Indicator
...
def test_indicator_id_must_start_with_indicator():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.Indicator(id='my-prefix--', **INDICATOR_KWARGS)
...
def test_indicator_required_property_pattern():
    with pytest.raises(mispstix2.exceptions.MissingPropertiesError) as excinfo:
        mispstix2.v21.Indicator(indicator_types=['malicious-activity'])
...
def test_indicator_created_ref_invalid_format():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.Indicator(created_by_ref='myprefix--12345678', **INDICATOR_KWARGS)
...
def test_indicator_revoked_invalid():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v21.Indicator(revoked='no', **INDICATOR_KWARGS)
...
def test_cannot_assign_to_indicator_attributes(indicator):
    with pytest.raises(mispstix2.exceptions.ImmutableError) as excinfo:
...
def test_invalid_kwarg_to_indicator():
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError) as excinfo:
        mispstix2.v21.Indicator(my_custom_property="foo", **INDICATOR_KWARGS)
...
def test_created_modified_time_are_identical_by_default():
    ...
    ind = mispstix2.v21.Indicator(**INDICATOR_KWARGS)
...
def test_parse_indicator(data):
    idctr = mispstix2.parse(data, version="2.1")
...
def test_indicator_with_custom_embed_objs_extra_props_error():
    ...
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError) as excinfo:
        mispstix2.v21.Indicator(external_references=[ext_ref], bad_custom_prop="shouldn't be here", **INDICATOR_KWARGS)
