...
def test_location_with_some_required_properties():
    ...
    location = mispstix2.v21.Location(
        id=LOCATION_ID,
        created=now,
        modified=now,
        latitude=48.8566,
...
def test_parse_location(data):
    location = mispstix2.parse(data, version="2.1")
...
def test_location_complex_presence_constraint():
    with pytest.raises(mispstix2.exceptions.PropertyPresenceError):
        mispstix2.parse({
            "type": "location",
            "spec_version": "2.1",
            "id": LOCATION_ID,
