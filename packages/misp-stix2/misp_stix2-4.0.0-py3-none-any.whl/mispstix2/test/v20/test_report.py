...
import mispstix2
from mispstix2.exceptions import InvalidValueError
...
def test_report_example_objects_in_object_refs():
    report = mispstix2.v20.Report(
        ...
        labels=["campaign"],
        object_refs=[
            mispstix2.v20.Indicator(id=INDICATOR_ID, **INDICATOR_KWARGS),
            CAMPAIGN_ID,
            RELATIONSHIP_ID,
        ],
    )
...
def test_report_example_objects_in_object_refs_with_bad_id():
    ...
    assert excinfo.value.cls == mispstix2.v20.Report
...
def test_parse_report(data):
    rept = mispstix2.parse(data, version="2.0")
