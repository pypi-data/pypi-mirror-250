...
def test_bundle_with_wrong_type():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v20.Bundle(type="not-a-bundle")

    assert excinfo.value.cls == mispstix2.v20.Bundle
...
def test_bundle_id_must_start_with_bundle():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v20.Bundle(id='my-prefix--')
...
def test_create_bundle_fp_serialize_pretty(indicator, malware, relationship):
    bundle = mispstix2.v20.Bundle(objects=[indicator, malware, relationship])
    buffer = io.StringIO()
...
def test_create_bundle_with_positional_args(indicator, malware, relationship):
    bundle = mispstix2.v20.Bundle(indicator, malware, relationship)
...
def test_create_bundle_with_listarg_and_kwarg(indicator, malware, relationship):
    bundle = mispstix2.v20.Bundle([indicator, malware], objects=[relationship])
...
def test_parse_bundle(version):
    bundle = mispstix2.parse(EXPECTED_BUNDLE, version=version)

    assert bundle.type == "bundle"
    assert bundle.id.startswith("bundle--")
    assert isinstance(bundle.objects[0], mispstix2.v20.Indicator)
...
def test_parse_unknown_type():
    ...
    with pytest.raises(mispstix2.exceptions.ParseError) as excinfo:
        mispstix2.parse(unknown, version="2.0")
...
def test_stix_object_property():
    prop = mispstix2.properties.STIXObjectProperty(spec_version='2.0')

    identity = mispstix2.v20.Identity(name="test", identity_class="individual")
...
def test_bundle_with_different_spec_objects():
    ...
    with pytest.raises(InvalidValueError) as excinfo:
        mispstix2.v20.Bundle(objects=data)
...
def test_bundle_obj_id_found():
    bundle = mispstix2.parse(EXPECTED_BUNDLE)
...
def test_bundle_objs_ids_found(bundle_data):
    bundle = mispstix2.parse(bundle_data)
