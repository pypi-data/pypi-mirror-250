...
def test_sighting_all_required_properties():
    ...
    sighting = mispstix2.v20.Sighting(
        type='sighting',
        id=SIGHTING_ID,
        created=now,
        modified=now,
...
def test_sighting_bad_where_sighted_refs():
    ...
    assert excinfo.value.cls == mispstix2.v20.Sighting
    assert excinfo.value.prop_name == "where_sighted_refs"


def test_sighting_type_must_be_sightings():
    with pytest.raises(mispstix2.exceptions.InvalidValueError) as excinfo:
        mispstix2.v20.Sighting(type='xxx', **SIGHTING_KWARGS)
...
def test_invalid_kwarg_to_sighting():
    with pytest.raises(mispstix2.exceptions.ExtraPropertiesError) as excinfo:
        mispstix2.v20.Sighting(my_custom_property="foo", **SIGHTING_KWARGS)
...
def test_create_sighting_from_objects_rather_than_ids(malware):  # noqa: F811
    rel = mispstix2.v20.Sighting(sighting_of_ref=malware)
...
def test_parse_sighting(data):
    sighting = mispstix2.parse(data, version="2.0")
