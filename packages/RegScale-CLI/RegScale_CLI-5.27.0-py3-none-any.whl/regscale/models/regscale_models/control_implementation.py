...
class ControlImplementation(RegScaleModel):
    ...
    def create_or_update(self) -> "ControlImplementation":
        """
        Either create or update a model instance.

        :return: The created or updated object
        """
        existing_object = self.get_by_security_control_id(
            security_control_id=self.controlID
        )
        if any(existing_object):
            self.id = existing_object[0].id
            return self.save()
        else:
            return self.create()
    ...
    def get_by_security_control_id(
        ...
        response = cls._model_api_handler.get(
            endpoint=cls.get_endpoint("get_by_security_control_id").format(
                parentControlId=security_control_id
            )
        )
        if response and response.ok:
    ...
    @staticmethod
    def _extract_text_and_log(element, logger):
        """
    ...
    @staticmethod
    def _update_implementation_status(element, imp):
        """
    ...
    def process_controls(
        ...
        :param bool include_not_implemented: Whether to include not implemented controls, defaults to False
