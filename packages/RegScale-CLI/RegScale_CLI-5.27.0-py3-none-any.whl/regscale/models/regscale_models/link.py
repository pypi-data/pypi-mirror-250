...
class Links(RegScaleModel):
    ...
    def get_all_by_parent(cls, parent_id, parent_module):
        """
        Retrieve all links for a given parent record.
        :param int parent_id: The ID of the parent record
        :param str module: The module name
        :return: A list of link objects or None
        """
        ...
        if not response or response.status_code in [204, 404]:
            logger.info(f"No links found for {parent_id} in {parent_module}")
            return None
