...
from regscale.core.app.internal.login import verify_token, login
...
class APIHandler(metaclass=Singleton):
    ...
    def _handle_login_on_401(
        ...
        logger.info(f"is token valid: {is_token_valid}")
    ...
    def _make_request(
        ...
        try:
            ...
            logger.info(f"   [API_HANDLER] - Making {method.upper()} request to {url}")
            if method == "get":
