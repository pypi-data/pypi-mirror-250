...
from .sd import SDPipeline

ADAPTER_LIST = [
    must_exists("ip_adapter_full_face", NetworkType.Other, ModelType.Adapter),
    must_exists("ip_adapter_plus_face", NetworkType.Other, ModelType.Adapter),
]
...
def image_adapter_factory(pipe, adapt_model):
    ...
    if adapt_model == "ip_adapter_full_face":
    ...
    elif adapt_model == "ip_adapter_plus_face":
        ...
        return IPAdapterPlus(pipe,
                             ...
                             device="cuda",
                             num_tokens=16)
    elif adapt_model == "None":
        return None
    else:
        raise ValueError(f"adapt model {adapt_model} not found")


class AdapterPipeline(SDPipeline):
    def add_adapt(self, adapt_model):
    ...
    def init(self,
        ...
        from .pipeline.sd import MyselfStableDiffusionPipeline
        self.pipe = MyselfStableDiffusionPipeline.from_pretrained(
            base_model_path,
