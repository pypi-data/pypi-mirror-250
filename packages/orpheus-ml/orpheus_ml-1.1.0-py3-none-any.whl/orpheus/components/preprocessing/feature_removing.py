...
class FeatureRemoving(_ComponentBase):
    ...
    def sequential_order_of_removing(
        ...
        if random_subset < 1.0:
            logger.notice(
                f"Using a random subset of {random_subset*100:.2f}% of the data to remove features..."
            )
    ...
    def remove_features_by_correlation(
        ...
        for threshold in search_space[1:]:
            _X_temp = self._remove_features_by_correlation(
                _X.copy(), corr_with_y, threshold
            )
            if _X_temp is None:
                logger.notice(
                ...
                logger.notice(
                    f"None of the current features changed. Skipping threshold {threshold}..."
                )
                continue
        ...
        result_df = pd.DataFrame(result_dict).T.sort_values(
            by="score", ascending=not self.maximize_scoring
        )
        best_threshold = result_df.index[0]
        self._kept_features_by_correlation = result_df.loc[best_threshold][
            "features"
        ].to_list()
    ...
    def remove_features_by_selection(
        ...
        - increase 'step'
        - increase 'min_features_to_select'
        - decrease 'cv'
        ...
        X = _X.copy()
        logger.notice(
            f"Removing features by Recursive Feature Elimination with n_jobs={self.num_workers}..."
        )
        ...
        self._kept_features_by_selection = X.columns[
            self.feature_selector_.get_support()
        ].to_list()
        ...
        logger.notice(
            f"Removed features: {[ i for i in X_features if i not in self._kept_features_by_selection]}"
        )
        len_cols_new = X.shape[-1]
        logger.notice(
            f"removed {len_cols_old-len_cols_new}/{len_cols_old} features by feature selection."
        )

        # print feature importances:
        importances, features = self._get_feature_importances(X, self.feature_selector_)
        ...
        if plotting:
            self._plot_optimal_num_of_feats(
                min_features_to_select, self.feature_selector_
            )
            self._plot_feature_importances(X, self.feature_selector_)
    ...
    def _remove_features_by_correlation(self, _X, corr_with_y, threshold):
        non_correlated_features_with_y = (
            (corr_with_y <= (1 - threshold)).to_numpy().nonzero()[0]
        )
        ...
        columns_to_remove = np.intersect1d(
            columns_to_remove, non_correlated_features_with_y
        )
    ...
    def _select_estimator_for_removing_features(
        ...
        if estimator is None:
            ...
            if isinstance(estimator, type):
                raise TypeError(
                    f"{estimator} was passed uninstantiated. Please pass instantiated estimator!"
                )
    ...
    def _find_best_random_forest_estimator(
        ...
        logger.notice(
            f"Best found randomforest estimator using RandomizedSearchCV: {estimator}"
        )
        logger.notice(
            f"Best found randomforest estimator score using RandomizedSearchCV: {grid_search.best_score_}"
        )
        logger.notice(
    ...
    def _plot_feature_importances(
        self, importances: List[float], features: List[str]
    ) -> None:
        # Create the bar plot
    ...
    def _get_feature_importances(self, X: pd.DataFrame, feature_selector_: RFECV):
        importances = feature_selector_.ranking_

        # Zip together the feature names and importances
        importances_dict = dict(zip(X.columns, importances))

        # Sort the feature importances by their ranking
        sorted_importances = dict(
            sorted(importances_dict.items(), key=lambda item: item[1])
        )
        ...
        # Convert the importances to percentages
        importances = [i / max(importances) * 100 for i in importances]
        return importances, features

    def _plot_optimal_num_of_feats(
        self, min_features_to_select: int, feature_selector_: RFECV
    ) -> None:
        ...
        plt.plot(
            range(
                ...
                len(test_scores) + min_features_to_select,
            ),
            test_scores,
        )
