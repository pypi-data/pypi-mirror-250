...
from orpheus.experimental.pytorch.utils import pytorch_to_component_adapter

warnings.filterwarnings("ignore")


@pytorch_to_component_adapter
class HyperTuner(_ComponentBase):
    ...
    ) -> Tuple[List[EstimatorErrorInfo | None], List[EstimatorErrorInfo | None]]:
        """
    ...
    def _get_paramgrid_for_R3(
        ...
        for idx, (estimator, score) in enumerate(sorted_scores_R2.items()):
            ...
            estimator_score = {
                p: getattr(estimator, p) for p in best_estimator_default_grid if p in default_ints | default_floats
            } | {"score": score}
