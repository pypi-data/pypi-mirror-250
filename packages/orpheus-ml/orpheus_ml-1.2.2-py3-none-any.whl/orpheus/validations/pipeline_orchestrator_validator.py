...
class PipelineOrchestratorValidator:
    @staticmethod
    def validate_parameters(X, y, test_size: float, validation_size: float, verbose: int):
        ...
        PipelineOrchestratorValidator.validate_train_test_size(test_size, validation_size)
    ...
    @staticmethod
    def validate_train_test_size(test_size, validation_size):
        if not validation_size or not test_size:
            raise ValueError(
                f"validation_size and test_size cannot be None or 0.0. Got validation_size={validation_size} and test_size={test_size}"
            )

        if validation_size < 0 or test_size < 0:
            raise ValueError(
                f"validation_size and test_size cannot be negative. Got validation_size={validation_size} and test_size={test_size}"
            )
