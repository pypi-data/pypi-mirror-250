...
class FeatureRemoving(_ComponentBase):
    ...
    def remove_features_by_selection(
        ...
        if plotting:
            self._plot_optimal_num_of_feats(
                min_features_to_keep, self.feature_selector_
            )
    ...
    def _get_feature_importances(
        self, X: pd.DataFrame, estimator: EstimatorType
    ) -> pd.DataFrame:
        if not hasattr(estimator, "feature_importances_"):
            logger.warning(
                f"Estimator {estimator} has no attribute feature_importances_"
            )
        ...
        feature_importances_df = pd.DataFrame(
            sorted_importances.items(), columns=["Feature", "Importance"]
        )
        feature_importances_df["Importance"] = feature_importances_df["Importance"].map(
            lambda x: f"{x * 100:.2f} %"
        )
        return feature_importances_df

    def _plot_optimal_num_of_feats(
        self, min_features_to_keep: int, feature_selector_: RFECV
    ) -> None:
