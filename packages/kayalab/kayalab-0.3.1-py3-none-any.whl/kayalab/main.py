...
def main() -> None:
    ...
    disable_warnings() # though this is not working when package published to pypi
    # ensure the system is configured
    if not os.path.isfile("config.ini"):
        print("Configuration is required.")
        config.set()
    with warnings.catch_warnings(): # since the disable_warnings() not working, trying if this does the job, 
        # again shouldn't use this as it is not secure
