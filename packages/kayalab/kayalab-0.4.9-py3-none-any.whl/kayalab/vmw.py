...
def create_template(si: vim.ServiceInstance):
    ...
    if vm_folder is None:
        ...
        print(f"standalone esx host, using {vm_folder}")
    ...
    try:
        ...
        if vm is not None:
            ...
            if not add_os_disk_to_vm(
                vm=vm,
                filepath=source_disk_path,
                disk_sizeGB=ez_node["os_disk_size"],
            ):
                fail(f"Failed at adding os disk to {vm.name}")
...
def clone_vm(vm_name: str, vm_ip: str, ez_node: dict, resources: dict):
    ...
    # password hash is generated by openssl passwd -6 command
    # password_hash = "$6$UE9VMyQeaspOt7uF$qQl698Js1RzL.DC.oOYSEHCOwJRXGezjOrk2UW2CI/SW5Vq89Ghyart58TyOQD1zyTvd2eaegS3/FNyuGoJ8q/"
    ...
    print(f"{vm.name} wait for start... ", end="")
...
def new_vm_spec(datastore, ez_node, name):
    ...
    config.annotation = f"Created by ezlab for {ez_node['name']} use"
...
def add_new_disk_to_vm(
    ...
    print(f"{vm.name} add {disk_size}GB disk...")
    print(reconfigure_vm(vm, disk_spec))

    return vm


def add_os_disk_to_vm(vm: vim.VirtualMachine, filepath, disk_sizeGB: int):
    controller = get_scsi_controller(vm)
    ...
    disk_spec.device.backing.fileName = (
        # f"[{datastore.name}] {vm.config.name}/{vm.config.name}.vmdk"
        filepath
    )
...
def delete_templates(si: vim.ServiceInstance):
    ...
    if isinstance(vmfolder, vim.VirtualMachine):
        vms = [vmfolder]
    else:
    ...
    if confirm(f"Are you sure to delete {[vm.name for vm in vms]}"):
        for vm in vms:
...
def delete_vms(si: vim.ServiceInstance):
    ...
    if len(vms) > 0 and confirm(
        f"Are you sure to delete {[vm.name for vm in vms]}"
...
def delete_os_disk_from_vm(vm: vim.VirtualMachine):
    ...
    except pyVmomi.VmomiSupport.vmodl.fault.NotSupported:
        pass
    disk_spec = vim.vm.device.VirtualDeviceSpec()
