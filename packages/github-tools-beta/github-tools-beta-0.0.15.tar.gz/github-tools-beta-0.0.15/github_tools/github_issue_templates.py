...
def main():
    ...
    team_alias = "act"
    title = "🏆 arne - col"
    template_filenames = "a_team.md b_goal.md c_epic.md d_collection.md e_task.md"


    #############################################################################

    logging.info(f"title: {title}")
    logging.info(f"template_filenames: {template_filenames}")
    ...
    if not template and os.path.isdir(templates_dir):
        ...
        logging.info(f"Will use template from repo {templates_repo} - {templates_dir}..")
        issue_template.handle_templates(new=True)
        template = issue_template.get_template(title)
        ...
        logging.warning(f"Template for {title} type issue not found.")
