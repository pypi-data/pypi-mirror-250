...
@sync_safety_context
def check(session, packages, db_mirror=False, cached=0, ignore_vulns=None, ignore_severity_rules=None, proxy=None,
...
def process_fixes_scan(file_to_fix, to_fix_spec, auto_remediation_limit, output, no_output=True, prompt=False):
    ...
    def get_remmediation_from(spec):
        ...
        try:
            ...
            LOG.error(f'Error getting upper remediation version, ignoring', e)

        try:
            lower = Version(spec.remediation.closest_secure.lower) if spec.remediation.closest_secure.lower else None
        except Exception as e:
            LOG.error(f'Error getting lower remediation version, ignoring', e)

        try:
            recommended = Version(spec.remediation.recommended)
        except Exception as e:
            LOG.error(f'Error getting recommended version for remediation, ignoring', e)            
...
@sync_safety_context
def get_licenses(session, db_mirror=False, cached=0, telemetry=True):
...
def get_announcements(session, telemetry=True):
    ...
    method = 'post'
    data = asdict(build_telemetry_data(telemetry=telemetry))
