...
def fail_if_authenticated(ctx, with_msg: str):
    ...
    if info:
        console.print(with_msg.format(email=f"[green]{ctx.obj.auth.email}[/green] {render_email_note(ctx.obj.auth)}"))
        sys.exit(0)

def render_email_note(auth: Auth):
    return "[bold](Verified)[/bold]" if auth.email_verified else "[red](email verification required)[/red]"
