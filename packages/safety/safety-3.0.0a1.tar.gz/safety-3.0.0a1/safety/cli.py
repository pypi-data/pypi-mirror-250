...
@cli.command(cls=SafetyCLILegacyCommand, utility_command=True, deprecated=True, help=CLI_CHECK_COMMAND_HELP)
@proxy_options
...
def check(ctx, db, full_report, stdin, files, cache, ignore, ignore_unpinned_requirements, output, json,
    ...
    try:
        ...
        vulns, db_full = safety.check(ctx.obj.auth.client, packages=packages, db_mirror=db, cached=cache, ignore_vulns=ignore,
