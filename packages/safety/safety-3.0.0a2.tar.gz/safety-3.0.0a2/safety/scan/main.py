...
def download_policy(session: SafetyAuthSession, 
    ...
    if result and "uuid" in result and result["uuid"]:
        ...
        try:
            ...
            LOG.error(f"Failed to parse policy file {uuid}.", e)
            raise SafetyError(f"{err}, details: {e}")
        except ValueError as e:
            LOG.error(f"Wrong YML file for policy file {uuid}.", e)
            raise SafetyError(f"{err}, details: {e}")
...
def load_policy_file(path: Path) -> Optional[PolicyFileModel]:
    ...
    try:
        ...
        LOG.error(f"Failed to parse policy file {path}.", e)
        raise SafetyError(f"{err}, details: {e}")
    except ValueError as e:
        LOG.error(f"Wrong YML file for policy file {path}.", e)
        raise SafetyError(f"{err}, details: {e}")
