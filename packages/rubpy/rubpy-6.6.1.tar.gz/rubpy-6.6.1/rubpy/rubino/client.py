...
class Rubino:
    ...
    def __init__(self, client: 'rubpy.Client') -> None:
        self.client = client
        self.session = client.connection.session
    ...
    async def get_post_by_share_link(self, share_link: str, profile_id: str) -> dict:
    ...
    async def upload_file(self, file: str, profile_id: str, file_type: str) -> dict:
        ...
        async with self.session.post(result.server_url, data=byte_file, headers=headers) as result:
            if result.ok:
