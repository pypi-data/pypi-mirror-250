...
async def main():
    events.start()
    await events.publish("Hello/World", value=True).all()
...
from .weakref_helpers import WeakHandleDictionary, weak_ref
...
class Topic:
    def __init__(self):
        ...
        WeakHandleDictionary is more technically correct as it will properly implicitly unsubscribe from topics when
        the parent object no longer exists.
        Make sure that unsubscribe is called on objects that are dynamically created and deleted otherwise a memory
        leak can occur. Can use the WeakHandleDictionary to check if this is correctly implemented by seeing if the
        object handle is 'dead' in the dictionary when you think that you have removed all references.
        There is a performance penalty to using the WeakHandleDictionary and could be eventually refactored back to
        a normal dictionary if an alternative method to auto unsubscribe is found.
        """
        self.handles_executor = WeakHandleDictionary()
        self.handles_async = WeakHandleDictionary()
        self._waiters = deque()
        self._cancel = False
        self._last = None
    ...
    def unsubscribe(self, handle):
        ...
        to_delete = []
        for _id, value in self.handles_executor.items():
            sub_handle, _ = value
            if sub_handle == handle:
                to_delete.append(_id)
        for _id in to_delete:
            try:
                del self.handles_executor[_id]
            except KeyError:
                continue
        to_delete.clear()
        for _id, value in self.handles_async.items():
            sub_handle, _ = value
            if sub_handle == handle:
                to_delete.append(_id)
        for _id in to_delete:
            try:
                del self.handles_async[_id]
            except KeyError:
                continue
    ...
    *handles: typing.Callable,
    ...
    for handle in handles:
        ...
        if isinstance(parent, int):
            _enabled[parent].append((_form_subscription_topic(topic), weak_ref(handle)))
        elif parent is not None:
            _enabled[id(parent)].append(
                (_form_subscription_topic(topic), weak_ref(handle))
            )
    else:
...
def disable_object(obj_id: int):
    """
    Removes all subscriptions which were enabled on an object
    """
    try:
        subscriptions = _enabled.pop(obj_id)
    except KeyError:
        return
    ...
    def __new__(cls, *args, **kwargs):
        ...
        for attr in dir(inst):
            try:
                ...
                _enabled[id(inst)].append(
                    (handle.topic.format(id=id(inst)), weak_ref(handle))
                )
...
_topic = str
_id = int
_handle = typing.Callable
_enabled: typing.Dict[_id, typing.List[typing.Tuple[_topic, _handle]]] = defaultdict(
    list
)
