...
class Imac2Protocol:
    ...
    def decode_roll_call_single(self, roll_call: AddressMapUint16) -> dict:
        ...
        # Priority for sorting when doing bulk device updates. Smaller is better
        # Potential GG2 CH4 first
        if params["module_type"] == 63 and params.get("version", 0) in [1, 2, 3]:
            params["priority"] = 10
        # Then AIM
        elif params["module_type"] in [48, 49, 50]:
            params["priority"] = 20
        # Then other GG2
        elif params["module_type"] in [61, 62, 63]:
            params["priority"] = 30
        # Everything else
        else:
            params["priority"] = 100
    ...
    async def read_by_serial_number(
        ...
        async with self.block_lock:
            ...
            await self.set_bit(0x409, 3, False)
    ...
    async def write_by_serial_number(
        ...
        async with self.block_lock:
            ...
            await self.set_bit(0x409, 4, False)
