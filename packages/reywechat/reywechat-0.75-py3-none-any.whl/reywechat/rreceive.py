...
class RMessage(object):
    ...
    def is_quote_self(self) -> bool:
        ...
        # Judge.
        if self.is_quote:
            pattern = "<fromusername>(\w+?)</fromusername>"
            quote_user = search(pattern, self.data)
            if quote_user == self.rreceive.rwechat.rclient.login_info["user_id"]:
    ...
    def quote_info(self) -> Dict[
        Literal["text", "quote_id", "quote_type", "quote_user", "quote_user_name", "quote_text"],
        Any
    ]:
        """
        Return information dictionary of quote message.
        ...
        # Extract.
        pattern = "<title>([\s\w]+?)</title>"
        text = search(pattern, self.data)
        pattern = "<fromusername>(\w+?)</fromusername>"
        quote_user = search(pattern, self.data)
        pattern = "<svrid>(\w+?)</svrid>"
        quote_id = search(pattern, self.data)
        pattern = "<content>(\w+?)</content>"
        quote_text = search(pattern, self.data)
        pattern = "<refermsg>\s+<type>(\w+?)</type>"
        quote_type = search(pattern, self.data)
        pattern = "<displayname>(\w+?)</displayname>"
