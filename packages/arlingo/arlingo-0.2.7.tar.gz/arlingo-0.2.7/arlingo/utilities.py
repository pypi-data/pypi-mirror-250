...
class Translator:
    ...
    def __init__(self, source, target):
        self.source = source
        self.target = target
        self.translator = EasyGoogleTranslate(source_language=source, target_language=target, timeout=100)
    
    def translate(self, phrase, get_audio=False):
        self.translated = self.translator.translate(phrase.strip())
        if get_audio==True:
            self.source_tts = self.get_audio(phrase, self.translator.source)
            self.target_tts = self.get_audio(phrase, self.translator.target)
