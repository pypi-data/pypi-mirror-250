...
class Conjugator:
    ...
    def get_conjugation(self, verb, tense='present', lang='fr'):
        ...
        if self.lang in ['fr', 'de']:
            ...
            pronouns = [span.text.strip() for span in self.conjugation_object.find_all('span')][0::2]
            verbs = [span.text.strip() for span in self.conjugation_object.find_all('span')][1::2]
...
class PhraseTranslationWriter:
    ...
    def __init__(self, phrase, translated, audiopath, part_of_speech, gender=None):
        self.phrase = phrase
        self.translated = translated
    ...
    def write(self, type='markdown'):
        if type not in ['markdown', 'telegram']:
            raise ValueError("type must be either 'markdown' or 'telegram'")
        
        output = ''
        if type == 'markdown':
            output += '| ' + self.phrase + ' | ' + self.translated + ' | ' + self.part_of_speech + ' | ' + self.gender + ' | ' + f'<audio controls><source src="{self.audiopath}" type="audio/mpeg"></audio>' + ' |'
            
        elif type == 'telegram':
            output += '| ' + self.phrase + ' | ' + self.translated + ' | ' + self.part_of_speech + ' | ' + self.gender + ' | '
