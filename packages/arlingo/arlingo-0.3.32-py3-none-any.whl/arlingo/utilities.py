...
def get_conjugation_output(verb, output_type='markdown', save_root_path='', github_root_path='', translated=None, lang='fr'):
    ...
    filepaths = []
    root_path = ''
    for pronoun, verb in zip(conjugator.pronouns, conjugator.verbs):
        pronoun = pronoun.split('/')[0] if '/' in pronoun else pronoun
        text_for_speech = pronoun + ' ' + verb
        save_filepath = save_root_path + str(time.time()).replace('.','P') + '-' + text_for_speech.replace(' ','').lower() + '.mp3'
        github_filepath = github_root_path + str(time.time()).replace('.','P') + '-' + text_for_speech.replace(' ','').lower() + '.mp3'
    ...
    return writer.write(output_type)


def get_phrase_translation_output(phrases, source='fr', target='fa', save_root_path='', github_root_path='', output_type='markdown'):
    ...
    for phrase in phrases:
        save_filepath = save_root_path + str(time.time()).replace('.','P') + '-' + phrase.replace(' ','').lower() + '.mp3'
        github_filepath = github_root_path + str(time.time()).replace('.','P') + '-' + phrase.replace(' ','').lower() + '.mp3'
