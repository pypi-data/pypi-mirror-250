...
def get_conjugation_output(verb, output_type='markdown', root_path='', translated=None, lang='fr'):
    ...
    for pronoun, verb in zip(conjugator.pronouns, conjugator.verbs):
        ...
        filepath = root_path + str(time.time()).replace('.','P') + '-' + text_for_speech.replace(' ','').lower() + '.mp3'
        filepaths.append(filepath)
        tr = Translator(source=lang, target='fa')
        tr.translate(text_for_speech, get_audio='source', filepath=filepath)
...
def get_phrase_translation_output(phrases, source='fr', target='fa', root_path='', output_type='markdown'):
    ...
    for phrase in phrases:
        filepath = root_path + str(time.time()).replace('.','P') + '-' + phrase.replace(' ','').lower() + '.mp3'
        translator.translate(phrase, get_audio='source', filepath=filepath)
        filepaths.append(filepath)
