...
class PhraseTranslationWriter:
    ...
    def write(self, type='markdown'):
        ...
        output = ''
        if type == 'markdown':
            for phrase, translated, part_of_speech, gender, file_path in zip(self.phrases, self.translations, self.part_of_speechs, self.genders, self.audiopaths):
                output += '| ' + phrase + ' | ' + translated + ' | ' + part_of_speech + ' | ' + gender + ' | ' + f'<audio controls><source src="{audiopath}" type="audio/mpeg"></audio>' + ' |'
