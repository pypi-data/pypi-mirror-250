...
class PartectorBluePrint(Thread, PartectorDefaults, ABC):
    ...
    def _checker_thread(self) -> None:
        while not self.thread_event.wait(0.5):
            if time.time() - self._time_last_message_received > 10:
                try:
                    logger.info(f"SN{self._serial_number}: Checking device connection...")
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"SN{self._serial_number} Exception occured during threaded serial reading: {e}"
            )
    ...
    def _serial_wrapper(self, func: Callable[[], Any]) -> Optional[Any]:
        ...
        for _ in range(self.SERIAL_RETRIES):
            try:
                ...
                logger.error(f"SN{self._serial_number} Exception in _serial_wrapper: {e}")
                excep = f"SN{self._serial_number} Exception occured during user function call: {e}"
