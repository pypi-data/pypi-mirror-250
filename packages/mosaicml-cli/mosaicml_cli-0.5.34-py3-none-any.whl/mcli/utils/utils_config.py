...
def experiment_trackers_to_experiment_tracker(experiment_trackers: List[Dict[str, Any]]) -> ExperimentTrackerConfig:
    """Converts deprecated experiment_trackers to experiment_tracker"""
    out = {}
    for tracker in experiment_trackers:
        if 'integration_type' not in tracker:
            raise MCLIConfigError("Missing integration_type in experiment_trackers")
        integration_type = tracker['integration_type']
        if integration_type == "wandb":
            out[integration_type] = strip_nones({"project": tracker.get("project"), "entity": tracker.get("entity")})
        elif integration_type == "mlflow":
            out[integration_type] = strip_nones({
                "experiment_path": tracker.get("experiment_name"),
                "tracking_uri": tracker.get("tracking_uri"),
                "model_registry_path": tracker.get("model_registry_prefix")
            })
        else:
            raise MCLIConfigError(f"Unknown integration_type {integration_type} in experiment_trackers")
    return ExperimentTrackerConfig(**out)
