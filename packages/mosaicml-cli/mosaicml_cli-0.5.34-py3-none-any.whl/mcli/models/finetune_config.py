...
from mcli.utils.utils_config import (BaseSubmissionConfig, ExperimentTrackerConfig, ExperimentTrackerTranslation,
                                     experiment_trackers_to_experiment_tracker, strip_nones)
...
class FinetuneConfig(BaseSubmissionConfig):
    ...
    Args:
        ...
        training_duration (str): Composer variable for the total number of epochs or tokens to train on
            (e.g. 2ep or 10000tok)
        experiment_trackers (Optional[List[Dict]]): DEPRECATED, List of experiment trackers to use
    ...
    _finetune_properties = {
        ...
        'training_duration', 'experiment_trackers', 'experiment_tracker', 'learning_rate', 'context_length',
        'task_type', 'disable_credentials_check'
    }
    ...
    def __post_init__(self):
        if self.experiment_trackers and self.experiment_tracker:
            raise MCLIConfigError('Cannot specify both experiment_trackers and experiment_tracker. '
                                  'Please use the latest field, experiment_tracker.')
        if self.experiment_trackers:
            warnings.warn("experiment_trackers is deprecated. Please use experiment_tracker instead.")
            self.experiment_tracker = experiment_trackers_to_experiment_tracker(self.experiment_trackers)
            self.experiment_trackers = None
