...
def individual_params() -> dict:
    return {"lower_bound": -2, "upper_bound": 2, "number_of_genes": NUMBER_OF_GENES}
...
def test_one_iteration(population_params: dict, individual_params: dict):
    def fitness(x: int | float, y: int | float) -> float:
        return x * (x - 1) * np.cos(2 * x - 1) * np.sin(2 * x - 1) * (y - 2)

    evo = Evolution(population_params, individual_params, fitness)
...
def test_many_iterations(population_params: dict, individual_params: dict):
    evo = Evolution(population_params, individual_params, fitness)
