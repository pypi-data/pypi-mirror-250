...
class ActivityAPIClient(object):
    ...
    def retrieve_process_by_process_type_and_external_key(self, process_type_slug, external_key):
        """
        Retrieve details of a single process by its process type slug and external key.
        Return response's content.
        Raise ClientError when request is failed
        :param process_type_slug: string slug based on process type name to identify the process (example: PROCESS_TYPE_SLUG).
        :param external_key: string to identify process that saved in Ronan
        :return: dict
        """
        uri = '/processes-by-type/%s/external-keys/%s' % (process_type_slug, external_key)
        return self.api.get(uri)
