...
def get_image(ra,dec,size=30,band='g'):
	...
	# above URL returns another set of URLs for the actual images, which are used in a request below
	try:
	...
	# read fits data, set nan values to zero and apply a contrast filter
	try:
...
'''
Returns just wcs and mjd of image
'''
...
def input_management(ra,dec,size=30,band='g'):
	...
	# sizeAS = size of image in arcseconds, size = size of image in pixels
	sizeAS=size
	size=size/3600
	...
	# Format bands input into format accepted by SkyMapper
	band=list(band)
	temp_string=''
	for i in range(0,len(band)):
		temp_string+=(band[i]+',')
...
'''
Set up axes and place image on them
'''
...
'''
Gets SkyMapper data by coordinates (pos)
'''
def SkyMapperQueryCoords(ObjRa,ObjDec,radius=3):
	radius=radius/3600

	# setup URL and send request
...
'''
Gets SkyMapper photometry by coordinates (pos)
'''
