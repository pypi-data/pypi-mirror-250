...
def get_image(ra,dec,size=30,band='g'):
	...
	# Read and format image data, sets nan values to zero and applies a contrast setting for readability
	try:
...
'''
Get PanSTARRS data by coordinates (pos)
'''
def PanSTARRSQueryCoords(ObjRa,ObjDec,radius=3):
	radius=radius/3600

	#Had to use JSON format to get this to work, the CSV format used by PanSTARRS is not easily formattable by pandas for whatever reason
...
'''
Get PanSTARRS photometry by coordinates (pos)
'''
