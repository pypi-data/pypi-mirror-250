...
class PYPP():
    ...
    async def 加载浏览器(self, user_agent=None, opthon=None):
        ...
        if not opthon:
            proxy_url= f'http://{self.host}:{self.host}' if (self.host and self.port) else ""
            opthon = {
    ...
    async def 点击新窗口打开(self,btn,html=True):
        while True:
            await btn.click()
            pages = await self.brower.pages()
            if len(pages) >= 3:
                ...
                if html:
                    await new_page.waitForNavigation(options=self.option_networkidle0)
