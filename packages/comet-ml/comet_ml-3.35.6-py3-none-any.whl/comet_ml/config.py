...
def init(directory=None, force=False, **settings):
    ...
    * timeout_upload
    ...
    ```
    """
    _init(directory=directory, settings=settings, force=force)


def _init_get_api_key(prompt_user, settings, config, force=False):
    # type: (bool, Optional[Dict[str, Any]], Config, Optional[bool]) -> Tuple[Optional[str], bool]
...
def _init(directory=None, should_prompt_user=None, settings=None, force=False):
    # type: (Optional[str], Optional[bool], Optional[Dict[str, Any]], Optional[bool]) -> None
    ...
    api_key, need_to_save = _init_get_api_key(
        should_prompt_user, settings, config, force
    )
...
def init_onprem(force=False):
    ...
    try:
        api_key, need_to_save = _init_get_api_key(
            prompt_user=True, settings={}, config=config, force=force
        )
