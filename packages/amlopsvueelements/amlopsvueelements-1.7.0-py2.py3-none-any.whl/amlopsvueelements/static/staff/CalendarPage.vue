...
import { useEntryTypesById } from '@/composables/useEntryTypesById'
...
const entryTypesById = useEntryTypesById(entryTypes)
...
const calendarOptions = reactive<CalendarOptions>({
  ...
  events: (info, successCallback, failureCallback) => {
    ...
    eventService
      .fetchEvents(info.start, info.end, selectedTeamNames, entryTypesById.value)
...
watch([selectedTeams, entryTypes, calendar], () => {
          ...
          <div v-if="arg.view.type !== 'resourceTimelineMonth'">
            {{
              moment(arg.event.start)
                .tz(selectedTimezone.value === 'local' ? moment.tz.guess() : selectedTimezone.value)
                .format('HH:mm')
            }}
            -
            {{
