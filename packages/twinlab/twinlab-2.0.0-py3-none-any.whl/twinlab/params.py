...
class ModelSelectionParams:
    ...
    - `base_kernels`: `str`|`Set[str]`, `Optional`. Specifies the list of kernels to use for Compositional Kernel Search. Can be `"all"`, `"restricted"` or `Set[str]` object. The set of available kernels are ["LIN", "M12", "M32", "M52", "PER", "RBF", "RQF"]. Default is `"restricted"` and uses ["LIN", "M32", "M52", "PER", "RBF"].
...
class CalibrateParams:
    ...
    - `y_std_model`: `bool`|`pd.DataFrame`, `Optional`. Whether to include model noise covariance in the likelihood. If a `pandas.DataFrame` is supplied, it must contain the same columns as `y`.
