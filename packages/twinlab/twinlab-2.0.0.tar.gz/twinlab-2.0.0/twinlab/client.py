...
def upload_dataset(
    ...
    - `filepath_or_df`: `str`|`pandas.DataFrame`. Location of csv dataset on local machine or `pandas.DataFrame`.
    ...
    **NOTE:** Evaluation data must be a .csv file, or a `pandas.DataFrame` that is interpretable as a .csv.
    ...
    if sync:
        ...
        response = api.predict_request_model(
            campaign_id, data_csv, processor=processor, verbose=debug
        )
        ...
        while "dataframe" not in response:
            response = api.predict_response_model(
                campaign_id, process_id, verbose=debug
            )
            time.sleep(ping_time)
