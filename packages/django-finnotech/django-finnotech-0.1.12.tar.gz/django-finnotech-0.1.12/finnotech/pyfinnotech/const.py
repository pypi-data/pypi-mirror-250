...
NATIONAL_CODE_MOBILE_VERIFICATION_CACHE_KEY = (
    ":national-code-mobile-verification:%(national_code)s:%(mobile)s:"
)
POSTAL_CODE_INQUIRY_CACHE_KEY = ":postal-code-inquiry:%(postal_code)s:"
CLIENT_IDENTIFICATION_INQUIRY_CACHE_KEY = (
    ":client-identification-inquiry:%(national_code)s:%(birth_date)s:"
)
IBAN_INQUIRY_CACHE_KEY = ":iban-inquiry:%(iban)s:"
DEPOSIT_TO_IBAN_CACHE_KEY = ":deposit-to-iban:%(deposit)s:%(bank_code)s:"
BACK_CHEQUES_INQUIRY_CACHE_KEY = ":back-cheques-inquiry:%(national_code)s:"
