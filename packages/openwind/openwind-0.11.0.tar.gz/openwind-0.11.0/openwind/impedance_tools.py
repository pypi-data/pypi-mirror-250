...
def match_freqs_with_notes(f_, concert_pitch_A=440, transposition = 0, display=False, simple_name=True):
    ...
    for (i,f) in enumerate(f_):
        ...
        # f_recons = concert_pitch_A*2**((12*octave+note_nb+cent/100)/12) # checking just to be sure with the formula: ok
        if(note_nb >3): # only for display
            # english notation changes at C
            octave = octave + 1
        octave_disp = octave + 4 # notation A4 for pitch A
        note_name = notes[note_nb] + str(int(octave_disp))
#            print(f"octave is {octave} and note is {note_nb}th")
        if display:
            print(f"{i+1} - frequency {f:.2f} Hz : {cent:3.2f} cents away from note {note_name} {nb_semitones_transp:+d} semitones ({fn:.2f} Hz) ")#
        # ie {f_recons:.2f} Hz") # checking just to be sure with the formula: ok
