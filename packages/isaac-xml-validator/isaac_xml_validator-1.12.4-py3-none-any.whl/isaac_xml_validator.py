...
parser.add_argument("-w", "--warningaserror", help="Treat warnings as error", type=bool)
...
if args.warningaserror is not None:
    warning_as_error = args.warningaserror
...
def main():
    ...
    global root_folder, expected_error_count, recursive, error_count, warning_as_error
    total_error_count = 0
...
def read_github_env_vars():
    global root_folder, expected_error_count, recursive, warning_as_error
    ...
    if "INPUT_WARNINGASERROR" in os.environ:
        warning_as_error = bool(os.environ["INPUT_WARNINGASERROR"])
    printf("\tTreat warning as error: ", warning_as_error)
