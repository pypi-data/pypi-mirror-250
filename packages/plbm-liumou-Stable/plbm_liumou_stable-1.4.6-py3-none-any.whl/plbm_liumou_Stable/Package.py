...
class NewPackage:
	def __init__(self, password, logs=True, file=None, package=None):
		...
		self.manger = NewApt(password=password, log=logs)
	...
	def init(self):
		...
		if self.install_tools.lower() == 'yum':
			self.manger = NewYum(password=self.password, log_file=self.file, log=self.logs)
	...
	def upgrade(self, update_index=True, dist=True):
		...
		if self.install_tools.lower() == 'apt':
			self.manger = NewApt(password=self.password, log=self.logs)
			if dist:
				return self.manger.upgrade_dist(update=update_index)
			else:
				return self.manger.upgrade(update=update_index)
		else:
			self.manger = NewYum(password=self.password, log=self.logs)
