...
class NewNmcli(object):
	...
	def get_dev_list_connected(self):
		...
		"""
		c = str("""nmcli device status  | grep ethernet | grep -v '\-\-' | awk '{print $1}'""")
		get = self.cmd.getout(c)
		if self.cmd.code == 0:
			self.deviceConnectedList = str(get).split("\n")
			if len(self.deviceConnectedList) >= 1:
				self.logger.info("网卡信息获取成功")
				return True
		print(c)
		self.logger.error("无法获取网卡设备信息")
	...
	def get_eth_info(self, eth):
		...
		:return:
		"""
		c = str("""nmcli device show %s  | grep ADDRESS | sed -n 1p | awk '""" % eth) + "'{print $2}'"
		info = {}
