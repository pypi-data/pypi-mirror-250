...
def handle_mrz_extraction(client, image_data, back_id_text, back_data):
    ...
    if not back_data_new.get('id_number') or back_data_new.get('card_number'):
        ...
        mrz1_data_pattern = r'IDI[\S]{2}([\S]{2}\d{7}|[A-Za-z]\d{8}).*?(\d{13})'
...
#         issuing_authority = ''
    ...
    if empty_string_keys or len(back_data_dict['nationality'])>3 or len(back_data_dict['id_number'])<12:
        print("handling")
        back_res = handle_mrz_extraction(client, image_data, back_id_text, back_data)
