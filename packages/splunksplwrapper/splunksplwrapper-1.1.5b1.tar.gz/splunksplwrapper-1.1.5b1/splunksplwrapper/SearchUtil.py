...
class SearchUtil:
    ...
    def checkQueryContainsRegex(
        ...
        while tryNum <= retries:
            job = self.jobs.create(
                query,
                auto_finalize_ec=number_results,
                max_time=max_time,
                output_mode="json",
            )
            job.wait()
            results = job.get_results(output_mode="json")
    ...
    def checkQueryCount(self, query, targetCount, interval=15, retries=4, max_time=120):
        ...
        tryNum = 0
        while tryNum <= retries:
            job = self.jobs.create(query, max_time=max_time, output_mode="json")
            job.wait(max_time)
    ...
    def checkQueryCountIsGreaterThanZero(
        ...
        while tryNum <= retries:
            job = self.jobs.create(
                query, auto_finalize_ec=200, max_time=max_time, output_mode="json"
            )
            job.wait(max_time)
            result_count = len(job.get_results(output_mode="json"))
            if result_count > 0:
    ...
    def deleteEventsFromIndex(self, index_name="*", max_wait_time=120):
        ...
        self.logger.debug("query is %s", query)
        try:
            job = self.jobs.create(query, output_mode="json")
    ...
    def checkQueryCountIsZero(self, query, max_time=120):
        ...
        if result_count == 0:
            ...
            self.logger.debug("Count of results is > 0, it is:%d", result_count)
            return False, job.get_results(output_mode="json")
    ...
    def get_search_results(self, query, max_time=120):
        ...
        try:
            job = self.jobs.create(
                query, auto_finalize_ec=120, max_time=max_time, output_mode="json"
            )
            job.wait(max_time)
            return job.get_results(output_mode="json")
        except Exception as e:
    ...
    def checkQueryFields(
        ...
        while tryNum <= retries and not status:
            ...
            job = self.jobs.create(
                query, auto_finalize_ec=120, max_time=120, output_mode="json"
            )
            job.wait(TIMEOUT)
            results = job.get_results(output_mode="json")
            messages = job.get_messages()

            if len(job.get_results(output_mode="json")) > 0:
    ...
    def checkExactQueryContent(
        ...
        while tryNum <= retries and not status:
            job = self.jobs.create(
                query, auto_finalize_ec=10, max_time=60, output_mode="json"
            )
            job.wait(TIMEOUT)
            results = job.get_results(output_mode="json")
            messages = job.get_messages()
    ...
    def checkQueryErrorMessage(
        ...
        """
        status = False
        job = self.jobs.create(query, max_time=60, output_mode="json")
        job.wait(TIMEOUT)
        messages = job.get_messages()
        if len(messages) > 0:
    ...
    def getRealtimeNotableSearchResults(
        ...
        while tryNum <= retries:
            ...
            job = self.jobs.create(searchQuery, max_time=60, output_mode="json")
            job.wait(TIMEOUT)
            searchResults = job.get_results(output_mode="json")
    ...
    def getFieldValuesList(self, query, interval=15, retries=4):
        ...
        while tryNum <= retries and not status:
            job = self.jobs.create(query, max_time=60, output_mode="json")
            job.wait(240)
            results = job.get_results(offset=0, count=4000, output_mode="json")
