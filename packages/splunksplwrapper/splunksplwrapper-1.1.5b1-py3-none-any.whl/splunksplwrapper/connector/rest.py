...
class RESTConnector(Connector):
    """
    This represents workaround to access REST through HTTP client library httplib2

    Associated with each object is a C{http} object from the httplib which in
    turn contains connection info, namespace and auth.

    When a connector is logged in a session key is generated and will be kept
    ...
    get appended with the 'Authorization' key when session key is used
    ...
    def login(self):
        ...
        Just hits the auth endpoint and retrieves and sets the session key.
