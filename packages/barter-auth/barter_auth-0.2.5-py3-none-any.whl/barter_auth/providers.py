...
class RedisModelClient(RedisClient):
    ...
    def is_data_exist(self, uuid: str, prefix: bool = True) -> str | None:
        return super(RedisModelClient, self).is_token_exist(uuid, prefix=prefix)

    def get_model(self, uuid: str, prefix: bool = True):
        data_json = self.is_data_exist(uuid, prefix=prefix)
        if data_json:
            return self.model.parse_raw(data_json)
        return self.non_model()

    def add_model(self, uuid: str, json_data: str = None, ttl: int = None, prefix: bool = True):
        return super().add_token(uuid, json_data=json_data, ttl=ttl, prefix=prefix)
...
class RedisProfileClient(RedisModelClient):
...
class RedisAdvertiserClient(RedisModelClient):
    _prefix = app_settings.ADVERTISER_PREFIX
    _ttl = None
    _db = app_settings.DB_ADV_BLOGERS
    model = Advertiser
    non_model = Anonymous


class RedisInstagramClient(RedisModelClient):
    _prefix = app_settings.INSTAGRAM_PREFIX
    _ttl = None
    _db = app_settings.DB_ADV_BLOGERS
    model = Instagram
    non_model = Anonymous
