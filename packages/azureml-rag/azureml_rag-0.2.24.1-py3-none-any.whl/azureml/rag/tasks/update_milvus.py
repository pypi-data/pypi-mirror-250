...
from azureml.rag.utils.connections import connection_to_credential, get_connection_credential
...
def create_index_from_raw_embeddings(
    ...
    with track_activity(
        ...
        if len(deleted_ids) > 0:
            logger.info(f"Deleting {len(deleted_ids)} documents (ie, records) from Milvus")
            start_time = time.time()
            results = client.delete(collection_name=collection_name, pks=deleted_ids)
            if len(results) < len(deleted_ids):
                logger.error('Failed to delete documents')
                raise RuntimeError('Failed to delete documents')
            duration = time.time() - start_time
            logger.info(f"Deleted {len(deleted_ids)} documents in Milvus in {duration:.4f} seconds")
        ...
        batch = []
        syncing_index = milvus_config.get("sync_index", milvus_config.get("full_sync", False))
        ...
        for doc_id, emb_doc in emb._document_embeddings.items():
            ...
            milvus_doc = {
                ...
                milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["embedding"]: emb_doc.get_embeddings(),
                milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["content"]: emb_doc.get_data()
            }
            if "url" in milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]:
                milvus_doc[milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["url"]] = doc_source.get("url", "")
            if "filename" in milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]:
                milvus_doc[milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["filename"]] = doc_source.get("filename", "")
            if "title" in milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]:
                milvus_doc[milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["title"]] = doc_source.get(
                    "title", emb_doc.metadata.get("title", "")
                )
            if "metadata" in milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]:
                milvus_doc[milvus_config[MLIndex.INDEX_FIELD_MAPPING_KEY]["metadata"]] = json.dumps(emb_doc.metadata)
            batch.append(milvus_doc)
            if len(batch) % batch_size == 0:
                logger.info(f"Sending {len(batch)} documents to Milvus")
                start_time = time.time()
                results = client.insert(collection_name=collection_name, data=batch)
                client.flush(collection_name=collection_name)
                if len(results) < len(batch):
                    logger.error('Failed to upload documents')
                    raise RuntimeError('Failed to upload documents')
                duration = time.time() - start_time
                logger.info(f"Uploaded {len(batch)} documents to Milvus in {duration:.4f} seconds")
                batch = []
