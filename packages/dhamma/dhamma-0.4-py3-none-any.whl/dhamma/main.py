...
def export(directory: str, format: str):
    if format not in ["pdf", "epub"]:
        print("format must be 'pdf' or 'epub'")
        exit(1)
...
"""
    ...
    output_file = os.path.join(directory, f"{retreat_title}.{format}")
...
def main():
    ...
    make_parser.add_argument(
        "format", type=str, help="The format for export", choices=["pdf", "epub"]
    )
    make_parser.add_argument(
        "directory",
        type=str,
