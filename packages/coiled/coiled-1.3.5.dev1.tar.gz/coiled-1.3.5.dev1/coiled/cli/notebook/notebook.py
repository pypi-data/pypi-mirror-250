...
def connect_mutagen_sync(cloud, cluster_id, console, debug=False) -> Tuple[bool, str]:
...
def start_standalone_sync(name: str, debug: bool):
    mutagen_path = get_mutagen_path()
    if not mutagen_path:
        return
    ...
    with coiled.Cloud() as cloud:
        ...
        _, error = connect_mutagen_sync(cloud=cloud, cluster_id=cluster_id, console=Console(), debug=debug)
        if error:
            print(error)
