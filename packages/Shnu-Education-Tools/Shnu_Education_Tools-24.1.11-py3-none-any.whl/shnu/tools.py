...
class Assistant:
    ...
    def __init__(self,name_list='人工智能.xlsx', homework_dir ='./期末试卷',sorted=False):
        '''
    ...
    def homework_check(self):
        ...
        if self.sorted:
            sorted_homework_dir = homework_dir + '_排序'
            data['学号-姓名'] = data.index.map(lambda x: Path(
                sorted_homework_dir) / f"{x + 1:02d}-{(df['学号'].str.strip() + '-' + df['姓名'].str.strip())[x]}")
