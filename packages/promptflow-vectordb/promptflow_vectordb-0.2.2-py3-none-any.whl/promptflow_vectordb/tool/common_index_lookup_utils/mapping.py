...
def _get_index_config(
    ...
    if index_type == IndexTypes.AzureCognitiveSearch:
        ...
        for connection in connections:
            ...
            raise ValueError(
                f'Unable to find workspace connection "{acs_index_connection}" in {context.workspace_name}.')
    ...
    if index_type == IndexTypes.Pinecone:
        ...
        for connection in connections:
            ...
            raise ValueError(
                f'Unable to find workspace connection "{pinecone_index_connection}" in {context.workspace_name}.')
        return {
            'kind': 'pinecone',
            'connection': {
                'id': selected_connection.id,
...
def _get_embeddings_config(
    ...
    if embedding_type == EmbeddingTypes.NoEmbedding:
        return {
            'kind': 'none',
            'schema_version': '2',
        }
    ...
    if embedding_type == EmbeddingTypes.AzureOpenAI:
        ...
        for connection in connections:
            ...
            raise ValueError(
                f'Unable to find workspace connection "{aoai_embedding_connection}" in {context.workspace_name}.')
