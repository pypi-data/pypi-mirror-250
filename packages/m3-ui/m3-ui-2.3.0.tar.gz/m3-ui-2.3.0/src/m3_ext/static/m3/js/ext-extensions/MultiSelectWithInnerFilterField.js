...
Ext.m3.MultiSelectWithInnerFilterField = Ext.extend(Ext.m3.MultiSelectField, {
     ...
     */
    isOptAllChecked: function(checkedItems) {
        var allChecked = false;

        for (var i = 0; i < checkedItems.length; i++) {
            if (checkedItems[i].data[this.valueField] === this.optAllId) {
                allChecked = true;
                break;
            }
        }
    ...
    initButtons: function() {
        ...
        var filterOkBtn = new Ext.Button({
            text: 'ОК',
            enableToggle: false,
            allowDepress: false,
            pressed: false,
        ...
        filterOkBtn.getEl().select('button').elements[0].style.width = '50px';
        filterOkBtn.getEl().on('click', this.okBtnHandler, this);

        var filterCloseBtn = new Ext.Button({
            text: 'Закрыть',
            enableToggle: false,
            allowDepress: false,
            pressed: false,
        ...
        filterCloseBtn.getEl().select('button').elements[0].style.width = '70px';
        filterCloseBtn.getEl().on('click', this.closeBtnHandler, this);
    },
    /**
    ...
    filterSelect : function(record, checkedIndex, store, checkedItems) {
        if (record.data.id === this.optAllId) {
            var allChecked = this.isOptAllChecked(checkedItems);
