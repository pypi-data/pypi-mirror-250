    ...
    def __call__(self, assay: xr.Dataset) -> xr.Dataset:
        ...
        for t in assay.time:
            for search_channel in self.search_channels:
                ...
                u = np.cos(theta)[:, :, np.newaxis]
                v = np.sin(theta)[:, :, np.newaxis]
                idx = np.indices((3 * self.max_bead_radius, 3 * self.max_bead_radius))
                r = np.arange(self.min_bead_radius, self.max_bead_radius + 1)
                centers = []
                max_mask = np.zeros_like(labels).astype(np.int16)
                for i in range(1, l.max() + 1):
                    acc = np.zeros(
                        [
                            3 * self.max_bead_radius,
                            3 * self.max_bead_radius,
                            self.max_bead_radius - self.min_bead_radius,
                        ]
                    )
                    if not is_fg[i]:
                        continue
                    ...
                    sub_grad = grad[top:bottom, left:right]
                    sub_theta = theta[top:bottom, left:right]
                    sub_u = u[top:bottom, left:right]
                    sub_v = v[top:bottom, left:right]
                    sub_l = l[top:bottom, left:right]
                    print(r.shape, v.shape)
                    row = np.round(idx[0][:, :, np.newaxis] + r * sub_v).astype(int)
                    col = np.round(idx[1][:, :, np.newaxis] + r * sub_u).astype(int)
                    acc = acc_func(row, col, sub_grad, r)
                    row = np.round(idx[0][:, :, np.newaxis] - r * sub_v).astype(int)
                    col = np.round(idx[1][:, :, np.newaxis] - r * sub_u).astype(int)
                    acc += acc_func(row, col, sub_grad, r)
                    i, j, k = np.unravel_index(
                        np.argmax(acc * (sub_l[:, :, np.newaxis] == i)), shape=acc.shape
                    )
                    centers.append(np.array([i + top, j + left]))
                    cv.circle(
                        max_mask, (centers[-1][1], centers[-1][0]), k + self.min_bead_radius, 1, -1
                    )
                ...
                bg_area[l == 0] = 1
                water = np.zeros_like(l, dtype=np.int32)
                water[l == 0] = 0
                ...
                dists = []
                min_dists = []
                idxs = np.indices((dist.shape))
                for i in range(1, l.max() + 1):
                    if is_fg[i]:
                        m = np.min(dist2 * (l == i) + (l != i) * 10000)
                        m_idx = np.unravel_index(
                            np.argmin(dist * (l == i) + (l != i) * 10000, axis=None), shape=l.shape
                        )
                        cond = (
                            np.sqrt((idxs[0] - m_idx[0]) ** 2 + (idxs[1] - m_idx[1]) ** 2) + m
                            >= dist
                        )
                        c = np.unravel_index(
                            np.argmax(dist * (l == i) * cond, axis=None), shape=l.shape
                        )
                        d = dist[c[0], c[1]]
                        if d > self.max_bead_radius:
                            continue
                        centers.append(np.array(c))
                        dists.append(d)
                        min_dists.append(np.min(dist * (l == i) + (l != i) * 10000))
                import plotly.express as px

                contours = cv.findContours(bg_area, cv.RETR_LIST, cv.CHAIN_APPROX_NONE)[0]
                cv.drawContours(image, contours, -1, 0, 3)
                # contour_tree = scipy.spatial.KDTree(contours)
                center_tree = scipy.spatial.KDTree(centers)
                pairs = center_tree.query_pairs(2 * self.max_bead_radius)
                exclude = set()
                for c0, c1 in pairs:
                    if np.linalg.norm(centers[c0] - centers[c1]) >= dists[c0] + dists[c1]:
                        continue
                    if min_dists[c0] < min_dists[c1]:
                        exclude.add(c1)
                    else:
                        exclude.add(c0)
                centers = np.array(centers)
                print(exclude)
                idxs = [i for i in range(len(centers)) if i not in exclude]
                print(idxs)
                centers = centers[idxs]
                plt.imshow(bg_area)
                # plt.imshow(image)
                plt.scatter(centers[:, 1], centers[:, 0], s=1.0, color="r")
                for k, c in enumerate(centers):
                    if True:  # k not in exclude:
                        print(dists[k])
                        cv.circle(max_mask, (c[1], c[0]), int(np.floor(dists[k])), k, -1)
                print(max_mask)
                # plt.imshow(bg_area)
                d = np.zeros_like(l)
                ...
                water = random_walker(image, water, mode="bf")
                plt.figure(figsize=(20, 20))
                plt.imshow(image, alpha=1.0, cmap="magma")
                plt.imshow(water, alpha=0.9)
