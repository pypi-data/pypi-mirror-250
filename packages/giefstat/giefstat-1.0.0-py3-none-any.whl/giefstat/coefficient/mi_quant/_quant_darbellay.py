...
class Cell(object):
    ...
    def def_cell_bounds_cal_area(self, bounds: List[tuple]):
        ...
        Params:
        -------
        bounds: 边界值list, 如[(x_min, x_max), (y_min, y_max)]
        """
...
def recursively_partition(cell: Cell, N_total: int, min_samples_split: int = 30, 
                          p_eps: float = 1e-3) -> List[Cell]:
    """
    对一个cell进行递归离散化
    ...
    -------
    cell: 初始cell
    p_eps: 子cell概率与父cell相对偏差阈值, 如果所有都小于该值则终止离散化, defaults to 1e-3
    """
