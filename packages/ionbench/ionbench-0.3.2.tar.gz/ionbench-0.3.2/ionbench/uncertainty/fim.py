...
def secondDeriv(bm, i, j, step=1e-4, buffer=1e-4):
    ...
    if i == j:
        ...
        h = step
        x = bm.input_parameter_space(bm._trueParams)[i]
        y = bm.input_parameter_space(bm._trueParams)[j]
        centre = f(x, y)
        up_up = f(x + x * h, y + y * h)
...
def run(bm, sigma=1, preoptimise=True, ftol=3e-6, step=1e-4, buffer=1e-4):
    ...
    if preoptimise:
        out = ionbench.optimisers.scipy_optimisers.nelderMead_scipy.run(bm, x0=bm.input_parameter_space(bm._trueParams), ftol=ftol)
        bm._trueParams = bm.original_parameter_space(out)
        print('Identified MLE at:')
        print(bm._trueParams)
