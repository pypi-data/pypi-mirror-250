...
def check_mimetype(file: str):
    link = readlink(file)
    if link == "":
        cprint("yellow", f"WARNING: Could not get link to file: '{file}'")
        return ""
    cmd = shlex.split(f"file --mime-type \"{link}\" -bL")
    result = return_cmd(cmd)
    return result["stdout"].strip()


def check_zip_mimetype(path: pathlib.Path, zip_cmd: str):
    process = subprocess.Popen(["printf", ""])
    recheck_flag = False
    for file_path in path.glob("*.zip"):
        if check_mimetype(file_path.as_posix()) == "application/zip":
            continue
        cprint("yellow", f"Remaking broken zip: {file_path}")
        if not recheck_flag:
            recheck_flag = True
        file_path.unlink()
        dir, chapter = file_path.as_posix().replace(".zip", "").rsplit("/", 1)
        if not dir.startswith("/"):
            cprint("red", f"ERROR: cannot fix zip -- '{file_path}'")
        process = subprocess.Popen([zip_cmd, path.as_posix(), dir, chapter])
    process.wait()
    return recheck_flag
...
def readlink(file: str):
    cmd = shlex.split(f"readlink -f {file}")
    result = return_cmd(cmd)
    return result["stdout"].strip()
