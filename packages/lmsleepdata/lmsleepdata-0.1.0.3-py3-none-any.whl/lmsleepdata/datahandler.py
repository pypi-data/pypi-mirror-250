...
class DataHandler(object):
    ...
    def load_data_file(self, data_name, eeg_path, acc_path, ble_path, sti_path, person_info=None):
        ...
        if self.eeg is not None and self.raw_eeg is not None:
            self.eeg = (self.eeg - 32767) / 65536 * 2.5 * 1000 * 1000 / 100
            # self.raw_eeg = self.raw_eeg[0]
            self.eeg = np.squeeze(self.eeg)
            eeg_sec = self.eeg.shape[1] // self.sf_eeg
        ...
        if eeg_sec != -1 and acc_sec != -1:
            self.logger.info(f"load_data_file eeg is {eeg_sec} seconds and acc is {acc_sec} seconds.")
    ...
    def wear_detect(self, model_path=None):
        ...
        # get first channel
        input_eeg = self.eeg[0]
        epochs = input_eeg.shape[0] // (self.sf_eeg * 1)
        input_eeg = input_eeg[0:epochs * self.sf_eeg * 1].reshape(-1, self.sf_eeg)
    ...
    def sleep_staging_with_internal_model(self, use_acc=False, use_time=False, staging_mode='offline'):
        ...
        if self.staging_mode == 'realtime':
            ...
            context_mode = 2
        # get first channel of eeg
        input_eeg = self.eeg[0]
        epochs = input_eeg.shape[0] // (self.sf_eeg * self.epoch_len)
        input_eeg = input_eeg[0:epochs * self.sf_eeg * self.epoch_len].reshape(-1, self.sf_eeg * self.epoch_len)
    ...
    def plot_ar4(self, plot_spectral=True, plot_acc=True, plot_staging=True, plot_variables=True, savefig=None):
        """
        绘制睡眠分析图，包括：
        1. 时频图
        2. 体动差分图
        ...
        subplot_count = plot_acc + 2 * plot_spectral
        fig_height = subplot_count * 4 + (1 if plot_spectral else 0)
        height_ratio = (np.ones(subplot_count) * 4).tolist()
        if plot_spectral:
            height_ratio[0] = 5.0
        ...
        i = 0
        if plot_spectral:
            ax_10 = fig.add_subplot(gs[i, 0])
            plot_spectrogram(fig, ax_10, self.eeg[0], self.start_time, self.sf_eeg)
            i += 1
            ax_11 = fig.add_subplot(gs[i, 0])
            plot_spectrogram(fig, ax_11, self.eeg[1], self.start_time, self.sf_eeg)
            i += 1
        if plot_acc:
            ax_2 = fig.add_subplot(gs[i, 0])
            plot_avg_diff_acc(fig, ax_2, self.acc, self.start_time, self.sf_acc)
        ...
        if savefig is not None:
            plt.savefig(savefig, dpi=300, bbox_inches='tight')
            self.analysis_fig_path = savefig
            self.logger.info("The ar4 plot is saved as {}".format(savefig))
        return self
        
    def export_pdf_ar4(self, pdf_save_path):
        """
        导出睡眠分析结果成PDF，包括各项睡眠指标、睡眠分析图和数据基本信息
        :param pdf_save_path: PDF保存路径
        :return:
