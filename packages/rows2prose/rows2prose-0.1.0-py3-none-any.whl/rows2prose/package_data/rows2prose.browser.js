var r2p = (function (exports) {
  ...
  const play_button_svg = `<svg width="15" height="15"><path d="M 1.7417867 .03596091 C 1.8622672 -.023858173 1.9987557 -.007007727 2.1007008 .07134685 L 13.316347 7.235314 C 13.395544 7.301031 13.446096 7.396236 13.446096 7.499024 C 13.446096 7.600126 13.394702 7.6987015 13.316347 7.763576 L 2.0998583 14.9267 C 2.0408818 14.974724 1.9633698 15 1.889228 15 L 1.7409441 14.966299 C 1.6263612 14.911535 1.5539044 14.791897 1.5539044 14.662149 L 1.5539044 .33842643 C 1.5539044 .21036303 1.6263612 .08988234 1.7417867 .03596091 Z" fill="currentColor"/></svg>`;
  const pause_button_svg = `<svg width="15" height="15"><path d="M 2.671564 8881784e-22 C 2.2758198 39254495e-12 1.9550156 .3208435 1.9549763 .7165877 L 1.9549763 14.283412 C 1.9550156 14.679157 2.2758198 14.99996 2.671564 15 L 5.006161 15 C 5.4019053 14.99996 5.7227096 14.679157 5.722749 14.283412 L 5.722749 .7165877 C 5.7227096 .3208435 5.4019053 39254495e-12 5.006161 8881784e-22 L 2.671564 8881784e-22 Z M 9.993839 8881784e-22 C 9.598095 39254495e-12 9.27729 .3208435 9.277251 .7165877 L 9.277251 14.283412 C 9.27729 14.679157 9.598095 14.99996 9.993839 15 L 12.328436 15 C 12.72418 14.99996 13.044984 14.679157 13.045024 14.283412 L 13.045024 .7165877 C 13.044984 .3208435 12.72418 39254495e-12 12.328436 8881784e-22 L 9.993839 8881784e-22 Z" fill="currentColor"/></svg>`;
  const restart_button_svg = `<svg width="15" height="15"><path d="M 13.163265 7.652841 C 13.164157 10.254344 11.345446 12.502046 8.801026 13.044028 C 6.256605 13.58601 3.679773 12.274595 2.6204082 9.898555 L .9171429 10.590392 C 2.3088786 13.779233 5.746293 15.554905 9.152299 14.844445 C 12.558306 14.133984 14.999179 11.132156 15 7.652841 C 15.000147 4.5090305 12.999983 1.7135174 10.024438 .6987875 C 7.048893 -.31594245 3.757456 .6750114 1.8367347 3.1638612 L 1.8367347 .9181469 L 0 .9181469 L 0 5.816106 L .9183673 6.7344734 L 5.2040816 6.7344734 L 5.2040816 4.897739 L 2.88 4.897739 C 4.1265 2.736772 6.669352 1.682862 9.079067 2.328475 C 11.488782 2.974088 13.164116 5.1581384 13.163265 7.652841 Z" fill="currentColor"/></svg>`;
  ...
  function scalarDistributionView() {
    ...
    function draw(canvasNode, div, points) {
      const min$1 = useDataMin
        ? min(points)
        : scale.domain()[0],
        max$1 = useDataMax
        ? max(points)
        : scale.domain()[1],
        width = scale(max$1) - scale(min$1);
      ...
      if (!canvasNode._r2pInitialized || (useDataMin || useDataMax)) {
        ...
        // toPrecisionThrifty causes too much flickering.
          .text(useDataMin ? min$1.toPrecision(2)
                           : toPrecisionThrifty(min$1, 2));
        div.select(".maxtext")
          .text(useDataMax ? max$1.toPrecision(2)
                           : toPrecisionThrifty(max$1, 2));
        ...
        .call(div => {
          div.each(function(points) {
            ...
            if (this._r2pPrevPoints) {
              const interpolator = numberArray(this._r2pPrevPoints, points);

              let timer$1 = timer((elapsed) => {
                const t = Math.min(1, cubicInOut(elapsed / anim_t));
  ...
  function timeControl() {
    ...
    function render(selection) {
      selection.each(function(d) {
        ...
        select(this).selectAll(".slider_container_container")
          .data([d])
          .join(enter => {
            ...
            slider_container_container.each(function(d) {
              const slider_container_container = select(this),
    ...
    const exprHTML = (function() {
      ...
      parsed.body.childNodes.forEach(node => {
        if (node.nodeType == Node.TEXT_NODE) {
          exprHTML += `<span class="r2p-code">${node.textContent}</span>`;
        } else {
    ...
    function initializeComponents() {
      if (valueType == "scalar") {
        scaleComponent = scalarView()
          .scale(linear().domain([0, 2.5]).range([0, 200]))
          .height(12)
          .fontSize(10);
        ...
        mixingWeightComponent = scalarDistributionView()
          .scale(linear().domain([0, 1]).range([0, 50]))
          .height(12)
          .fontSize(10);
      ...
      selection.each(function(model) {
        ...
        if (valueType == "scalar") {
          ...
          let scaleValue = expression.selectAll("span.scale-value"),
              ...
              min(data.map(d => d.min));
              let globalMax = max(data.map(d => d.max));

          let scaleComponent = scalarDistributionListView()
            .scale(linear().domain([0, globalMax]).range([0, 200]))
            .height(10)
            .useDataMax(true)
            .fontSize(10);
          ...
          if (asynchronous) {
            ...
            scaleValue.data(data)
              .each(function(d, i) {
                setTimeout(() => {
                  select(this).call(scaleComponent);
                  finished[i] = true;
          ...
          if (asynchronous) {
            ...
            mixingWeightValue.data(mwData)
              .each(function(d, i) {
                setTimeout(() => {
                  select(this).call(mixingWeightComponent);
                  finished[i] = true;
                  if (finished.every(d => d)) {
  ...
  exports.hiddenTimeState = hiddenTimeState;
  exports.parseColumns = parseColumns;
  exports.positionView = positionView;
  exports.scalarDistributionListView = scalarDistributionListView;
  exports.scalarDistributionView = scalarDistributionView;
  exports.scalarView = scalarView;
  exports.timeControl = timeControl;

  return exports;

})({});
//# sourceMappingURL=rows2prose.browser.js.map
