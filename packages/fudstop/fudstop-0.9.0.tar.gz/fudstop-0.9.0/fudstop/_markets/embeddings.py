...
async def option_condition_embed(data, hook, conn):
    """Scans real-time option conditions"""
    price_to_strike = data.get('price_to_strike', None)
    if price_to_strike is not None:
        price_to_strike = round(float(price_to_strike),2)
    condition = data.get('conditions')
    webhook = AsyncDiscordWebhook(hook, content=f"<@375862240601047070>")

    embed = DiscordEmbed(title=f"{data.get('underlying_symbol')} | ${data.get('strike')} | {data.get('call_put')} {data.get('expiry')}", description=f"```py\n{option_condition_desc_dict.get(condition)}```", color=hex_colors['yellow'])

    embed.set_timestamp()
    embed.add_embed_field(name=f"Trade Info:", value=f"> Price: **${data.get('price')}**\n> Size: **{data.get('size')}**\n> Exchange: **{data.get('exchange')}**\n> Volume CHange: **{data.get('volume_change', 0)}**")
    embed.add_embed_field(name=f"Trade Info:", value=f"> Price to Strike: **{price_to_strike}**\n> Price Change: **{data.get('price_change', 0)}**")

    embed.add_embed_field(name=f"Condition:", value=f"> **{data.get('conditions')}**")
    embed.add_embed_field(name=f"Time Info:", value=f"> Weekday: **{data.get('weekday')}**\n> Hour of Day: **{data.get('hour_of_day')}**")
    embed.set_footer(text=f"{data.get('option_symbol')} | {data.get('underlying_symbol')} | {data.get('strike')} | {data.get('call_put')} | {data.get('expiry')} | {data.get('conditions')}")
    webhook.add_embed(embed)
    ...
    #     ''', data.get('option_symbol'), data.get('underlying_symbol'), data.get('strike'), data.get('expiry'), data.get('call_put'), data.get('size'), data.get('price'), data.get('conditions'), data.get('timestamp'))
    

async def sized_trade_embed(data):
    ...
    exchange = data.get('exchange')
    price_to_strike = data.get('price_to_strike', 0)
...
async def profit_ratio_98_embed(profit_ratio,conn, cost_dist_98, stock_symbol, price, hex_colors, o, h, l, vol, avg10d, avg3m, buyPct, neutPct, sellPct, fifty_high, fifty_low, timestamp):
    ...
    embed.set_footer(text=f"{stock_symbol} | {profit_ratio} | {timestamp}")
    embed.set_timestamp()
    hook.add_embed(embed)
    ...
    try:
        ...
        # Insert data into the market_data table
        await conn.execute('''
            INSERT INTO profit_ratio(ticker, profit_ratio, timestamp)
            VALUES($1, $2, $3, $4, $5, $6)
            ''', stock_symbol, profit_ratio, timestamp)
...
async def profit_ratio_02_embed(profit_ratio,conn, cost_dist_98, stock_symbol, price, hex_colors, o, h, l, vol, avg10d, avg3m, buyPct, neutPct, sellPct, fifty_high, fifty_low, timestamp):
