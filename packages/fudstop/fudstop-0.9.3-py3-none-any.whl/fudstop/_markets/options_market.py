...
class OptionsMarket:
    def __init__(self, database:str='charlie', user:str='chuck', password:str='fud', port:int=5432, host:str='localhost'):

        self.webull = WebullTrading()
        self.polygon = Polygon(connection_string=os.environ.get('CHARLIE_STRING'))
        ...
        self.wb_opts = DbManager(database=database,user=user,host=host,password=password, port=port)
    ...
    async def consumer(self, queue: asyncio.Queue, conn):
        ...
        while True:
            ...
            if type =='EquityOptionAgg':
                ...
                # if volume >= 100:
                #     exists, deriv_id = await self.wb_opts.get_ticker_id_by_symbol(wb_symbol)
                #     if exists == True:
            ...
            if type == 'EquityOptionTrade':
                ...
                if size is not None and size >= 100 and conditions in option_conditions_hooks:
                    hook = option_conditions_hooks[conditions]
                    asyncio.create_task(option_condition_embed(data, hook, conn=conn))



                if size is not None and size >= 500 and size <= 999:
                    asyncio.create_task(sized_trade_embed(data, conn))
...
options = OptionsMarket(database='charlie')
...
async def main():
    conn = await options.wb_opts.connect()
