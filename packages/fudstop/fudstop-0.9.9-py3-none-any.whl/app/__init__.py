...
from fudstop.apis.treasury.treasury_sdk import Treasury
...
class FudstopAPP:
    def __init__(self,user,database,password,host:str='localhost',port:int=5432):
        ...
        self.wb_opts = WebullOptions(host=host,user=user)
        self.parser = WebullParser()

        self.ew =EarningsWhisper()
        self.gex= GEXBot()

        self.treasury = Treasury(host=host, user=user,database=database, port=port,password=password)
        self.fed = FedNewyork()
...
df = pd.read_csv('gpts/etf_list.csv')
# Configure caching
...
@app.route('/api/top_options/<rank_type>', methods=['GET'])
...
@app.route('/api/top_gainers/<rank_type>', methods=['GET'])
...
@app.route('/api/top_losers/<rank_type>', methods=['GET'])
...
@app.route('/api/get_ticker_id/<symbol>')
...
@app.route('/api/earnings/<end_date>', methods=['GET'])
...
@app.route('/api/most_active/<rank_type>', methods=['GET'])
...
df_etf = pd.read_csv('gpts/etf_list.csv')

async def is_etf(symbol):
    return symbol in df_etf['Symbol'].values
...
@app.route('/api/etfs/<type>', methods=['GET'])
...
@app.route('/api/bars/<timeframe>/<symbol>', methods=['GET'])
...
@app.route('/api/get_stock_quote/<symbol>', methods=['GET'])
...
@app.route('/api/get_analyst_ratings/<symbol>', methods=['GET'])
...
@app.route('/api/get_short_interest/<symbol>', methods=['GET'])
...
@app.route('/api/institutional_holding/<symbol>', methods=['GET'])
...
@app.route('/api/volume_analysis/<symbol>', methods=['GET'])
...
@app.route('/api/cost_distribution/<symbol>', methods=['GET'])
...
async def async_flask_news(symbol):
    try:
        ...
        return jsonify(e), 400
@app.route('/api/balance_sheet/<symbol>', methods=['GET'])
...
@app.route('/api/cash_flow/<symbol>', methods=['GET'])
...
@app.route('/api/income_statement/<symbol>', methods=['GET'])
...
@app.route('/api/capital_flow/<symbol>', methods=['GET'])
...
@app.route('/api/etfs/etf_holdings/<symbol>', methods=['GET'])
...
@app.route('/api/fed/all_agency_mortgage_backed_securities', methods=['GET'])
...
@app.route('/api/treasury/data_compliance', methods=['GET'])
def data_compliance():
    data = app.fed.data_act_compliance()
    return jsonify(data.to_dict('records'))


@app.route('/api/occ/market_share/<date>', methods=['GET'])
...
@app.route('/api/occ/volume_totals', methods=['GET'])
...
@app.route('/api/occ/threshold_list', methods=['GET'])
...
@app.route('/api/occ/open_interest', methods=['GET'])
async def oi_totals():
...
@app.route('/api/occ/position_limits')
def position_limits():
...
@app.route('/api/technicals/rsi_snapshot/<symbol>')
...
@app.route('/api/technicals/scan_td9/<interval>', methods=['GET'])
...
@app.route('/api/options/get_all_options/<symbol>', methods=['GET'])
...
@app.route('/api/options/get_all_options/<symbol>/download', methods=['GET'])
...
@app.route('/api/options/download_options/<symbol>', methods=['GET'])
...
# @app.route('/api/options/full_skew/<symbol>', methods=['GET'])
# def get_skew(symbol):
...
# @app.route('/api/options/highest_volume/<symbol>', methods=['GET'])
...
@app.route('/api/options/highest_oi/<symbol>', methods=['GET'])
...
@app.route('/api/options/unusual/<symbol>', methods=['GET'])
...
# @app.route('/api/options/gamma/<symbol>', methods=['GET'])
...
@app.route('/api/earnings/top_sentiment', methods=['GET'])
...
@app.route('/api/earnings/upcoming_russell', methods=['GET'])
def upcoming_russell():
    data = app.ew.upcoming_russell()
    return jsonify(data.as_dataframe.to_dict('records'))

@app.route('/api/earnings/upcoming_sectors', methods=['GET'])
def upcoming_sectors():
    data = app.ew.upcoming_sectors()
    return jsonify(data.as_dataframe.to_dict('records'))

@app.route('/api//dated_chart/<string:ticker>/<string:date>', methods=['GET'])
def dated_chart_data(ticker, date):
    data = app.ew.dated_chart_data(ticker,date)
    return jsonify(data.as_dataframe.to_dict('records'))

@app.route('/api/earnings/messages', methods=['GET'])
def messages():
    data = app.ew.messages()
    return jsonify(data.as_dataframe.to_dict('records'))

@app.route('/api/earnings/pivot_list', methods=['GET'])
def pivot_list():
    data = app.ew.pivot_list()
    return jsonify(data.as_dataframe.head(10).to_dict('records'))

@app.route('/api/earnings/todays_results', methods=['GET'])
def todays_results():
    data = app.ew.todays_results()
    return jsonify(data.as_dataframe.to_dict('records'))

@app.route('/api/earnings/calendar/<string:date>', methods=['GET'])
def calendar(date):
...
@app.route('/api/gex/all_major_levels')
...
@app.route('/api/bars/multi_bars/<tickers>/<start>/<end>/<period>', methods=['GET'])
...
@app.route('/discord/reddit_posts')
...
@app.route('/discord/momentum_scalps')
...
@app.route('/discord/option_trades')
