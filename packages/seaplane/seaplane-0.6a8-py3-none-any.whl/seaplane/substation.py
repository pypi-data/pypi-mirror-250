...
SUBSTATION_PROXY_STREAM = "_SEAPLANE_AI_RESULTS"


class Substation:
    """
    Class for handling requests to and responses from Substation proxy
    """
    ...
    def proxy_stream(self) -> str:
        """
        Returns a string with the proxy stream
        """
        return SUBSTATION_PROXY_STREAM
    ...
    def make_request(self, input_data: Dict[str, Any]) -> Dict[Any, Any]:
        """
        Makes the request to substation proxy and returns the request information, including ID
        """
        ...
        if resp.status_code != 200:
            raise SeaplaneError("Error making substation proxy request")
    ...
    def get_response(self, msg: Any) -> Generator[Any, None, None]:
        """
        Use this task to get the completed substation proxy response
        """
        ...
        # If this is a response from Substation proxy it will have "output"
