    ...
    def execute_query_efficient(
        ...
        try:
            ...
            with self._get_bq_client() as client:
                try:
                    try:
                        chalk_logger.info(f"Starting to execute BigQuery query with {client.credentials.service_account_email}")
                    except:
                    ...
                    chalk_logger("Blocking and downloading result table...")
                    table = res.to_arrow()
                    chalk_logger(f"Loaded table with {table.nbytes=}, {table.num_rows=}")
