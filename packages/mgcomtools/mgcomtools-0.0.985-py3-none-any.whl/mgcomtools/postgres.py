import sqlalchemy
from google.cloud.sql.connector import Connector as cnct
from google.cloud.sql.connector import IPTypes as IPT
import os
import pg8000
...
def connect_with_connector() -> sqlalchemy.engine.base.Engine:
    ...
    ip_type = IPT.PRIVATE if os.environ.get("PRIVATE_IP") else IPT.PUBLIC
...
#         FROM `{PROJECT}.{DATASET}.{TABLE_MODIFY}`
#         WHERE 
#             status = 'completed' 
...
#         SELECT 
#             distinct client
#         FROM `{PROJECT}.{DATASET}.{TABLE_MARTS}`,
#         UNNEST(marts) AS marts
#         WHERE marts = '{report_type}'
