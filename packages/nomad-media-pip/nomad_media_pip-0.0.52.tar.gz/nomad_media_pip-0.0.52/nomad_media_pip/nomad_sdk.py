...
class Nomad_SDK:
	...
	Exception: An error is thrown if the API type is not admin.
	"""
	def change_session_status(self, USER_ID: str | None, USER_SESSION_STATUS: str, APPLICATION_ID: str | None):
		try:
			if self.config["apiType"] != "admin":
				raise Exception("This function is only available for admin API type.")
	...
	"""
	def get_user_session(self, USER_ID: str | None):
		try:
			if USER_ID is None:
				USER_ID = self.id
	...
	OUTPUT_FOLDER_ID (string): The output folder ID of the asset.
	...
	DESTINATION_FOLDER_ID (string): The destination folder ID of the asset.
	...
	If the API type is not admin.
	"""
	def copy_asset(self, ASSET_IDS, DESTINATION_FOLDER_ID, BATCH_ACTION,
	...
	If the API type is not admin.
	"""
	def move_asset(self, ASSET_ID, DESTINATION_FOLDER_ID, BATCH_ACTION,
				   CONTENT_DEFINITION_ID, SCHEMA_NAME, RESOLVER_EXCEMPT):
		try:
			if self.config["apiType"] != "admin":
				raise Exception("This function is only available for admin API type.")
	...
	"""
	def media_search(self, QUERY, IDS, SORT_FIELDS, OFFSET, SIZE):
		try:
			if self.config["apiType"] != "portal":
				raise Exception("This function is only available for portal API type.")
	...
	"""
	def misc_function(self, URL_PATH, METHOD, BODY, NOT_API_PATH):
		try:
