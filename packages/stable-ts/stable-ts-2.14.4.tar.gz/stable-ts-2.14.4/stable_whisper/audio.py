...
def load_audio_in_chunks(
    ...
    """
    Version of :func:`stable_whisper.audio.load_audio` that splits `audio` into chunks.
    ...
    -------
    generator
        A generator that yields array containing the audio waveform in float32 up to `chunk_size` in length.
    """
    ...
    try:
        ...
        if inp is None:
            out = out.run_async(cmd=["ffmpeg", "-nostdin"], quiet=True, pipe_stdout=True)
        else:
            out, _ = out.run(cmd=["ffmpeg", "-nostdin"], quiet=True, input=inp)
    ...
    if isinstance(out, bytes):
        def next_chunk():
            nonlocal out
            temp_chunk, out = out[:chunk_size], out[chunk_size:]
            return temp_chunk
    else:
        def next_chunk():
            return out.stdout.read(chunk_size)

    while True:
        if not (chunk := next_chunk()):
            break
        yield np.frombuffer(chunk, np.int16).flatten().astype(np.float32) / 32768.0
