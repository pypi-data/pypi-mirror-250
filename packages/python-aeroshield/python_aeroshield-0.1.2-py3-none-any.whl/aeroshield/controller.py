...
class AeroController:
    ...
    def run(self, freq: int, cycles: int, ref: Optional[float | int | Iterable[float|int]]=None, live_plotter: Optional[LivePlotter]=None):
        ...
        # t1 - tstart, ref, angle, motor, any additional variables
        hist = np.zeros((maxcntr, 4 + extra_hist_size))
        ...
        with self.aero_shield as shield:
            ...
            while not done:
                try:
                    ...
                    self._update_hist(hist, cntr, t1 - tstart, ref[cntr], angle, motor)
    ...
    def _update_hist(self, hist: np.ndarray[float], cntr: int, t: float, ref: float, angle: float, motor: float):
        ...
        :type motor: float
        """
        hist[cntr, 0:4] = t, ref, angle, motor
