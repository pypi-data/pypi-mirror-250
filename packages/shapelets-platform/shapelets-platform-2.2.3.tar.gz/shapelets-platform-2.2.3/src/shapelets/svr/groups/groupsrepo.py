...
def _load_one(conn: Connection, group_id: int = None, group_name: str = None) -> Optional[GroupProfile]:
    """
    Load group details using either its group id or name.
    """
...
def _load_group_by_provider_id(provider_group_id: str, conn: Connection) -> Optional[GroupProfile]:
    """
    Load group using an ID from an external provider like Azure or Google.
    """
    conn.execute(""" 
        SELECT uid, groupName, description
        FROM groups
        WHERE providerGroupId = ?;
    """, [provider_group_id])
...
def _delete_group_by_name(group_name: str, conn: Connection):
    uid = _load_group_by_name(group_name, conn).uid
...
class GroupsRepo(IGroupsRepo):
    ...
    def get_group_by_remote_id(self, provider_group_id: str) -> Optional[GroupProfile]:
        ...
        """
        with transaction() as conn:
            return _load_group_by_provider_id(provider_group_id, conn)
    ...
    def update_group_info(self, new_attributes: GroupProfile):
        ...
        with transaction() as conn:
            current_group = _load_group_by_id(new_attributes.uid, conn)
