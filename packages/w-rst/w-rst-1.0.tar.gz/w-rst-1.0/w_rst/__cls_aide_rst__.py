...
class _cls_rst_files(_cls_base):
	...
	def __init__(self, rst_dir: str = None):
		...
		self.conf_ENVIROMENT = "[ENVIRONMENT]"
	...
	def check_path(self, check_config_file: bool = False, check_log_file: bool = True):
		...
		if check_config_file:
			...
			if not os.path.exists(self.conf_file_path):
				...
				f.writelines("\n")
				f.writelines(f"{self.conf_ENVIROMENT}=1")
...
class _cls_aide_rst_base(_cls_base):
	...
	def __init__(self, module: str, log_level: int = -1):
		...
		if log_level not in [0, 1, 2, 3]:
			if text.find(f"{self.file.conf_ENVIROMENT}=0") >= 0:
				self.log_level = 0
			elif text.find(f"{self.file.conf_ENVIROMENT}=1") >= 0:
				self.log_level = 1
			elif text.find(f"{self.file.conf_ENVIROMENT}=3") >= 0:
				self.log_level = 2
			else:
				self.log_level = 3
		else:
	...
	def set_state(self, new_state: bool = False):
		...
		if self.log_level == 0:
			...
			if new_state in [False]: self.file.addlog(self.all)
		elif self.log_level == 2:
			if new_state in [False, 2]: self.file.addlog(self.all)
		elif self.log_level == 3:
			self.file.addlog(self.all)
