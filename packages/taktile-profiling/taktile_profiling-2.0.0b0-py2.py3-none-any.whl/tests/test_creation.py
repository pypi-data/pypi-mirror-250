...
def test_importance():
    for endpoint in tktl.endpoints:
        ...
        assert Path(BASE_RESULTS_PATH, name, "anatomy", "varimp.html").exists()
    shutil.rmtree(BASE_RESULTS_PATH)
...
def test_accuracy():
    for endpoint in tktl.endpoints:
        ...
        assert Path(BASE_RESULTS_PATH, name, "accuracy", "calibration.html").exists()
        ...
        if endpoint.KIND.value == "binary":
            assert Path(BASE_RESULTS_PATH, name, "accuracy", "roc_curve.html").exists()
    shutil.rmtree(BASE_RESULTS_PATH)
