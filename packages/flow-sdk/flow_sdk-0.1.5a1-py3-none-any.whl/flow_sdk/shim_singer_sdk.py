...
class CaptureShim(Connector):
    ...
    def discover(self, discover: request.Discover) -> response.Discovered:
        ...
        while stack:
            ...
            if entry.key_properties:
            ...
            elif not entry.key_properties and stream.replication_method == REPLICATION_FULL_TABLE:
                ...
                if entry.schema.properties:
                    ...
                    raise RuntimeError(f"Something unexpected happened: schema for stream {stream.name} does not have a properties field.")
            else:
                raise RuntimeError(
                    f"Stream {stream.name} must have a source-defined primary key in order to use {REPLICATION_INCREMENTAL} replication."
                )
