...
class CaptureShim(Connector):
            ...
            if entry.key_properties:
                ...
                if entry.schema.properties is not None:
                    if not "_meta" in entry.schema.properties:
                        entry.schema.properties["_meta"] = ObjectType(Property("_meta", ObjectType(row_id_type), required=True))
                    else:
    ...
    def open(self, open: request.Open, emit: t.Callable[[Response], None]) -> None:
        ...
        index: dict[str, t.List[int]] = state.get("rowIds", {}) # type: ignore
        ...
        delegate: singer_sdk.Tap = self.delegate_factory(config=config, catalog=catalog, state=state.get("bindingStateV1",{})) # type: ignore
                    ...
                    Response(
                        checkpoint=response.Checkpoint(
                            state=flow.ConnectorState(
                                updated={
                                    # TODO: unwrap/re-wrap this state using resourcepathpointers
                                    "bindingStateV1": state.to_dict(),
                                    "rowIds": index
                                },
                                mergePatch=False
                            )
