...
from . import vad, wh_transcribe, vq_stoks, extract_acoustic, utils
...
def flac_to_t2s_name(input):
    if '-flac-' in input:
        ...
        return input.rsplit("/", 1)[1].replace('raw', 't2s') + ".gz"
...
def prepare_t2s(
    ...
    ds = wds.WebDataset(input, shardshuffle=True, rename_files=rename_files).compose(
