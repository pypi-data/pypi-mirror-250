...
# %% auto 0
__all__ = ['load_dataset', 'CMLMVisual', 'Rotary', 'rotate_half', 'apply_rotary_pos_emb', 'ResidualAttentionBlock',
...
# %% ../nbs/4B. Semantic to acoustic token modeling.ipynb 27
def load_dataset(
        shard_spec:str,             # webdataset folder
    ...
    langs = tuple(whisper.tokenizer.LANGUAGES.keys())
    language = langs.index(whisper.tokenizer.TO_LANGUAGE_CODE[language])
...
# %% ../nbs/4B. Semantic to acoustic token modeling.ipynb 47
import pylab as plt
import fastprogress
import IPython
import numpy as np
...
# %% ../nbs/4B. Semantic to acoustic token modeling.ipynb 48
...
# %% ../nbs/4B. Semantic to acoustic token modeling.ipynb 49
...
# %% ../nbs/4B. Semantic to acoustic token modeling.ipynb 50
...
#             spk_embs = self.speaker_embedding(speakers)
            spk_embs = speakers / 200 # use extracted embeddings
...
# %% ../nbs/4B. Semantic to acoustic token modeling.ipynb 51
...
def make_model(size:str, quantizers:int=4, frozen_embeddings_model:str=None, spk_width:int=None, tunables:Tunables=Tunables(), dataset:torch.utils.data.Dataset=None):
    if frozen_embeddings_model:
        vqmodel = vq_stoks.RQBottleneckTransformer.load_model(frozen_embeddings_model)
        model = _make_model(size, quantizers, tunables, dataset, spk_width=spk_width, stoks_codes=vqmodel.vq_codes+1, stoks_width=vqmodel.rq.layers[0]._codebook.embed[0].shape[-1])
        model.load_frozen_semantic_embeddings(vqmodel)
    else:
        model = _make_model(size, quantizers, tunables, dataset, spk_width=spk_width)
    return model
