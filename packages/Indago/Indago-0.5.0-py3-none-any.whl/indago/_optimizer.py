...
class Optimizer:
    ...
    _scatter : callable (list of CandidateState)
    ...
    def _initialize_X(self, cS):
        """Private method for initializing CandidateState positions.

        Parameters
        ----------
        cS : list of CandidateState
            Candidates of which X must be initialized.
    ...
    def _evaluate_initial_candidates(self):
        ...
        and evaluates private list of initial candidates (Optimizer._cS0) 
        ...
        if self.X0 is not None:
            ...
            if isinstance(self.X0, int):
                ...
                self._initialize_X(self._cS0)

            self._collective_evaluation(self._cS0)
    ...
    def optimize(self, resume=False, seed=None):
        ...
        ----------
        resume : bool
            If ``True``, the user is intentionally running an optimizer instance 
            which has been run earlier. Otherwise, error is thrown when trying 
            to re-run an optimizer instance.
        seed : int or None
        ...
        if self.status != 'Optimization not started':
            assert resume, \
                ('Unintentionally trying to start the optimizer which was started earlier.'
                 ' Reinitialize the optimizer, or state resume=True in optimize().')
    ...
    def _update_history(self):
        ...
        to current Optimizer.best CandidateState.
    ...
    def plot_history(self, filename=None, title='Optimization convergence'):
        ...
        if ax_f:
            ...
            for ic in range(self.constraints):
                ...
                ax_f.fill_between(E[:i_feasible], f_lim[0], f_lim[1],
                                  color='red', alpha=0.5/self.constraints,
...
class CandidateState:
    ...
    def __str__(self):
        """Method for a useful printout of CandidateState properties. 
        
        Returns
        -------
        printout : str
            String of the fancy table of CandidateState properties.
            
        """

        title = f'Indago {type(self).__name__}'
        if type(self) != CandidateState:
            title += ' (subclass of CandidateState)'
    ...
    def is_feasible(self):
        """
        Determines whether the design vector of a CandidateState is a feasible solution. A feasible solution is
        a solution which satisfies all constraints.

        Returns
        -------
        is_feasible : bool
            ``True`` if the CandidateState design vector is feasible, ``False`` otherwise.
