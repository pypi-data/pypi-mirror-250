...
class MSGS(Optimizer):
    ...
    def _reinit_grid(self, center, scale, grid_action):
        ...
        self.point_y = self._x_to_y(self.point_x)
    ...
    def _run(self):
        ...
        while True:
            ...
            if grid_action == 'shift':
                ...
                if self._reinit_grid(self.point_x.X, scale, grid_action):
                    # Center is changed
                    grid_action = 'none'
                    shift_count += 1
                ...
                if scale > 10e15:
                    ...
                    break
                self._reinit_grid(self.point_x.X, scale, grid_action)
                action = 'find_dir_3p'
                grid_action = 'none'
                shift_count = 0
                ...
                self._reinit_grid(self.point_x.X, scale)
                action = 'find_dir_3p'
                grid_action = 'none'
                shift_count = 0
            ...
            if action == 'find_dir_3p':
                ...
                if list(self.point_y) in self._H:
                    ...
                    # Calculate x_next
                    x_last = self.point_x.copy()
                    self.point_x = self._y_to_x(self.point_y)
                    # Evaluate x_next
                    self._collective_evaluation([self.point_x])
                    self._H.append(list(self.point_y))

                # Set finding new direction if there is no improvement in new point
                if self.point_x <= x_last:
                    action = 'check_dir'
                elif k < k_max:
                    action = 'find_dir_2p'
                else:
