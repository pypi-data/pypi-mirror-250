...
class TaskRpcClient(TaskRpcInterface):
    ...
    def _json_post(self, url, data=None):
        ...
        return self._handle_inner_error(response)
    ...
    @staticmethod
    def _handle_request_error(response: requests.Response):
        if response.status_code >= 300:
            raise RuntimeError(f"RPC internal error: {response.text}")

    @staticmethod
    def _handle_inner_error(response: requests.Response):
        json_body = json.loads(response.text)
        if json_body['code'] == 0 or json_body['code'] == 200:
            return json_body
        raise RuntimeError(f"RPC internal error: {json_body['msg']}")
