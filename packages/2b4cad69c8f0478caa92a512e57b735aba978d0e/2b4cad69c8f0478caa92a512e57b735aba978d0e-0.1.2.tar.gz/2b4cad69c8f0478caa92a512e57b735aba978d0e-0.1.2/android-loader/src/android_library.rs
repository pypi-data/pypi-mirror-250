...
impl AndroidLibrary<'_> {
    ...
    pub unsafe fn dlopen(name: *const c_char) -> *mut c_void {
        ...
        let _path: String;
        let new_path = PathBuf::from("/workspaces/apple-private-apis/applemusic/lib/arm64-v8a/")
            .join(path_str);

        // Convert the PathBuf to a String to ensure it has its own lifetime
        let new_path_str = new_path.to_str().unwrap().to_owned();
    ...
    fn symbol_finder(symbol_name: &str, hooks: &HashMap<String, usize>) -> *const () {
        ...
        if let Some(func) = hooks.get(symbol_name) {
            info!("Foumd symbol {} in hooks", symbol_name);
            *func as *const ()
