		...
		if is_tool("plastimatch") and mask_path == "":
			...
			with tempfile.TemporaryDirectory() as temp_dir:
				...
				for i,ds in enumerate(temporary_rtstructs):
					...
					command += "--output-prefix {} --prefix-format nrrd --output-ss-list {}/image.txt ".format(temp_dir+"/"+i, temp_dir+"/"+i)
					command += "--input-dose-img {}".format(self.path_to_rtdose_dose) + " > /dev/null 2> /dev/null"
					
					subprocess.run(command, shell=True)
				
				mask_paths = [temp_dir+"/"+i+"/"+structure_name+".nrrd" for i in range(len(temporary_rtstructs))]
