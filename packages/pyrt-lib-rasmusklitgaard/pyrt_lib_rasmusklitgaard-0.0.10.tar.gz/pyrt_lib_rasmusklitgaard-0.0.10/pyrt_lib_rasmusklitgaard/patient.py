...
class Patient():
	...
	def __init__(self, 
				...
				wanted_structures: list = None) -> None:
	...
	def collect_metadata(self):
		...
		patient_data_sheet = xlsx_workbook.get_sheet_by_name("Sheet1")
		this_patient_row = -1
	...
	def dvh_above_let_value(self, let_value, organ, pydicom_rtdose="this"):
		self.set_default_rtdoses()
		...
		organ_inds = find_struct_indices_in_rtdose(self.rtstruct, usethis_rtdose, self.get_actual_structure_names(self.wanted_structures)[organ])
		
		dosegrid = usethis_rtdose.pixel_array * usethis_rtdose.DoseGridScaling
		letdgrid = self.rtdose_letd.pixel_array * self.rtdose_letd.DoseGridScaling
		dosegrid_above_let = dosegrid[np.where(letdgrid>=let_value)]
	...
	def set_structure_indices(self): # sets rectum and bladder structures
		indices = {}
		bladder = find_struct_indices_in_rtdose(self.rtstruct, self.rtdose_dose, self.actual_structure_names["Bladder"])
		rectum = find_struct_indices_in_rtdose(self.rtstruct, self.rtdose_dose, self.actual_structure_names["Rectum"])
	...
	def get_indices_in_structure(self, structure):
		...
		if is_tool("plastimatch") and mask_path == "":
			with tempfile.TemporaryDirectory() as temp_dir:
				...
				command += "{} ".format(self.path_to_rtstruct)
				command += "--output-prefix {} --prefix-format nrrd --output-ss-list {}/image.txt ".format(temp_dir, temp_dir)
				command += "--input-dose-img {}".format(self.path_to_rtdose_dose) + " > /dev/null 2> /dev/null"
				
				subprocess.run(command, shell=True)
				
				mask_path = temp_dir+"/"+structure_name+".nrrd"
