...
def perform_gapfilling(model, universe, mid=None, slim=None, minflux=1.0, exr=False, nsol=3, solver='glpk', timeout=120): 
    ...
    def inner_gafiller(model, universe, mid, slim, minflux, exr, nsol, solver):
        ...
        with model, universe: 
            ...
            for i, solution in enumerate(solutions):
                ...
                if i+1 != len(solutions):
                    print()  # newline to separate solutions
    ...
    def worker(results_channel):
        try:
            result = inner_gafiller(model, universe, mid, slim, minflux, exr, nsol, solver)
    ...
    if process.is_alive():  
        process.terminate()
        process.join()
        result = "perform_gapfilling() timed out."
