...
class ImageDataGenerator(Sequence):
    ...
    def __init__(self, dataframe, batch_size, target_size, shuffle=True, image_path_col='image_path', target_col='target', weight_col=None, process_image=None, file_cache=None, n_jobs=-1):
        """
        :param dataframe:
        :param batch_size:
        :param target_size:
    ...
    def __getitem__(self, idx):
        ...
        images = xparallel.x_on_iter(batch, lambda row: self.load_and_preprocess_image(row[self.image_path_col], self.target_size), with_tqdm=False, n_jobs=self.n_jobs)
    ...
    def load_and_preprocess_image(self, image_path, target_size, is_from_cache=False):
        if self.file_cache:
            ...
            val = self._load_and_preprocess_image_do(image_path, target_size)
    ...
    def _load_and_preprocess_image_do(self, image_path, target_size):
        img = load_img(image_path)
        img = xcv.resize_and_center_crop(img, target_size)
