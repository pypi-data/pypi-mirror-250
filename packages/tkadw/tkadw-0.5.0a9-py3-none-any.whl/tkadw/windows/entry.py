...
class AdwEntry(AdwDrawWidget):
    ...
    def _init(self):
        ...
        self.element_widget_entry_var = StringVar(value=self._text)
        self.element_widget_entry = Entry(self, textvariable=self.element_widget_entry_var, borderwidth=0,
                                          show=self._show, insertwidth=1)
        ...
        self.element_entry = self.create_window(
            self.winfo_width() / 2, self.winfo_height() / 2,
            width=self.winfo_width() - self._radius - self._border_width,
            height=self.winfo_height() - self._border_width * 2 - 5,
            window=self.element_widget_entry
        )

        self.element_entry_bottomsheet = self.create_rectangle(
    ...
    def _draw(self, event=None):
        ...
        try:
            ...
            self.element_widget_entry.configure(
            ...
            if __bottomsheet_width <= 0:
                self.itemconfigure(
                    self.element_entry_bottomsheet,
                    state="hidden"
                )
            else:
                self.itemconfigure(
    ...
    def _event_focus_in(self, event=None):
        ...
        self.element_widget_entry.focus_set()
