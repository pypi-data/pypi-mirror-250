...
Note:
1. Do not send SQL statement to the tool, but send natural language question to the tool.
...
2. answer_all：最终答案（如果有文件或图片的下载链接也需要包括在内）
...
根据给定的数据源、元数据和问题生成一个或多个结构化查询语句（structure_queries）用来从数据源中查询出所需要的数据。
若没有相关的表可以查询到所需的数据，则提供可能缺失的表名（possible_missing_table）。

请注意以下准则：
1. 数据处理转化: 将数据操作转换为SELECT查询。例如，添加一列作为现有字段值的平方。
2. 使用聚合函数: 合理利用COUNT、SUM等聚合函数进行数据计算。
3. 限制查询类型: 仅使用SELECT语句，避免使用INSERT、UPDATE、DELETE等。
4. JOIN选择准则: 
  4.1 当两个表的关联字段（如姓名）在两个表中的覆盖程度不确定时，默认使用 FULL JOIN。
  4.2 使用 INNER JOIN 仅当确定需要两个表中都有的记录。
  4.3 使用 LEFT JOIN 或 RIGHT JOIN 当需要包含一个表的所有记录，同时包含另一个表中的匹配记录。
5. 数据源与元数据考量: 根据数据源特性和元数据，决定是否需要多条查询语句才能回答该问题。
...
4. answer_file(excel download link, nullable)
5. answer_image(image download link, nullable)
...
按如下格式返回：
{ 
    "field_full_name_1": "SELECT DISTINCT name FROM school_db.students LIMIT 10",
}
