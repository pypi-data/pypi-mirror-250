...
async def scrape_website(config: Config):
    ...
    async with async_playwright() as playwright:
        ...
        for url, output_file_name in zip(config.urls, config.output_file_name):
            ...
            # Check to see if knowledge is a directory in the project if it is not create it in the root of the project
            if not os.path.exists("knowledge"):
                os.makedirs("knowledge")
            with open(f"knowledge/{output_filename}", "w") as f:
...
def get_output_filename(file_name: str):
    json_files = glob.glob(f"knowledge/{file_name}*.json")
...
#     max_pages_to_crawl=3,  # default is 10
# )
# scrape_it(config)
