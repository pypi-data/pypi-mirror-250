...
def get_class(user_query, table, column, enum, description):
    ...
    description += " " + "'"+column+"_class' column provides whether the content in the '"+column+"' column satisfies the conditions listed in the user query ('True'/'False'/'Undefined');"
...
def schema_gpt(user_query, column_description, description):
    ...
    functions = [
        ...
        {
            "name": "get_class",
            "description": "This function takes in one of the columns as input, checks whether the contents of that column satisfy the conditions provided in the user query that can't be easily checked with other functions, and generates an additional column labeling the content as 'True' or 'False' or 'Undefined'. Don't select this if there are no explicit conditions listed in the user query.",
            "parameters": {
                "type": "object",
                "properties": {
                    "column": {
