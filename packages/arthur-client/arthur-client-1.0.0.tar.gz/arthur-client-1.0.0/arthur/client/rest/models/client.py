...
from arthur.client.rest.models.models import (
    ...
    ModelAttribute,
    ModelDefaultColumns,
    ModelDefaultColumnsPatchRequest,
    ModelExpand,
    ModelHealthResponse,
    ModelObject,
    ModelRequest,
...
class ArthurModelsClient:
    ...
    def get_model_default_columns(self, model_id: str) -> ModelDefaultColumns:
        """
        Returns the default columns to be displayed to the user for the model's \"Inferences Deep Dive\" page
        ...
        parsed_resp: Dict[str, Any] = self.http_client.get(  # type: ignore
            f"/v3/models/{model_id}/default_columns", validation_response_code=200
        )
        return ModelDefaultColumns(**parsed_resp)

    def set_model_default_columns(
        self, model_id: str, json_body: ModelDefaultColumns
    ) -> ModelDefaultColumns:
        """
        Sets the default columns to be displayed to the user for the model's \"Inferences Deep Dive\" page

        Each item in the \"columns\" section of the request corresponds to either a model attribute name or a pre-defined column name for non-attribute columns such as inference_id, partner_inference_id, inference_timestamp, anomaly_score, or raw_anomaly_score. The order of the columns in the request will be the order in which the columns will be displayed in the UI. This request overwrites any existing default columns for the model.
        ...
        parsed_resp: Dict[str, Any] = self.http_client.put(  # type: ignore
            f"/v3/models/{model_id}/default_columns",
            json=json_body.dict(by_alias=True, exclude_none=True),
            validation_response_code=200,
        )
        return ModelDefaultColumns(**parsed_resp)

    def delete_model_default_columns(self, model_id: str) -> Response:
        """
        Deletes the default columns for the model

        This will result in the default columns for the model being reset on the \"Inferences Deep Dive\".
    ...
    def add_or_remove_model_default_columns(
        self, model_id: str, json_body: ModelDefaultColumnsPatchRequest
    ) -> ModelDefaultColumns:
        """
        Removes columns from the existing list of default columns for the model specified in columns_to_remove and appends columns specified in columns_to_add to the end of the list of default columns for the model
