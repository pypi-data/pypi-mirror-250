...
def convert(infile: AnyPath, outfile: AnyPath, *, tecmp_mapping: Optional[Dict[int, str]] = None) -> None:
    """
    Args:
    ...
    if out_types[-2:] == (".unwrap", ".pcapng"):
        tecmp_convert(infile, outfile, tecmp_mapping=tecmp_mapping)
    elif out_types[-1] == ".asc":
        if tecmp_mapping:
            with temp_path(suffix='.unwrap.pcapng') as tmpfile:
                tecmp_convert(infile, tmpfile, tecmp_mapping=tecmp_mapping)
