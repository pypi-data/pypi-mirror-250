...
class PromptManager:
    ...
    def create_prompt(self,
        ...
        """
        def get_adjusted_distribuion(value,chunk_token_distribution_data,max_tokens):
            minimum_completion = max_tokens *.10
            if value:
                new_tokens = num_tokens_from_string(str(value))
                completion_available = chunk_token_distribution_data['completion']['available'] - new_tokens
                if completion_available >= minimum_completion:
                    chunk_token_distribution_data['prompt']['used']=chunk_token_distribution_data['prompt']['used'] + new_tokens
                    chunk_token_distribution_data['prompt']['available']=chunk_token_distribution_data['prompt']['available'] - new_tokens
                    chunk_token_distribution_data['completion']['used'] =chunk_token_distribution_data['completion']['used'] + new_tokens
                    chunk_token_distribution_data['completion']['available'] = chunk_token_distribution_data['completion']['available'] - new_tokens
                else:
                    value = False
            return value,chunk_token_distribution_data
        
        chunk_token_distribution_data=self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]

        self.prompt_as_previous,self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]=get_adjusted_distribuion(prompt_as_previous,chunk_token_distribution_data,self.model_mgr.selected_max_tokens)
        chunk_token_distribution_data=self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]

        self.notation,self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]=get_adjusted_distribuion(notation,chunk_token_distribution_data,self.model_mgr.selected_max_tokens)
        chunk_token_distribution_data=self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]

        self.request_chunks,self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]=get_adjusted_distribuion(request_chunks,chunk_token_distribution_data,self.model_mgr.selected_max_tokens)
        chunk_token_distribution_data=self.chunk_token_distributions[chunk_token_distribution_number][chunk_number]
