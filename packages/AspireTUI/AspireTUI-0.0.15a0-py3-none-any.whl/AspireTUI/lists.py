...
LOG_SEVERITY = [ "DEBUG" , "INFO", "WARNING", "ERROR", "CRITICAL", "FATAL" ]
#
#	Status
#
class StatusEnum(_Enum):
	...
	# Log Level
	class DEBUG(_Enum):
		id = 1000 + int(LOG_LEVEL.DEBUG.value)
		string = f" 🐞 "
	class INFO(_Enum):
		id = 1000 + int(LOG_LEVEL.INFO.value) 
		string = f"ℹ️ℹ️ℹ️"
	class Warning(_Enum):
		id = 1000 + int(LOG_LEVEL.WARNING.value)
		string = f" ⚠️ "
	class ERROR(_Enum):
		id = 1000 + int(LOG_LEVEL.ERROR.value)
		string = f" ❌ "
	class CRITICAL(_Enum):
		id = 1000 + int(LOG_LEVEL.CRITICAL.value)
		string = f" 🔴 "
	class FATAL(_Enum):
		id = 1000 + int(LOG_LEVEL.FATAL.value)
		string = f" ☠️ "
	# Default: Pseudo-Bool
	class Off(_Enum):
		id = int(False) + 10
