...
class DingtalkChatbot(object):
    ...
    def __init__(self, webhook, secret=None, pc_slide=False, fail_notice=False):
        ...
        if len(webhook) == 64:  #位数不够要加前缀
            self.webhook = "https://106.11.23.24/robot/send?access_token=" + webhook
        else:  #位数够了要转换IP
    ...
    def send_markdown_img_without_url(self, title, text, is_at_all=False, at_mobiles=[], at_dingtalk_ids=[], is_auto_at=True, debug=False):
        """
        markdown类型
