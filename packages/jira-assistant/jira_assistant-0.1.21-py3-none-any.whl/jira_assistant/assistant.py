        ...
        for required_field in required_fields:
            ...
            if allowed_values:
                if (
                    ...
                    print(
                        f"{excel_column_name} has not allowed value: {current_value}. Allowed values: {'|'.join(allowed_values)}, ProjectType: {project_type['name']} and IssueType: {input_issue_type}." # pylint: disable=line-too-long
                    )
                    has_not_allowed_value = True
                    break
            if is_array:
                new_story_fields[excel_column["jira_field_mapping"]["path"]] = str(
                    current_value
                ).split("|")
            else:
                new_story_fields[
                    excel_column["jira_field_mapping"]["path"]
                ] = current_value
        # Some special fields
        new_story_fields["project.id"] = project_type["id"]
        if not all_fields_exist:
            print(f"IssueType: {input_issue_type} missing required fields.")
            continue
