...
def _check_allowed_value(
    ...
    allowed_values = jira_field.get("allowedValues")
    if allowed_values:
        if current_value is None or str(current_value) not in allowed_values:
            print(
                f"{excel_column_name} has not allowed value: {current_value}. Allowed values: {'|'.join(allowed_values)}, ProjectType: {project_type_name} and IssueType: {issue_type_name}."  # pylint: disable=line-too-long
            )
            return False
    return True
...
def _create_jira_stories(
    ...
    for story in stories:
        ...
        if input_project_type is None or input_issue_type is None:
            print("Please fulfill ProjectType/IssueType field.")
            continue
        ...
        for required_field in required_fields:
            ...
            if excel_column is None or excel_column["jira_field_mapping"] is None:
                print(
                    f"Excel definition missing required field: {required_field_name}."
                )
            ...
            is_array = required_field["isArray"]
        ...
        for not_required_field in not_required_fields:
            not_required_field_name = not_required_field["id"].replace(" ", "").lower()
            excel_column = excel_definition.get_column_by_jira_field_mapping_name(
                not_required_field_name
            )
            if excel_column is None or excel_column["jira_field_mapping"] is None:
                continue
            excel_column_name = excel_column["name"]
            if not hasattr(story, excel_column_name):
                continue
            # TODO: Check value is not None?
            is_array = not_required_field["isArray"]
            current_value = story[excel_column_name]
            if not _check_allowed_value(
                current_value,
                not_required_field,
