...
def config_search(root: str = "/") -> List[str]:
    ...
    for file in progress_bar(glob.glob("{root}/**".format(root=root), recursive=True)):
...
def verify(
    ...
    module_path = pathlib.Path(function.__module__).parent.resolve()
    ...
    if config_dir is not None:
        ...
        path = config_search(root=module_path)[0]
        config_file = ".".join((function.__name__, ".param.json"))
        if not pathlib.Path(path).joinpath(config_file).exists():
            pass
