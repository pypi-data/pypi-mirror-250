...
class Main:
	...
	def unified(self,line):
		...
		if len(args) > 1:
			if args[0].lower() in ["remove","rm",'del','delete']:
				...
				if len(args) == 2:
					...
					with Session(self.engine) as session:
						...
						print(result.InList)		
				return True
			elif args[0].lower() in ['display_1',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
					...
					with Session(self.engine) as session:
						...
						print(result.Display_1)		
				return True
			elif args[0].lower() in ['#display_2',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
					...
					with Session(self.engine) as session:
						...
						print(result.Display_2)		
				return True
			elif args[0].lower() in ['#display_3',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
					...
					with Session(self.engine) as session:
						...
						print(result.Display_3)	
				return True
			elif args[0].lower() in ['#display_4',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
					...
					with Session(self.engine) as session:
						...
						print(result.Display_4)		
				return True
			elif args[0].lower() in ['display_5',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
					...
					with Session(self.engine) as session:
						...
						print(result.Display_5)		
				return True
			elif args[0].lower() in ['display_6',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
				...
				if len(args) == 2:
					...
					with Session(self.engine) as session:
						...
						print(result.InList)	
				return True
			elif args[0].lower() in ['qty','listQty']:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
				...
				if len(args) == 2:
					...
					with Session(self.engine) as session:
						...
						print(result.Location)
				return True
			elif args[0].lower() in ['upce2upca',]:
				if len(args) == 2:
					with Session(self.engine) as session:
							result=session.query(Entry).filter(Entry.EntryId==int(args[1])).first()
							if result:
			...
			print("-"*10)
			return True
		elif args[0].lower() in ["save_csv","save",]:
