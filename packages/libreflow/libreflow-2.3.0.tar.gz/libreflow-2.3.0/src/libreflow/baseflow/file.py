        ...
        if default_tasks.has_mapped_name(self._task.name()):
            ...
            if dt.files.edits.has_mapped_name(self._file.name()):
                ...
                if df.is_primary_file.get():
                    ...
                    self._file.is_primary_file.set(False)
                    print('Status removed')
                else:
                    df.is_primary_file.set(True)
                    self._file.is_primary_file.set(True)
                    print('Status added')
                dt.files.edits.touch()
                dt.files.touch()
            else:
                if self._file.is_primary_file.get():
                    self._file.is_primary_file.set(False)
                    print('Status removed')
                else:
                    self._file.is_primary_file.set(True)
...
class TrackedFile(File):
    ...
    upload_playblast = flow.Child(UploadPlayblastToKitsu).ui(label='Upload to Kitsu').injectable()
