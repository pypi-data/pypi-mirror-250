...
from .geometry_derived import crossproduct_tour_zaxis_flip
from .geometry_derived import dotproduct_cruise_yaxis_polarity
from .time import utc2win, win2lst, sec2hhmmss
...
def stellar_occultations(utc_start='', utc_end='',
    ...
    if winsiz == 0 and verbose:
        logging.warning(f'No stellar occultatios for {star} with {front_body}.')
    else:
...
def cruise_yaxis_polarity(utc_start='', utc_end='', interval='',  step=3000, verbose=False):
    """
    Calculate the spacecraft's Y-axis polarity between Earth and the Sun.
    ...
    @spice.utils.callbacks.SpiceUDFUNS
    def gfq(et):
        dotproduct = dotproduct_cruise_yaxis_polarity(et=et)
        if dotproduct > 0:
            return -1 * et
        return et
