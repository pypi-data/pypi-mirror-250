...
def dotproduct_cruise_yaxis_polarity(utc: str = '', et = None):
    """
    Calculate the dot product of the spacecraft's Y-axis with the polarity between Earth and the Sun.

    Parameters
    ----------
    utc : str, optional
        UTC time string (default: '').
    et : float or None, optional
        Ephemeris Time (ET) (default: None).

    Returns
    -------
    float
        Dot product of the spacecraft's Y-axis with the polarity between Earth and the Sun.
    """
    if utc:
        et = spice.utc2et(str(utc))

    sc2sun = sun_direction(et=et, frame='J2000')
    sc2earth = earth_direction(et=et, frame='J2000')
    dotxearthsun_sc = spice.vdot(sc2earth, sc2sun)

    return dotxearthsun_sc
...
def ground_track_velocity(time_et, body):
    ...
    for i in range(n_t):
        ...
        state, _ = spice.spkezr(observer, time_et[i], refFrame, 'NONE', body)
        # Compute longitude, latitude, and altitude from the state vector
        lon_tmp, lat_tmp, alt = spice.reclat(state[:3])
        lon[i] = lon_tmp
        lat[i] = lat_tmp
        ...
        temp = np.dot(jacobi, state[2:5])
        geodetic_speed[i,:] = temp
        # Compute the radial and tangential components of the ground track velocity
