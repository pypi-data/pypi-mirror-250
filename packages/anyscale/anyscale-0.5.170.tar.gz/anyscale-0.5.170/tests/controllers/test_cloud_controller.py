...
def test_init(cli_token):
    ...
    with patch.multiple(
        ...
        cloud_controller = CloudController(
            log=None, initialize_auth_api_client=True, cli_token=cli_token
        )
    mock_get_auth_api_client.assert_called_once_with(
        cli_token=cli_token, raise_structured_exception=False
    )
    assert cloud_controller.initialize_auth_api_client is True
...
def test_setup_managed_cloud(
    cloud_setup_failure, enable_head_node_fault_tolerance, is_aioa
) -> None:
    ...
    mock_prepare_for_managed_cloud_setup.assert_called_with(
        mock_region, mock_cloud_name, cluster_management_stack_version, is_aioa
    )
