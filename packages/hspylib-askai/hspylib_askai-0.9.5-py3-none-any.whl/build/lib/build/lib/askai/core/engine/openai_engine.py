...
class OpenAIEngine(Enumeration):
    ...
    def ask(self, question: str) -> str:
        ...
        try:
            response = self._client.chat.completions.create(model=self._model_name, messages=self._chat)
            reply = response.choices[0].message.content
            self._chat.append({"role": "assistant", "content": reply})
        except RateLimitError as error:
            reply = "RateLimitError => " + error.body["message"] if error.body else error
        except APIConnectionError as error:
            reply = "APIConnectionError => " + error.body["message"] if error.body else error
        except APIStatusError as error:
            reply = "APIStatusError => " + error.body["message"] if error.body else error
    ...
    def speak(
        ...
        if not file_is_not_empty(speech_file_path):
            ...
            response = self._client.audio.speech.create(model="tts-1", voice="onyx", input=text)
