...
def _get_grpc_header(engine_ip=None, cluster_uuid=None):
    metadata = []
    if engine_ip:
        metadata.append(('plannerip', engine_ip))
    if cluster_uuid:
        metadata.append(('cluster_uuid', cluster_uuid))
    return metadata
...
class Connection(object):
        ...
        if not self._session_id:
            try:
                ...
                authenticate_response = self._client.authenticate(
                    ...
                    metadata=_get_grpc_header(cluster_uuid=self.cluster_uuid)
                )
    ...
    def clear(self, query_id, engine_ip=None):
        ...
        self._client.clear(
            clear_request,
            metadata=_get_grpc_header(engine_ip=engine_ip, cluster_uuid=self.cluster_uuid)
        )
        self._session_id = None
...
class Cursor(DBAPICursor):
    ...
    @property
    def metadata(self):
        return _get_grpc_header(engine_ip=self._engine_ip, cluster_uuid=self.connection.cluster_uuid)
