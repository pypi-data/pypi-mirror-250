...
class Connection(object):
        ...
        if not self._session_id:
            try:
                ...
                authenticate_response = self._client.authenticate(
                    ...
                    metadata=_get_grpc_header(cluster=self.cluster)
                )
    ...
    def clear(self, query_id, engine_ip=None):
        ...
        self._client.clear(
            clear_request,
            metadata=_get_grpc_header(engine_ip=engine_ip, cluster=self.cluster)
        )
        self._session_id = None
...
class Cursor(DBAPICursor):
    ...
    @property
    def metadata(self):
        return _get_grpc_header(engine_ip=self._engine_ip, cluster=self.connection.cluster)
