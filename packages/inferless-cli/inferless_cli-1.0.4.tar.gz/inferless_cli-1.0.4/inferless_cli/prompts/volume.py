...
def list_files(
    id: str = typer.Option(None, "--id", "-i", help="ID of the volume."),
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                if volume_data["region"] == "AWS":
                    s3_path = s3_path.replace(
                        "volumes/AWS/", f"volumes_temp/AWS/{workspace_id}/{vol_id}/"
                    )
                else:
                    s3_path = s3_path.replace(
                        "volumes/AZURE/", f"volumes_temp/AZURE/{workspace_id}/{vol_id}/"
                    )
                ...
                if all(results):
                    ...
                    vol_id = volume_data["id"]
                    print({"s3_path": base_s3_path, "volume_id": vol_id}, flush=True)
