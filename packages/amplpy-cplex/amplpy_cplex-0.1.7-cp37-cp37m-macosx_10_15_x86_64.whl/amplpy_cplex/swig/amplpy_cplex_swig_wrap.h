/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (https://www.swig.org).
 * Version 4.1.1
 *
 * Do not make changes to this file unless you know what you are doing - modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

#ifndef SWIG_amplpy_cplex_swig_WRAP_H_
#define SWIG_amplpy_cplex_swig_WRAP_H_

#include <map>
#include <string>


class SwigDirector_GenericCallback : public ampls::GenericCallback, public Swig::Director {

public:
    SwigDirector_GenericCallback(PyObject *self);
    virtual int doAddCut(ampls::Constraint const &c,int type);
    virtual int doAddCutSwigPublic(ampls::Constraint const &c,int type) {
      return ampls::GenericCallback::doAddCut(c,type);
    }
    virtual ampls::Variant getValueImpl(ampls::Value::CBValue v);
    virtual ampls::Variant getValueImplSwigPublic(ampls::Value::CBValue v) {
      return ampls::GenericCallback::getValueImpl(v);
    }
    virtual bool canDo(ampls::CanDo::Functionality f);
    virtual int run();
    virtual ~SwigDirector_GenericCallback();
    virtual int setHeuristicSolution(int nvars,int const *indices,double const *values);
    virtual std::vector< double,std::allocator< double > > getSolutionVector();
    virtual int getSolution(int len,double *sol);
    virtual double getObj();
    virtual int getWhere();
    virtual char const *getWhereString();
    virtual char const *getMessage();
    virtual ampls::Where::CBWhere getAMPLWhere();
    virtual ampls::Variant getValue(ampls::Value::CBValue v);
    virtual std::vector< double,std::allocator< double > > getValueArray(ampls::Value::CBValue v);

/* Internal director utilities */
public:
    bool swig_get_inner(const char *swig_protected_method_name) const {
      std::map<std::string, bool>::const_iterator iv = swig_inner.find(swig_protected_method_name);
      return (iv != swig_inner.end() ? iv->second : false);
    }
    void swig_set_inner(const char *swig_protected_method_name, bool swig_val) const {
      swig_inner[swig_protected_method_name] = swig_val;
    }
private:
    mutable std::map<std::string, bool> swig_inner;

#if defined(SWIG_PYTHON_DIRECTOR_VTABLE)
/* VTable implementation */
    PyObject *swig_get_method(size_t method_index, const char *method_name) const {
      PyObject *method = vtable[method_index];
      if (!method) {
        swig::SwigVar_PyObject name = SWIG_Python_str_FromChar(method_name);
        method = PyObject_GetAttr(swig_get_self(), name);
        if (!method) {
          std::string msg = "Method in class GenericCallback doesn't exist, undefined ";
          msg += method_name;
          Swig::DirectorMethodException::raise(msg.c_str());
        }
        vtable[method_index] = method;
      }
      return method;
    }
private:
    mutable swig::SwigVar_PyObject vtable[12];
#endif

};


class SwigDirector_CPLEXCallback : public ampls::CPLEXCallback, public Swig::Director {

public:
    SwigDirector_CPLEXCallback(PyObject *self);
    virtual int doAddCut(ampls::Constraint const &c,int type);
    virtual ampls::Variant getValueImpl(ampls::Value::CBValue v);
    virtual ampls::Variant getValueImplSwigPublic(ampls::Value::CBValue v) {
      return ampls::CPLEXCallback::getValueImpl(v);
    }
    virtual bool canDo(ampls::CanDo::Functionality f);
    virtual bool canDoSwigPublic(ampls::CanDo::Functionality f) {
      return ampls::CPLEXCallback::canDo(f);
    }
    virtual int run();
    virtual ~SwigDirector_CPLEXCallback();
    virtual int setHeuristicSolution(int nvars,int const *indices,double const *values);
    virtual int setHeuristicSolutionSwigPublic(int nvars,int const *indices,double const *values) {
      return ampls::CPLEXCallback::setHeuristicSolution(nvars,indices,values);
    }
    virtual std::vector< double,std::allocator< double > > getSolutionVector();
    virtual int getSolution(int len,double *sol);
    virtual int getSolutionSwigPublic(int len,double *sol) {
      return ampls::CPLEXCallback::getSolution(len,sol);
    }
    virtual double getObj();
    virtual double getObjSwigPublic() {
      return ampls::CPLEXCallback::getObj();
    }
    virtual char const *getWhereString();
    virtual char const *getWhereStringSwigPublic() {
      return ampls::CPLEXCallback::getWhereString();
    }
    virtual char const *getMessage();
    virtual char const *getMessageSwigPublic() {
      return ampls::CPLEXCallback::getMessage();
    }
    virtual ampls::Where::CBWhere getAMPLWhere();
    virtual ampls::Where::CBWhere getAMPLWhereSwigPublic() {
      return ampls::CPLEXCallback::getAMPLWhere();
    }
    virtual ampls::Variant getValue(ampls::Value::CBValue v);
    virtual std::vector< double,std::allocator< double > > getValueArray(ampls::Value::CBValue v);
    virtual std::vector< double,std::allocator< double > > getValueArraySwigPublic(ampls::Value::CBValue v) {
      return ampls::CPLEXCallback::getValueArray(v);
    }
    virtual int run(int threadid);

/* Internal director utilities */
public:
    bool swig_get_inner(const char *swig_protected_method_name) const {
      std::map<std::string, bool>::const_iterator iv = swig_inner.find(swig_protected_method_name);
      return (iv != swig_inner.end() ? iv->second : false);
    }
    void swig_set_inner(const char *swig_protected_method_name, bool swig_val) const {
      swig_inner[swig_protected_method_name] = swig_val;
    }
private:
    mutable std::map<std::string, bool> swig_inner;

#if defined(SWIG_PYTHON_DIRECTOR_VTABLE)
/* VTable implementation */
    PyObject *swig_get_method(size_t method_index, const char *method_name) const {
      PyObject *method = vtable[method_index];
      if (!method) {
        swig::SwigVar_PyObject name = SWIG_Python_str_FromChar(method_name);
        method = PyObject_GetAttr(swig_get_self(), name);
        if (!method) {
          std::string msg = "Method in class CPLEXCallback doesn't exist, undefined ";
          msg += method_name;
          Swig::DirectorMethodException::raise(msg.c_str());
        }
        vtable[method_index] = method;
      }
      return method;
    }
private:
    mutable swig::SwigVar_PyObject vtable[14];
#endif

};


#endif
