...
class CustomAdapter(logging.LoggerAdapter):
    def process(self, msg, kwargs):
        return f"[DatMorph_Logger] {msg}", kwargs


#dm_logger = CustomAdapter(logging.getLogger("DataMorph_Logs"), {})
dm_logger = logging.getLogger("DataMorph_Logger")
myFormatter = logging.Formatter('DataMorph_Logger - %(asctime)s - %(levelname) - %(message)s')
...
class DMRunJavaJarOperator(BashOperator):
    ...
    def execute(self, context):
        try:
            ...
            print(params)
            print(str(params))
            dag_system_params = DM_CLASSNAME_PROP + self.classname_extn + " " + DM_JSONPARAMS_PROP + "\"" + str(
                params).replace("\"", "\\\"") + "\"" + " " + self.systemparams
            ...
            dm_logger.error("Java Node Error", stack_info=True)
            dm_logger.error(traceback.print_exc())
            dm_logger.exception("Java Node Exception")
