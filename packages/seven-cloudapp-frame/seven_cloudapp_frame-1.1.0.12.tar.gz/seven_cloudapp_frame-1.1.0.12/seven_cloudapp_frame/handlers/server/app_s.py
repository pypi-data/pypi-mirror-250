...
@LastEditTime: 2023-12-27 11:35:51
            ...
            if len(app_template_dict_list) > 0:
                ...
                for app_template_dict in app_template_dict_list:
                    if query(app_info_dict_list).count(lambda x: x["template_id"] == app_template_dict["template_id"]) > 0:
                        continue
                    relation_invoke_result_data = top_base_model.instantiate("", user_nick, access_token, app_key, app_secret, is_log, main_user_open_id, app_name=app_template_dict["product_name"], description=app_template_dict["product_desc"], icon=app_template_dict["product_icon"], template_id=app_template_dict["template_id"], template_version=app_template_dict["client_ver"], is_instance=2)
        ...
        if len(app_info_list) > 0:
            ...
            for app_info in app_info_list:
                ...
                if app_template_dict and app_template_dict['client_ver'] != app_info.client_ver:
                    relation_invoke_result_data = top_base_model.version_upgrade(app_info.app_id, app_template_dict['template_id'], app_template_dict['client_ver'], access_token, app_key, app_secret, app_info, is_log)
                    if relation_invoke_result_data.success == False:
