            ...
            for item in page_list:
                try:
                    if item.get("real_name"):
                        item["real_name"] = CryptoHelper.aes_decrypt(item["real_name"], sensitive_encrypt_key)
                    if item.get("telephone"):
                        item["telephone"] = CryptoHelper.aes_decrypt(item["telephone"], sensitive_encrypt_key)
                    if item.get("province"):
                        item["province"] = CryptoHelper.aes_decrypt(item["province"], sensitive_encrypt_key)
                    if item.get("city"):
                        item["city"] = CryptoHelper.aes_decrypt(item["city"], sensitive_encrypt_key)
                    if item.get("county"):
                        item["county"] = CryptoHelper.aes_decrypt(item["county"], sensitive_encrypt_key)
                    if item.get("street"):
                        item["street"] = CryptoHelper.aes_decrypt(item["street"], sensitive_encrypt_key)
                    if item.get("address"):
                        item["address"] = CryptoHelper.aes_decrypt(item["address"], sensitive_encrypt_key)
                except Exception as ex:
                    if self.context:
                        self.context.logging_link_error("【获取订单列表-敏感字段解析】" + traceback.format_exc())
                    elif self.logging_link_error:
                        self.logging_link_error("【获取订单列表-敏感字段解析】" + traceback.format_exc())
