...
@LastEditTime: 2024-01-08 11:28:05
...
class LeftNavigationHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        is_instantiate_relation_app = invoke_result_data.data["is_instantiate_relation_app"] if invoke_result_data.data.__contains__("is_instantiate_relation_app") else False
        app_base_model = AppBaseModel(context=self)
        access_token = self.get_access_token()
        app_key, app_secret = self.get_app_key_secret()
        invoke_result_data = app_base_model.get_left_navigation(self.get_user_nick(), access_token, app_key, app_secret, self.get_app_id(), is_instantiate_relation_app)
        if invoke_result_data.success == False:
