...
@LastEditTime: 2024-01-10 10:32:27
...
class UserBaseModel(FrameBaseModel):
    ...
    def get_user_address_list(self, app_id, act_id, user_id, is_cache=True):
        ...
        # 敏感字段加密
        sensitive_encrypt_key = share_config.get_value("sensitive_encrypt_key")
        if sensitive_encrypt_key :
            for user_address_dict in user_address_dict_list:
                try:
                    user_address_dict["real_name"] = CryptoHelper.aes_decrypt(user_address_dict["real_name"], sensitive_encrypt_key)
                    user_address_dict["telephone"] = CryptoHelper.aes_decrypt(user_address_dict["telephone"], sensitive_encrypt_key)
                    user_address_dict["province"] = CryptoHelper.aes_decrypt(user_address_dict["province"], sensitive_encrypt_key)
                    user_address_dict["city"] = CryptoHelper.aes_decrypt(user_address_dict["city"], sensitive_encrypt_key)
                    user_address_dict["county"] = CryptoHelper.aes_decrypt(user_address_dict["county"], sensitive_encrypt_key)
                    user_address_dict["street"] = CryptoHelper.aes_decrypt(user_address_dict["street"], sensitive_encrypt_key)
                    user_address_dict["address"] = CryptoHelper.aes_decrypt(user_address_dict["address"], sensitive_encrypt_key)
                except Exception as ex:
                    if self.context:
                        self.context.logging_link_error("【获取用户地址列表-敏感字段解析】" + traceback.format_exc())
                    elif self.logging_link_error:
                        self.logging_link_error("【获取用户地址列表-敏感字段解析】" + traceback.format_exc())
    ...
    def save_user_address(self, app_id, act_id, user_id, open_id, user_address_id, real_name, telephone, province, city, county, street, address, is_default, remark=""):
        ...
        sensitive_encrypt_key = share_config.get_value("sensitive_encrypt_key")
        if sensitive_encrypt_key:
            real_name = CryptoHelper.aes_encrypt(real_name, sensitive_encrypt_key)
            telephone = CryptoHelper.aes_encrypt(telephone, sensitive_encrypt_key)
            province = CryptoHelper.aes_encrypt(province, sensitive_encrypt_key)
            city = CryptoHelper.aes_encrypt(city, sensitive_encrypt_key)
            county = CryptoHelper.aes_encrypt(county, sensitive_encrypt_key)
            street = CryptoHelper.aes_encrypt(street, sensitive_encrypt_key)
            address = CryptoHelper.aes_encrypt(address, sensitive_encrypt_key)
