...
class ManifestExcel:
    ...
    @classmethod
    def generate_excel_file_data(cls, alation_schema_id, config, json_data_definition_file_path):
        """
        ...
        with tracer.start_as_current_span("generate_excel_file_data"):
            ...
            try:
                ...
                table = alation_table.Table(
                    None, json_data_definition_file_path)

                # Get tables
                df_tables, hidden_fields, df_table_fields_data_definition = table.fetch_schema_tables_extended(
                    edc_alation_api_token, edc_alation_base_url, alation_datasource_id, alation_schema_id, schema_result)
                ...
                # Get table columns
                df_columns, df_column_fields_data_definition = table.fetch_table_columns_extended(
                    edc_alation_api_token, edc_alation_base_url, alation_datasource_id, alation_schema_id, alation_table_id, schema_result)

                # Return DataFrames
                return df_schema, df_tables, manifest_excel_file, df_table_fields_data_definition, df_columns, df_column_fields_data_definition
    ...
    @classmethod
    def create_excel_from_data(cls, config, df_tables, manifest_excel_file, df_table_fields_data_definition, df_columns, df_column_fields_data_definition):
        """
        ...
        with tracer.start_as_current_span("create_excel_from_data"):
            ...
            df_access_level, excel_data_definition_file = table.fetch_valueset(
                "Access Level")
            df_format, excel_data_definition_file = table.fetch_valueset(
                "Format")
            df_language, excel_data_definition_file = table.fetch_valueset(
                "Language")
            df_steward, excel_data_definition_file = table.fetch_valueset(
                "steward")
            df_update_frequency, excel_data_definition_file = table.fetch_valueset(
                "update_frequency")

            logger.info(
                f"excel_data_definition_file: {excel_data_definition_file}")
            ...
            cls.create_valueset_worksheets(
                workbook, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency)
            ...
            cls.create_worksheet_table(
                list_worksheet, df_list, json_data_definition_file, workbook, df_fields_data_definition, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency, col_init, row_init, header_col_init, "Table")
            ...
            cls.create_worksheet_table(
                list_worksheet, df_list, json_data_definition_file, workbook, df_fields_data_definition, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency, col_init, row_init, header_col_init, "Columns")
    ...
    @classmethod
    def create_valueset_worksheets(cls, workbook, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency):
        ...
        with tracer.start_as_current_span("create_valueset_worksheets"):
            ...
            return ws_vs_status, ws_vs_access_level, ws_vs_format, ws_vs_steward, ws_vs_language, ws_vs_update_frequency

    @classmethod
    def create_worksheet_table_validation_options(cls, headers, list_worksheet, df_list, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency, col_init, row_init, date_fields):
        """
        ...
        with tracer.start_as_current_span("create_worksheet_table_validation_options"):
            ...
            if status_of_dataset_col_num:  # If the "status_of_dataset" column is present
                ...
                list_worksheet.data_validation(
                    f'{xlsxwriter.utility.xl_rowcol_to_cell(row_init + 1, status_of_dataset_col_num)}:{xlsxwriter.utility.xl_rowcol_to_cell(row_init + len(df_list), status_of_dataset_col_num)}', options_status_of_dataset)
            ...
            if access_level_col_num:  # If the "access_level" column is present
                ...
                list_worksheet.data_validation(
                    f'{xlsxwriter.utility.xl_rowcol_to_cell(row_init + 1, access_level_col_num)}:{xlsxwriter.utility.xl_rowcol_to_cell(row_init + len(df_list), access_level_col_num)}', options_access_level)
            ...
            if format_col_num:  # If the "format" column is present
                ...
                list_worksheet.data_validation(
                    f'{xlsxwriter.utility.xl_rowcol_to_cell(row_init + 1, format_col_num)}:{xlsxwriter.utility.xl_rowcol_to_cell(row_init + len(df_list), format_col_num)}', options_format)
            ...
            if language_col_num:  # If the "language" column is present
                ...
                list_worksheet.data_validation(
                    f'{xlsxwriter.utility.xl_rowcol_to_cell(row_init + 1, language_col_num)}:{xlsxwriter.utility.xl_rowcol_to_cell(row_init + len(df_list), language_col_num)}', options_language)
            ...
            date_col_numbers = [i for i, header in enumerate(
                headers) if header in date_fields]
    ...
    @classmethod
    def create_worksheet_table(cls, list_worksheet, df_list, json_data_definition_file, workbook, df_fields_data_definition, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency, col_init, row_init, header_col_init, header_name):
        """
        ...
        with tracer.start_as_current_span("create_excel_from_data"):
            ...
            # Set up formats
            editable_format, readonly_format, date_format, header_text_format = cls.create_excel_formats(
                workbook)
            ...
            list_worksheet.write(
                'E2', f"{header_name} Updates", header_text_format)
            ...
            for i in range(len(max_length) - (header_col_init - 3)):
                list_worksheet.write(1, i + header_col_init,
                                     ' ', header_text_format)
            ...
            date_fields = table.fetch_date_fields(
                df_list, df_fields_data_definition)
            ...
            hidden_fields = [
                col for col in headers if col not in visible_fields]
            ...
            for header_num, header in enumerate(headers):
                ...
                if header in editable_fields:
                    ...
                    list_worksheet.write(row_init, header_num +
            ...
            logger.info(
                f"Number of columns in df_list {num_of_columns}")
            ...
            for row_num, row in enumerate(df_list.itertuples(index=False), start=1):
                for col_num, value in enumerate(row):
                    ...
                    if col_num in date_col_numbers:
                        ...
                        list_worksheet.write(row_num + row_init,
            ...
            status_json = cls.create_worksheet_table_validation_options(
                headers, list_worksheet, df_list, df_status, df_access_level, df_format, df_steward, df_language, df_update_frequency, col_init, row_init, date_fields)
