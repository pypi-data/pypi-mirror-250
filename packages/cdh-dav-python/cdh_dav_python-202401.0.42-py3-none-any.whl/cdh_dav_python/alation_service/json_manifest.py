...
This module also imports the following class objects from the 'cdh_dav_python.cdc_admin_service' module:
...
- The 'Table' class from the 'cdh_dav_python.alation_service.table' module is used for representing tables in the 'ManifestJson' class.
"""
        ...
        with tracer.start_as_current_span("set_alation_data"):
            ...
            try:
                ...
                self.tables = {table.name: table for table in map(
                    lambda t: Table(t, self.data_definition_file_path), manifest_json['tables'])}
        ...
        with tracer.start_as_current_span("get_submitting_user_from_manifest_file"):
            try:
                data_definition_file_path = open(
                    self.data_definition_file_path, encoding='utf-8')
                logger.info(
                    f"data_definition_file_path: {data_definition_file_path}")
    ...
    def validate_manifest(self, manifest_file, data_definition_file_path):
        ...
        with tracer.start_as_current_span("validate_manifest"):
            ...
            try:
                ...
                excel_data_definition_file_json = json.load(
                    excel_data_definition_file)
                manifest_file = open(manifest_file, 'r', encoding='utf-8')
                manifest_json = json.load(manifest_file)

                self.validate_json(
                    manifest_json, excel_data_definition_file_json)
