...
class EnvironmentFile:
    ...
    @staticmethod
    def save_text_to_file(text: str, file_name: str, file_extension: str):
        """
        Save the given text to a file with the specified name and extension.
        ...
        with tracer.start_as_current_span("save_text_to_file"):
            try:
                logger.info("Remove extension if it exists")
                ...
                if os.name == "nt":
                    file_path = f"{file_name_without_extension}.{file_extension}"
                else:
                    file_path = f"{file_name_without_extension}.{file_extension}"
    ...
    @classmethod
    def is_valid_json(cls, file_path):
        """
        ...
        with tracer.start_as_current_span("is_valid_json"):
            try:
                file_path = cls.convert_to_current_os_dir(file_path)
