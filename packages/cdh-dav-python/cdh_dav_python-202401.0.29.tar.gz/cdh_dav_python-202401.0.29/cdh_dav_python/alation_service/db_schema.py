...
class Schema:
    ...
    def fetch_schema(
        ...
        with tracer.start_as_current_span(
            "fetch_schema_" + str(alation_schema_id)
        ):
            try:
                # Log the parameters
                logger.info(
                ...
                logger.info(
                    "edc_alation_base_url: %s", str(edc_alation_base_url)
                )
                ...
                if len(response_schema_json) == 1:
                    ...
                    if "title" in schema_result:
                        ...
                        error_msg = "Failed to get schema:" + str(
                            response_schema_text
                        )
                        error_msg = error_msg + " for api_url: " + str(api_url)
                        error_msg = (
                            error_msg + " for schema_id: " + str(schema_id)
                        )
                        error_msg = (
                            error_msg
                            + " and datasource_id: "
    ...
    def get_excel_manifest_file_path_temp(
        upload_or_download, repository_path, environment, alation_user_id
    ):
        """
    ...
    @staticmethod
    def get_json_data_definition_file_path(repository_path, environment):
        """
        ...
        with tracer.start_as_current_span(
            "get_json_data_definition_file_path"
        ):
            try:
                obj_file = cdc_env_file.EnvironmentFile()
    ...
    def get_excel_data_definition_file_path(repository_path, environment):
        ...
        with tracer.start_as_current_span(
            "get_excel_data_definition_file_path"
        ):
            try:
                obj_file = cdc_env_file.EnvironmentFile()
                data_definition_path = (
                ...
                logger.info("data_definition_path: " + data_definition_path)
                data_definition_xls_file = (
                    "excel_data_definition_for_tables_sql.xlsx"
                )
        ...
        with tracer.start_as_current_span("download_manifest_json"):
            try:
                ...
                json_data_definition_file_path = (
                    cls.get_json_data_definition_file_path(
                            ...
                            if field in schema:
                                ...
                                for custom_field in schema["custom_fields"]:
                                    ...
                                    if (
                                        ...
                                        manifest_dict[field] = custom_field[
                                            "value"
                                        ]
                                ...
                                if not found_custom_field:
                                    ...
                                    if (
                                        ...
                                        manifest_dict[field] = schema_fields[
                                            field
                                        ]
    ...
    def upload_manifest_json(
        cls, metadata_json_data, config, authenticated_user_id
    ):
        """
        ...
        with tracer.start_as_current_span("upload_schema_manifest"):
            try:
                ...
                excel_data_definition_file = (
                    cls.get_excel_data_definition_file_path(
                        repository_path, environment
                    )
                )
                ...
                if (
                    ...
                    schema_key = (
                        str(alation_datasource_id) + "." + encoded_schema_name
                    )
    ...
    def upload_manifest_excel(
        ...
        with tracer.start_as_current_span("upload_schema_manifest"):
            try:
                ...
                manifest_excel = ManifestExcel()
                df_tables = (
                    manifest_excel.read_manifest_excel_file_tables_worksheet(
                        manifest_excel_file_path
                    )
                )
                ...
                if (
                    ...
                    tables_dict = df_tables.set_index("name").to_dict(
                        orient="index"
                    )
