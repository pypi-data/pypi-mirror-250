...
def list_docs(docpath, start=None, end=None, include_string=""):
    """
...
def get_output_docnames(metadata, include_string=""):
    """
    ...
    """
    output_docpaths = []
    for folder_name in ["skip_folder", "output_folder"]:
        # Construct the path for the current folder
        output_path = metadata.input_path.replace("/input_folder", f"/{folder_name}")
...
def get_docpaths_to_process(metadata, include_string=""):
    """
    ...
    input_docpaths = list_docs(metadata.input_path, include_string=include_string)
    # Get names of documents in the output folders
    output_docnames = get_output_docnames(metadata, include_string=include_string)
