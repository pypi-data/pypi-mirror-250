...
class AccessoryInterface(i2c.I2CBusMaster):
    ...
    def get_list_of_available_interfaces(self) -> list[str]:
        ...
        count_ai = self._obj_i2c.GetAccessoryInterfaceInUseCount
        for i in range(count_ai):
            self._list_of_found_ai.append(self._obj_i2c.GetAccessoryInterfaceInUseSerial(i))

        count_ai = self._obj_i2c.GetAccessoryInterfaceAvailableCount
        for i in range(count_ai):
            self._list_of_found_ai.append(self._obj_i2c.GetAccessoryInterfaceAvailableSerial(i))
    ...
    def select_port(self, port_nr: int):
        ...
        ret = self._obj_i2c.I2CSetupMetaData(0,1,self._bus_id,0x70,0,1) # Setup for switch(multiplexer)
        assert ret == 0, "Accessory Interface: Error: Cannot connect to bus switch"
        ret = self._obj_i2c.I2CIsConnected
        ...
        ret = self._obj_i2c.I2CWrite(0,1,[1 << port_nr])
    ...
    def is_slave_device_connected(self) -> bool:
        ...
        assert self._bus_id != i2c.I2CBusID.Unassigned, "Accessory Interface: Error: Slave Access, but connection not assigned"
        ret = self._obj_i2c.I2CSetupMetaData(0,1,self._bus_id,0x57,2,1) # Setup for eeprom
        assert ret == 0, "Accessory Interface: Error: Cannot setup device metadata"
        id_eeprom_connected = self._obj_i2c.I2CIsConnected
    ...
    def get_slave_device_id(self) -> AISlaveIDHeader:
        ...
        ret = self._obj_i2c.I2CSetupMetaData(0,1,self._bus_id,0x57,2,1) # Setup for eeprom
        assert ret == 0, "Accessory Interface: Error: Cannot setup device metadata"
        id_eeprom_data = self._obj_i2c.I2CReadEx(0, 19)
    ...
    def get_bus_addr_of_ai_device(self, serial_no: str) -> int:
        ...
        count_ai = self._obj_i2c.GetAccessoryInterfaceInUseCount
        for i in range(count_ai):
            if serial_no == self._obj_i2c.GetAccessoryInterfaceInUseSerial(i):
                found_bus_addr = self._obj_i2c.GetAccessoryInterfaceInUseI2CBus(i)
                break
        ...
        if found_bus_addr == i2c.I2CBusID.Unassigned:
            count_ai = self._obj_i2c.GetAccessoryInterfaceAvailableCount
            for i in range(count_ai):
                if serial_no == self._obj_i2c.GetAccessoryInterfaceAvailableSerial(i):
                    found_bus_addr = self._obj_i2c.ActivateAccessoryInterfaceAvailable(i)
                    break
