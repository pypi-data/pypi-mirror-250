...
class SpecWorker(nsf_thread.SPMWorker):
    ...
    def do_work(self):
        ...
        if self.connect_to_controller():
            ...
            self.result.amplitude_unit = lockin.input_amp_unit 
            self.result.output_unit = dacout.unit
            ...
            for spec_dir in SpecDir:
                ...
                for cur_step in range(self.cur_steps): 
                    ...
                    self.result.amplitude_on = lockin.input_amp 
                    self.result.phase_on = lockin.input_phase

                    # switch potential back to offset voltage and measure response again
                    dacout.dc = self.cur_offset_voltage 
                    time.sleep(self.cur_time_per_step)
                    self.result.amplitude_off = lockin.input_amp 
                    self.result.phase_off = lockin.input_phase
