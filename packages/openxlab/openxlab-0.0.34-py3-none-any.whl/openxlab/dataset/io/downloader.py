        ...
        def ctrl_c_handler(signal, frame):
            ...
            for thread in threading.enumerate():
                if isinstance(thread, Worker):
                    thread.terminate_flag = True
            sys.exit(1)

        # add signal catching ctrl+c for windows to interrupt thread
        signal.signal(signal.SIGINT, ctrl_c_handler)  # Register Ctrl+C handler
