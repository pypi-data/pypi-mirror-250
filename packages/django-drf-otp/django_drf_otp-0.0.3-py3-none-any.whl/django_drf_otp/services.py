...
class OTPService:
    ...
    def resend_token(self, user: Type[AbstractUser], extra_context: Optional[Dict] = None) -> VerificationToken:
        last_active_token = VerificationToken.objects.filter(user=user, is_active=True).order_by("-created_date").first()

        if last_active_token and (timezone.now() - last_active_token.created_date).seconds < otp_settings.TOTP_TOKEN_TTL:
