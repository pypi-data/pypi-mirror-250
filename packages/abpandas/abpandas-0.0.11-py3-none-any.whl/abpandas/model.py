...
class Model:
    ...
    def manage_class_variables(self, agents: list or Agent):
        """
        Check if the class of the agent is in the model, create space and Model variables and count the agent accordingly
        Forces all created variables to a lowercase (e.g., "House" class becomes Model.houses)
        Not designed to be called by end user
        ...
        if type(agents) == list:
            for agent in agents:
                ...
                if agent.my_class is not None and agent.my_class not in self.__classes:
                    ...
                    # add the class to the geodataframe and the list of Model variables in __dict)__
    ...
    def add_agents(self, agents:Agent or list, loc_index:int= None):
        ...
        if type(agents) == list:
            ...
            if loc_index is not None:
                self.space.at[loc_index, "agents"].append(agents)
                self.space.at[loc_index, "n_agents"] += 1
                agents.location_index = loc_index
