...
class Model:
    ...
    def indeces_in_radius(self, centre: Agent or int or geometry.polygon.Polygon, radius: int or float, outline_only: bool=False, return_patches=False):
        """
        ...
        for i in self.space.index:
            ...
            # include polygons intersecting with the circle
            else:
                if circle.intersects(self.space.at[i, "geometry"])
                    p_in_radius.append(self.space.at[i, "geometry"])
                    i_in_radius.append(i)
