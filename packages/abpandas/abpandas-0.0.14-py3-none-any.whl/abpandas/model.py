...
class Model:
    ...
    def add_agent(self, agent:Agent, loc_index:int= None):
        ...
        try:
            ...
            self._manage_class_variables(agent)
        except ValueError:
    ...
    def _manage_class_variables(self, agents: list or Agent):
        """
        ...
        if type(agents) == list:
            try:
                for agent in agents:
                    ...
                    if agent.my_class is not None and agent.my_class not in self.__classes:
                        ...
                        self.space[f"{self.__classes[-1]}s".lower()] = [list() for i in range(len(self.space.index))]
                        self.space[f"n_{self.__classes[-1]}s".lower()] = [0 for i in range(len(self.space.index))]
                        self.__dict__[f"{self.__classes[-1]}s".lower()] = [agent]
                        if agent.location_index is not None:
                            self.space.at[agent.location_index, f"{self.__classes[-1]}s"].append(agent)
                            self.space.at[agent.location_index, f"n_{self.__classes[-1]}s"] += 1
                        return False
                    # if the class has already been added
                    elif agent.my_class in self.__classes:
                        self.__dict__[f"{self.__classes[-1]}s".lower()].append(agent)
                        if agent.location_index is not None:
                            self.space.at[agent.location_index, f"{agent.my_class}s"].append(agent)
                            self.space.at[agent.location_index, f"n_{agent.my_class}s"] += 1
                        return True
            except ValueError:
            ...
            try:
                ...
                if agent.my_class is not None and agent.my_class not in self.__classes:
                    self.__classes.append(agent.my_class)
                    self.space[f"{self.__classes[-1]}s".lower()] = [list() for i in range(len(self.space.index))]
                    self.__dict__[f"{self.__classes[-1]}s".lower()] = 1
                    if agent.location_index is not None:
                        self.space.at[agent.location_index, f"{self.__classes[-1]}s"] += 1
                    return False
                elif agent.my_class in self.__classes:
                    self.__dict__[f"{self.__classes[-1]}s".lower()] += 1
                    if agent.location_index is not None:
                        self.space.at[agent.location_index, f"{agent.my_class}s"] += 1
                    return True
            except ValueError:
