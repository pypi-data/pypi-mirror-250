...
def get_client(
    client_id: str,
    client_secret: str,
    tenant_id: Optional[Optional[str]] = None,
...
def upload_file(
    ...
    directory: str = "/",
    overwrite: bool = True,
    dataset_id: Optional[Optional[int]] = None,
) -> FileMetadata:
    """
...
def query_file_versions(
    ...
    metadata: Optional[Optional[Dict]] = None,
...
def setup_schedule_for_latest_model_in_prod(
    external_id: str,
    client: Optional[Optional[CogniteClient]] = None,
) -> None:
    if client is None:
        client = global_client["write"]
