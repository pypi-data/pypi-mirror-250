...
class DialogCalendar(GenericCalendar):
    ...
    async def _get_days_kb(self, year: int, month: int, available_dates: list = None):
        ...
        for week in month_calendar:
            ...
            for day in week:
                ...
                # draw empty cell if date is not available
                if (available_dates and datetime(year, month, day).strftime("%Y-%m-%d") not in available_dates):
                    days_row.append(
                        InlineKeyboardButton(
                            text=" ",
                            callback_data=DialogCalendarCallback(
                                act=DialogCalAct.ignore).pack(),
                        )
                    )
                else:
                    days_row.append(
    ...
    async def start_calendar(
        ...
        month: int = None,
        available_dates: list = None,
    ) -> InlineKeyboardMarkup:
        today = datetime.now()
        now_year = today.year

        if month:
            return await self._get_days_kb(year, month, available_dates)
        kb = []
