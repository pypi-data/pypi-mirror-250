...
#from drawranflow.servicelogic.handlers.nsaProcessPackets import nsaPacketHandler as phnsa
...
from .models import UploadedFile, Identifiers, Message, IdentifiersNsa, MessageNsa,Identifierslte,Messagelte
...
def process_file_async(item):
    try:
        ...
        return {'message_type': 'success', 'message_text': f'{item.filename} process completed successfully'}
    except Exception as e:
...
def fetch_packet_data(request):
    ...
    if associated_data:
        ...
        return render(request, 'drawSequence_sa.html', context)
    else:
        return render(request, 'drawSequence_sa.html')
...
def draw_sequence_view_nsa(request, main_id):
    if main_id is not None:
        ...
        return render(request, 'nsa_draw_sequence.html', context)
    else:
        return render(request, 'nsa_draw_sequence.html')
