...
class GFAParser:
    ...
    def get_gfa_format(gfa_file_path: str | list[str]) -> str | list[str]:
        ...
        for gfa_file in gfa_file_path:
            ...
            if not gfa_file.endswith('.gfa') or not gfa_file.endswith('.gfa.gz'):
                raise IOError(
            ...
            with open(gfa_file, 'r', encoding='utf-8') if gfa_file.endswith('.gfa') else gz_open(gfa_file, 'rt', encoding='utf-8') as gfa_reader:
