...
class CommonSupportTicketService:
    ...
    def max_active_tickets_opened(cls, user) -> bool:
        ...
        return (
            ...
            <= cls.get_support_ticket_model().objects.filter(
    ...
    def create_ticket(
        ...
        return cls.get_support_ticket_model().objects.create(
            user=user,
    ...
    def create_ticket_message(
        ...
        with transaction.atomic():
            ticket_message = cls.get_support_ticket_message_model().objects.create(
                ticket=ticket,
                text=text,
                sender=sender,
                is_system=is_system,
            ...
            if files:
                cls.get_support_ticket_message_file_model().objects.filter(
    ...
    def take_ticket(cls, manager, ticket) -> None:
        ...
        if ticket.manager:
            raise SupportTicketServiceException(
                message="Заявка занята менеджером '{manager}'.".format(manager=ticket.manager),
            )
    ...
    def close_inactive_tickets(cls) -> None:
        ...
        """
        now = timezone.now()
        for ticket in cls.get_support_ticket_model().objects.filter(
