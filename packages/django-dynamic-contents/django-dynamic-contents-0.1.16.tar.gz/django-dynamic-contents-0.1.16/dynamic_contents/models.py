...
class DynamicContentModelMixin(models.Model):
    ...
    def get_html(self):
        ...
        for part in self.parts.all():
            ...
            replacements["{{" + part.field + "}}"] = replacement

        # format_string 내의 모든 필드를 순차적으로 <0>...</0>, <1>...</1> 등으로 변환
        for idx, (key, value) in enumerate(replacements.items()):
            format_string = format_string.replace(key, f'<{idx}>{value}</{idx}>')
