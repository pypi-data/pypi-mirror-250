...
class DynamicContentModelMixin(models.Model):
    ...
    def get_html(self):
        ...
        content_field = f"content_{current_language}"
        format_string = getattr(self.format, content_field, self.format.content)
        placeholders = {}  # placeholder와 대응하는 part 인덱스를 저장할 딕셔너리
        ...
        for idx, part in enumerate(self.parts.all()):
            ...
            format_string = format_string.replace(placeholder, f'<{idx}>{replacement}</{idx}>', 1)
            placeholders[f'<{idx}>'] = idx

        # placeholder 순서대로 재배치
        for key in sorted(placeholders.keys(), key=lambda k: placeholders[k]):
            idx = placeholders[key]
            format_string = format_string.replace(key, f'<{idx}>', 1)
