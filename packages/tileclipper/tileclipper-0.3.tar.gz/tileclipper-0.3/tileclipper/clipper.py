...
class TileClipper:
    def __init__(self, base_url, bbox, output_folder, max_workers=10,tile_server_type='xyz'):
        self.base_url = base_url
        self.bbox = bbox
        self.output_folder = output_folder
        self.max_workers = max_workers
        self.tile_server_type = tile_server_type
    ...
    def download_tile(self, x, y, zoom):
      ...
      if self.tile_server_type == 'xyz':
          tile_url = f"{self.base_url}{zoom}/{x}/{y}"
      elif self.tile_server_type == 'tms':
          tile_url = f"{self.base_url}{zoom}/{x}/{(2 ** zoom) - y - 1}"  # Conversion for TMS
      else:
          self.logger.error("Unknown tile server type. Provide either 'xyz' or 'tms'.")
          return
    ...
    def download_tiles(self, zoom_start, zoom_end):
        ...
        for zoom_level in range(zoom_start, zoom_end + 1):
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = [executor.submit(self.download_tile, x, y, zoom_level) for x in x_tiles for y in y_tiles]
