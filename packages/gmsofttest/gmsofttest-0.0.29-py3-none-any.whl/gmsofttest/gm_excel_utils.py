...
class ReadExcel:
    ...
    def read_excel_data(self, sheet_name):
        ...
        excel = AnalysisExcel(self.case_file_dir)
...
if __name__ == '__main__':
    case_file_path = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
    ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(os.getcwd())))  # 获取工作项目根目录
    file_path = os.sep.join([ROOT_DIR, 'data', 'stock_a', 'hah.xlsx'])  # 拼接xlxs完整路径
    rd = ReadExcel(file_path)
    a = rd.read_excel_data('Sheet1')
    print(a)
