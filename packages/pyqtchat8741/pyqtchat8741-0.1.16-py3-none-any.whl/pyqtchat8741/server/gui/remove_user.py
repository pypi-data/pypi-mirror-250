...
class DelUserDialog(QDialog):
    ...
    def remove_user(self):
        ...
        username = self.selector.currentText()
        if self.db.check_user_existing(username):
            self.db.remove_user(username)

            if username in self.server.names:
                sock = self.server.names[username]
                del self.server.names[username]
                self.server.remove_client(sock)
            # Рассылаем клиентам сообщение о необходимости обновить справочники
            self.server.service_update_lists()
            self.close()
        else:
            self.messages.critical(self, "Ошибка", "Не удалось удалить пользователя.")
            return
